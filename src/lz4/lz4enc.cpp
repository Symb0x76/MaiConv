...
static void LZ4_putPositionOnHash(U32 offset, U32 h, void *tableBase,
                                  tableType_t const tableType) {
  switch (tableType) {
  default:           /* fallthrough */
  ...
  case byU32: {
    ...
    hashTable[h] = (U32)(offset);
    return;
  }
  case byU16: {
    U16 *hashTable = (U16 *)tableBase;
    assert(offset < 65536);
    hashTable[h] = (U16)(offset);
    return;
  }
  }
}
...
LZ4_FORCE_INLINE void LZ4_putPosition(const void *const p, U32 offset,
...
static int LZ4e_inputBuffer(BYTE *istart, BYTE *&ip, BYTE *imax,
  ...
  if (baseOffset > LZ4_64Klimit)
    baseOffset = LZ4_64Klimit;
  if (baseOffset > absIPOffset)
    baseOffset = absIPOffset;
...
static int LZ4e_outputBuffer(BYTE *ostart, BYTE *&op, BYTE *omax,
...
static U32 LZ4e_Read32Stream(unsigned int pos, LZ4e_instream_t *inStream,
                             int *error) {
...
static uint8_t LZ4e_Read8Stream(unsigned int pos, LZ4e_instream_t *inStream,
                                int *error) {
  uint64_t oldPos = inStream->pos;
  inStream->pos = pos;
  U32 result = 0;
...
static int LZ4e_ReadStreamTemp(unsigned int pos, void *buf, unsigned int len,
                               LZ4e_instream_t *inStream) {
  uint64_t oldPos = inStream->pos;
  inStream->pos = pos;
...
LZ4_FORCE_INLINE
unsigned LZ4e_count(U32 inOffs, U32 matchOffs, BYTE *const bufBase,
                    const U32 bufLen, const BYTE *const inBufBase,
                    const U32 inBufLen, const U32 inBufOffs,
...
LZ4_FORCE_INLINE unsigned int
LZ4_compress_generic(LZ4_stream_t_internal *const cctx,
                     LZ4e_instream_t *const ssource,
                     LZ4e_outstream_t *const sdest, void *const streamBuf,
                     unsigned int streamBufSize, const tableType_t tableType,
  ...
  DEBUGLOG(5, "LZ4_compress_generic: streamBufSize=%u, tableType=%u",
           streamBufSize, tableType);
  if (streamBufSize < LZ4_64Klimit)
    return 0;
  /* Init conditions */
  // if (outputLimited == fillOutput && maxOutputSize < 1) return 0; /*
  // Impossible to store anything */ if ((U32)inputSize >
  // (U32)LZ4_MAX_INPUT_SIZE) return 0;   /* Unsupported inputSize, too large
  // (or negative) */ if ((tableType == byU16) && (inputSize>=LZ4_64Klimit))
  // return 0;  /* Size too large (not within 64K limit) */
  if (tableType == byPtr)
    return 0; // assert(dictDirective==noDict);      /* only supported use case
              // with byPtr */
  ...
  for (;;) {
    ...
    { /* byU32, byU16 */
      ...
      do {
        ...
        U32 const current =
            curOffset + (forwardIp - ipb); //(U32)(forwardIp - base);
        ...
        } else if (LZ4e_Read32Stream(matchIndex, ssource, &error) ==
                       LZ4_read32(ipb) &&
                   likely(!error)) {
          offset = current - matchIndex;
      ...
      while ((curOffset > anchor) & (matchIndex > 0)) {
        ...
        if (matchIndex <= startOffset) {
          if (unlikely(tempLen == 0 || matchIndex <= tempStartOffset)) {
            tempStartOffset =
                (matchIndex < streamBufSize) ? 0 : (matchIndex - streamBufSize);
            tempLen = LZ4e_ReadStreamTemp(tempStartOffset, tempbase,
                                          streamBufSize, ssource);
    ...
    /*if (likely((curOffset - matchIndex) <= (ipb-baseb)))
    {
            DEBUGLOG(6, "match for %x (0x%x) found at %x (0x%x), anchor %x",
    curOffset, LZ4_read32(ipb), matchIndex, LZ4_read32(ipb - (curOffset -
    matchIndex)), anchor);
    }
    else
    {
            int error;
            DEBUGLOG(6, "match for %x (0x%x) found at %x (0x%x), anchor %x",
    curOffset, LZ4_read32(ipb), matchIndex, LZ4e_Read32Stream(matchIndex,
    ssource, &error), anchor);
    }*/
    // while (((curOffset>anchor) & (matchIndex > 0)) &&
    ...
    /* Encode Literals */
    {
      if (unlikely((2 + 1 + LASTLITERALS) + (litLength / 255) + 2 >
                   streamBufSize - (opb - opbaseb))) {
        if (unlikely((2 + 1 + LASTLITERALS) + (litLength / 255) >
                     streamBufSize)) {
          DEBUGLOG(1, "break 11");
          return 0;
        }
      ...
      // since the token at the beginning of the block has to be updated for the
      // match length.written before the literals has to be still accessible at
      // that point.
      DEBUGLOG(6, "seq.start:%x, literals=%x, match.start:%x", (int)(anchor),
               litLength, (int)(curOffset));
    }

  _next_match:
    /* at this stage, the following variables must be correctly set :
     * - ip : at start of LZ operation
     * - match : at start of previous pattern occurence; can be within current
     * prefix, or within extDict
    ...
    unsigned matchCode;
    {
      matchCode = LZ4e_count(curOffset + MINMATCH, matchIndex + MINMATCH,
                             tempbase, streamBufSize / 2, baseb, curSize,
                             curOffset - (ipb - baseb), ssource);
      // The token needs to be updated before flushing the output buffer (which
    ...
    /* Write Literals (if any) */
    {
      ...
      while (litLength > 0) {
        BYTE *pAnchor;
        U32 readLength;
        if (unlikely(anchor + readOffs < (curOffset - (ipb - baseb)) ||
                     anchor + readOffs + litLength + 8 >
                         curOffset + (baseb + curSize - ipb))) {
          if (litLength < streamBufSize)
            readLength = litLength;
          else
            readLength = streamBufSize;
          if ((readLength = LZ4e_ReadStreamTemp(anchor + readOffs, tempbase,
                                                readLength, ssource)) <= 0) {
            DEBUGLOG(1, "break 9");
            return 0;
          }
          if (readLength > litLength)
            readLength = litLength;
          pAnchor = tempbase;
        } else {
    ...
    LZ4_writeLE16(opb, (U16)offset);
    opb += 2;
    {
      // matchCode = LZ4_count(ip+MINMATCH, match+MINMATCH, matchlimit);
      ipb += MINMATCH + matchCode;
      curOffset += MINMATCH + matchCode;
      DEBUGLOG(6, "             with matchLength=%x and match.source=%x",
               matchCode + MINMATCH, matchIndex);
      if (unlikely((1 + LASTLITERALS) + (matchCode >> 8) >
                   streamBufSize - (opb - opbaseb))) {
    ...
    /* Fill table */
    // Since LZ4e_inputBuffer keeps previous data (up to 64K), there should
    // always be 2 bytes behind ipb at this place.
    ...
    { /* byU32, byU16 */
      ...
      if (((tableType == byU16) ? 1 : (matchIndex + MAX_DISTANCE >= current))) {
        int error = 0;
        if (likely((curOffset - matchIndex) <= (ipb - baseb))
                ? (LZ4_read32(ipb - (curOffset - matchIndex)) ==
                   LZ4_read32(ipb))
                : (LZ4e_Read32Stream(matchIndex, ssource, &error) ==
                   LZ4_read32(ipb))) {
          if (unlikely(error)) {
            DEBUGLOG(1, "break 5");
            return 0;
...
_last_literals:
  /* Encode Last Literals */
  {
    ...
    int curSizeTemp = curSize;
    unsigned int curTempOffset = curOffset + (baseb + curSize - ipb);
    while (curSizeTemp == streamBufSize) {
      curSizeTemp =
          LZ4e_ReadStreamTemp(curTempOffset, tempbase, streamBufSize, ssource);
      // if (curSizeTemp >= 0)
      {
    ...
    while (lastRun > 0) {
      ...
      curSizeTemp =
          LZ4e_ReadStreamTemp(curTempOffset, tempbase, batchSize, ssource);
  ...
  totalOutputSize += expectedLen;
  DEBUGLOG(5, "LZ4_compress_generic: compressed %llu bytes into %i bytes",
           (unsigned long long)ssource->pos, totalOutputSize);
  return totalOutputSize; //(int)(((char*)op) - dest);
}

unsigned int LZ4_compress_fast_extState(void *state, LZ4e_instream_t *source,
                                        LZ4e_outstream_t *dest, void *streamBuf,
                                        int acceleration,
  ...
  if (totalSize < LZ4_64Klimit) {
    return LZ4_compress_generic(ctx, source, dest, streamBuf, streamBufSize,
                                byU16, acceleration);
  } else {
    return LZ4_compress_generic(ctx, source, dest, streamBuf, streamBufSize,
                                byU32, acceleration);
...
unsigned int LZ4e_compress_fast(LZ4e_instream_t *source, LZ4e_outstream_t *dest,
                                int acceleration, unsigned int totalSize,
                                unsigned int streamBufSize) {
  unsigned int result;
#if (LZ4_HEAPMODE)
  LZ4_stream_t *ctxPtr = (LZ4_stream_t *)ALLOC(
...
#endif
  result = LZ4_compress_fast_extState(ctxPtr, source, dest, streamBuf,
                                      acceleration, totalSize, streamBufSize);
