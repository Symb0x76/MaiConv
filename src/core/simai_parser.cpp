...
NoteType parse_slide_type(const std::string &token, std::size_t offset,
  ...
  case '<':
    if (start_key == 0 || start_key == 1 || start_key == 6 || start_key == 7) {
...
bool simai::Parser::contains_slide_notation(const std::string &token) {
  return std::any_of(token.begin(), token.end(),
                     [](char ch) { return is_slide_notation_char(ch); });
}

std::vector<std::string>
simai::Parser::each_group_of_token(const std::string &token) {
...
Chart simai::Parser::parse_tokens(
  ...
  for (const auto &raw : tokens) {
    ...
    for (auto group : groups) {
      ...
      if (group.front() == '(' && group.back() == ')') {
        const double bpm =
            to_double(group.substr(1, group.size() - 2), current_bpm);
        current_bpm = bpm;
        ...
        for (std::size_t i = 0; i < segments.size(); ++i) {
          ...
          const int absolute_tick = bar * chart.definition() + tick +
  ...
  if (!saw_end_marker) {
    source_bar_count = std::max(source_bar_count, bar + (tick > 0 ? 1 : 0));
  }
