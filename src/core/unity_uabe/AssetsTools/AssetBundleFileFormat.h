...
struct AssetBundleBlockInfo06 {
  ...
  uint16_t flags; //(flags & 0x40) : is streamed (read in 64KiB blocks); (flags
                  //& 0x3F) :  compression info;
  ...
  // LZMA is used as stream-based compression (the whole file is decompressed
  // before usage),
  //  LZ4 as chunk-based compression (many equally large, independent LZ4 chunks
  //  for random access, decompressed in memory).
};
struct AssetBundleBlockAndDirectoryList06 {
  ...
  ASSETSTOOLS_API bool Read(QWORD filePos, IAssetsReader *pReader,
                            AssetsFileVerifyLogger errorLogger = NULL);
  // Write doesn't compress
  ASSETSTOOLS_API bool Write(IAssetsWriter *pWriter, QWORD &curFilePos,
                             AssetsFileVerifyLogger errorLogger = NULL);
};
...
struct AssetBundleHeader06 {
  ...
  char signature[13]; // 0-terminated; UnityFS, UnityRaw, UnityWeb or
                      // UnityArchive
  ...
  ASSETSTOOLS_API bool ReadInitial(IAssetsReader *pReader,
                                   AssetsFileVerifyLogger errorLogger = NULL);
  ...
  inline QWORD GetBundleInfoOffset() {
    if (this->flags & 0x80) {
      ...
      // if (!strcmp(this->signature, "UnityWeb") || !strcmp(this->signature,
      // "UnityRaw")) 	return 9;
      QWORD ret = static_cast<QWORD>(strlen(minPlayerVersion)) +
                  static_cast<QWORD>(strlen(fileEngineVersion)) +
                  static_cast<QWORD>(0x1A);
      if (this->flags & 0x100)
        ret = (ret + static_cast<QWORD>(0x0A));
      else
        ret = (ret + static_cast<QWORD>(strlen(signature)) +
               static_cast<QWORD>(1));
      if (this->fileVersion >= 7)
        ret = (ret + static_cast<QWORD>(15)) &
              ~static_cast<QWORD>(15); // 16 byte alignment
      return ret;
    }
  }
  inline QWORD GetFileDataOffset() {
    ...
    if (!strcmp(this->signature, "UnityFS") ||
        !strcmp(this->signature, "UnityWeb")) {
      ret = static_cast<QWORD>(strlen(minPlayerVersion)) +
            static_cast<QWORD>(strlen(fileEngineVersion)) +
            static_cast<QWORD>(0x1A);
      if (this->flags & 0x100)
        ret += static_cast<QWORD>(0x0A);
      else
        ret += static_cast<QWORD>(strlen(signature)) + static_cast<QWORD>(1);
      if (this->fileVersion >= 7) {
        ret = (ret + static_cast<QWORD>(15)) &
              ~static_cast<QWORD>(15); // 16 byte alignment of list
      }
    }
    if (!(this->flags & 0x80)) {
      ...
      if (this->flags & 0x200)
        ret = (ret + static_cast<QWORD>(15)) &
              ~static_cast<QWORD>(15); // 16 byte alignment of first block
    }
    return ret;
  }
};
...
struct AssetBundleEntry {
  ...
  uint32_t length;
  char name[1];
  ASSETSTOOLS_API unsigned int
  GetAbsolutePos(AssetBundleHeader03 *pHeader); //, uint32_t listIndex);
  ASSETSTOOLS_API unsigned int
  GetAbsolutePos(class AssetBundleFile *pFile); //, uint32_t listIndex);
};
struct AssetsList {
  uint32_t pos;
  uint32_t count;
  ...
  ASSETSTOOLS_API void Free();
  ASSETSTOOLS_API bool Read(IAssetsReader *pReader, QWORD &curFilePos,
                            AssetsFileVerifyLogger errorLogger = NULL);
  ASSETSTOOLS_API bool Write(IAssetsWriter *pWriter, QWORD &curFilePos,
                             AssetsFileVerifyLogger errorLogger = NULL);
  ASSETSTOOLS_API bool Write(IAssetsReader *pReader, IAssetsWriter *pWriter,
                             bool doWriteAssets, QWORD &curReadPos,
                             QWORD *curWritePos = NULL,
                             AssetsFileVerifyLogger errorLogger = NULL);
};
...
public:
  ...
  // allowCompressed : Don't fail if there are compressed blocks, or if the
  // directory list exceeds maxDirectoryLen. maxDirectoryLen : If the file table
  // is compressed, this specifies the maximum compressed and decompressed
  // length for in-memory decompression.
  ASSETSTOOLS_API bool Read(IAssetsReader *pReader,
                            AssetsFileVerifyLogger errorLogger = NULL,
                            bool allowCompressed = false,
                            uint32_t maxDirectoryLen = 16 * 1024 * 1024);
  ASSETSTOOLS_API bool IsCompressed();
  ASSETSTOOLS_API bool Write(IAssetsReader *pReader, IAssetsWriter *pWriter,
                             class BundleReplacer **pReplacers,
  ...
  // settings : array of ECompressionTypes values, or NULL for default settings;
  // settings and fileTableCompression unused for version 3 bundles
  ASSETSTOOLS_API bool
  Pack(IAssetsReader *pReader, IAssetsWriter *pWriter,
       ECompressionTypes *settings = NULL,
       ECompressionTypes fileTableCompression = COMPRESS_LZ4);
  ASSETSTOOLS_API bool IsAssetsFile(IAssetsReader *pReader,
  ...
  inline const char *GetEntryName(size_t entryIdx) {
    if (bundleHeader6.fileVersion >= 6)
      return (bundleInf6 == nullptr || bundleInf6->directoryCount <= entryIdx)
                 ? nullptr
                 : bundleInf6->dirInf[entryIdx].name;
    else if (bundleHeader6.fileVersion == 3)
      return (assetsLists3 == nullptr || assetsLists3->count <= entryIdx)
                 ? nullptr
                 : assetsLists3->ppEntries[entryIdx]->name;
    return nullptr;
  }
  ASSETSTOOLS_API IAssetsReader *
  MakeAssetsFileReader(IAssetsReader *pReader,
                       AssetBundleDirectoryInfo06 *pEntry);
  ASSETSTOOLS_API IAssetsReader *MakeAssetsFileReader(IAssetsReader *pReader,
                                                      AssetBundleEntry *pEntry);
};
