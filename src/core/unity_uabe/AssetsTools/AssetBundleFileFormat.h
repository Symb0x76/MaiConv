...
struct AssetBundleHeader06
{
	...
	inline QWORD GetBundleInfoOffset()
	{
		...
		else
		{
			...
			QWORD ret = static_cast<QWORD>(strlen(minPlayerVersion)) +
				static_cast<QWORD>(strlen(fileEngineVersion)) + static_cast<QWORD>(0x1A);
			if (this->flags & 0x100)
				ret = (ret + static_cast<QWORD>(0x0A));
			else
				ret = (ret + static_cast<QWORD>(strlen(signature)) + static_cast<QWORD>(1));
			if (this->fileVersion >= 7)
				ret = (ret + static_cast<QWORD>(15)) & ~static_cast<QWORD>(15); //16 byte alignment
			return ret;
		}
	}
	inline QWORD GetFileDataOffset()
	{
		...
		if (!strcmp(this->signature, "UnityFS") || !strcmp(this->signature, "UnityWeb"))
		{
			...
			if (this->fileVersion >= 7)
			{
				ret = (ret + static_cast<QWORD>(15)) & ~static_cast<QWORD>(15); //16 byte alignment of list
			}
		}
		if (!(this->flags & 0x80))
		{
			...
			if (this->flags & 0x200)
				ret = (ret + static_cast<QWORD>(15)) & ~static_cast<QWORD>(15); //16 byte alignment of first block
		}
		return ret;
	}
};
