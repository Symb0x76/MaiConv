...
void AddReopenable(IAssetsReopenable *pReopenable) {
  std::scoped_lock<std::mutex> reopenableInstancesLock(
      reopenableInstancesMutex);
...
void Free_AssetsReopenable(IAssetsReopenable *pObject) { delete pObject; }
...
public:
  AssetsReaderFromFile(const TCHAR *filePath, bool binary,
                       AssetsRWOpenFlags flags) {
    this->flags = flags;
    this->binary = binary;
  ...
  bool Reopen() {
    bool ret = true;
    std::scoped_lock<std::recursive_mutex> fileOperationLock(
        fileOperationMutex);
    if (IsOpen())
      ret = true;
      ...
      FILE *pTempFile = NULL;
      TCHAR mode[3] = {_T('r'), static_cast<TCHAR>(binary ? _T('b') : 0), 0};
  ...
  QWORD Read(QWORD pos, QWORD size, void *outBuffer, bool nullUnread) {
    std::unique_lock<std::recursive_mutex> fileOperationLock(
        fileOperationMutex);
    QWORD ret;
    if (!IsOpen() && !Reopen())
      ret = 0;
  ...
  bool Seek(AssetsSeekTypes origin, long long offset) {
    ...
    case AssetsSeek_Cur:
      if (offset < 0) {
        offset = -offset;
        if (offset < 0)
          return false; // INT64_MIN
        if ((unsigned long long)offset > newPos)
          return false;
    ...
    case AssetsSeek_End:
      if (offset > 0)
        return false;
      std::scoped_lock<std::recursive_mutex> fileOperationLock(
          pBaseReader->fileOperationMutex);
...
IAssetsReader *Create_AssetsReaderFromFile(const wchar_t *filePath, bool binary,
                                           AssetsRWOpenFlags openFlags) {
  if (filePath == NULL)
    return NULL;

  AssetsReaderFromFile *pReader =
      new AssetsReaderFromFile(filePath, binary, openFlags);
...
IAssetsReader *Create_AssetsReaderFromFile(const char *filePath, bool binary,
                                           AssetsRWOpenFlags openFlags) {
  if (filePath == NULL)
    return NULL;
  ...
  IAssetsReader *pReader =
      Create_AssetsReaderFromFile(tcPath, binary, openFlags);
...
protected:
  ...
  bool findSplitFile(QWORD absolutePos, size_t &splitFileIndex,
                     QWORD &splitFileOffset) {
    if (curSplitIndex < splitSizes.size() &&
        splitSizes[curSplitIndex].absolutePos <= absolutePos &&
        splitSizes[curSplitIndex].absolutePos + splitSizes[curSplitIndex].size >
            absolutePos) {
...
public:
  AssetsReaderFromSplitFile(const TCHAR *filePath, bool binary,
                            bool allowUpdate, AssetsRWOpenFlags flags)
      : AssetsReaderFromFile(filePath, binary,
                             flags & (~RWOpenFlags_Unclosable)) {
  ...
  ~AssetsReaderFromSplitFile() { Close(); }
  bool Reopen() {
    bool ret = true;
    ...
    if (IsOpen())
      ...
      if (!AssetsReaderFromFile::Reopen())
        ...
        if (!AssetsReaderFromFile::Seek(AssetsSeek_End, 0) ||
            !AssetsReaderFromFile::Tell(totalFileSize)) {
          ret = false;
      ...
      if (ret && splitSizes.size() > 0) {
        ...
        while ((++curOpenSplitIndex) < 10000) {
          if ((wasOpened && !allowUpdate) &&
              curOpenSplitIndex >= splitSizes.size())
            break;
          FILE *pTempFile = NULL;

          _sntprintf(&filePath[filePathLen - 4], 5, _T("%d"),
                     curOpenSplitIndex);
          filePath[filePathLen] = 0;

          TCHAR mode[3] = {_T('r'), static_cast<TCHAR>(binary ? _T('b') : 0),
                           0};
            ...
            if (fseek(pTempFile, 0, SEEK_END) == 0) {
              ...
              if (tempSize > 0) {
                ...
                if (!wasOpened || allowUpdate) {
                  ...
                  FileSizeEntry entry = {splitSizes[lastIndex].absolutePos +
                                             splitSizes[lastIndex].size,
                                         (QWORD)tempSize, pTempFile};
                  totalFileSize = entry.absolutePos + entry.size;
                  splitSizes.push_back(entry);
                } else if ((QWORD)tempSize !=
                           splitSizes[curOpenSplitIndex].size) {
                  fclose(pTempFile);
                  pTempFile = NULL;
                }
              } else {
      ...
      if (wasOpened) {
        if (splitSizes.size() > 0) {
          ...
          if (!findSplitFile(absoluteSplitFilePos, curSplitIndex,
                             curSplitPos) ||
              (_fseeki64(splitSizes[curSplitIndex].pFile,
                         (long long)curSplitPos, SEEK_SET) != 0)) {
            Close();
            ret = false;
          }
        } else {
  ...
  bool IsOpen() { return AssetsReaderFromFile::IsOpen(); }
  bool Close() {
    bool ret;
    std::unique_lock<std::recursive_mutex> fileOperationLock(fileOperationMutex,
                                                             std::defer_lock);
    ...
    if (offset < 0)
      ...
      case AssetsSeek_End: {
        if (offset > 0)
          ...
          if (tempAbsOffset < 0) // if offset is the minimum signed long long
            ...
            if (splitSizes.size() > 0) {
              QWORD endPos = splitSizes[splitSizes.size() - 1].absolutePos +
                             splitSizes[splitSizes.size() - 1].size;
      ...
      case AssetsSeek_Cur: {
        if (offset == 0) {
          ...
          QWORD curAbsolutePos =
              splitSizes[curSplitIndex].absolutePos + curSplitPos;
        ...
        } else // if (offset < 0)
        {
          ...
          if (tempRemainingOffset <
              0) // if offset is the minimum signed long long
            ret = false;
          else {
            QWORD curAbsolutePos =
                splitSizes[curSplitIndex].absolutePos + curSplitPos;
  ...
  bool SetPosition(QWORD pos) {
    ...
    if (((long long)pos) < 0) //_fseeki64 uses a signed offset
      ...
      if (findSplitFile(pos, curSplitIndex, curSplitPos) &&
          (_fseeki64(splitSizes[curSplitIndex].pFile, (long long)curSplitPos,
                     SEEK_SET) == 0))
        ret = true;
      else
        ret = false;
  ...
  QWORD Read(QWORD pos, QWORD size, void *outBuffer, bool nullUnread) {
    ...
    if (!IsOpen() && !Reopen())
      ...
      ret = 0;
             ...
             (curSplitPos > splitSizes[curSplitIndex].size))
      ret = 0;
    else {
      QWORD remainingSize = size;
      while (remainingSize > 0) {
        QWORD curReadSize = splitSizes[curSplitIndex].size - curSplitPos;
        if (curReadSize > remainingSize)
          curReadSize = remainingSize;
        if (curReadSize > 0) {
          if (splitSizes[curSplitIndex].pFile == NULL)
            break;
          QWORD read = fread(&((uint8_t *)outBuffer)[size - remainingSize], 1,
                             curReadSize, splitSizes[curSplitIndex].pFile);
          curSplitPos += read;
          remainingSize -= read;
        ...
        if (curSplitPos >= splitSizes[curSplitIndex].size) {
          if (splitSizes.size() > (curSplitIndex + 1)) {
            curSplitIndex++;
            curSplitPos = 0;
            if (splitSizes[curSplitIndex].pFile)
              fseek(splitSizes[curSplitIndex].pFile, 0, SEEK_SET);
          } else
            break;
        }
      }
...
public:
  AssetsReaderFromSplitFile_View(AssetsReaderFromSplitFile *pBaseReader)
      : pBaseReader(pBaseReader), filePos(0), splitFileIndex(0),
        splitFilePos(0) {}
  ...
  bool Seek(AssetsSeekTypes origin, long long offset) {
    ...
    case AssetsSeek_End:
      ...
      if (offset < 0)
        return false; // INT64_MIN
      if ((unsigned long long)offset > pBaseReader->totalFileSize)
        return false;
...
IAssetsReader *Create_AssetsReaderFromSplitFile(const wchar_t *filePath,
                                                bool binary, bool allowUpdate,
                                                AssetsRWOpenFlags openFlags) {
  if (filePath == NULL)
    return NULL;
  ...
  AssetsReaderFromSplitFile *pReader = new AssetsReaderFromSplitFile(
      tempFilePath.c_str(), binary, allowUpdate, openFlags);
...
IAssetsReader *Create_AssetsReaderFromSplitFile(const char *filePath,
                                                bool binary, bool allowUpdate,
                                                AssetsRWOpenFlags openFlags) {
  if (filePath == NULL)
    return NULL;
  ...
  IAssetsReader *pReader =
      Create_AssetsReaderFromSplitFile(tcPath, binary, allowUpdate, openFlags);
...
public:
  AssetsReaderFromMemory(const void *buf, size_t bufLen, bool copyBuf,
                         cbFreeMemoryResource freeBufCallback)
  ...
  QWORD Read(QWORD pos, QWORD size, void *outBuffer, bool nullUnread) {
    ...
    if ((pos != (QWORD)-1) && !SetPosition(pos))
      ...
      memcpy(outBuffer, &((uint8_t *)this->buf)[this->readerPos],
             (size_t)actualSize);
      ret = actualSize;
      this->readerPos += ret;
    }
  ...
  bool HasBuffer() { return buf != NULL || bufLen == 0; }
...
public:
  ...
  bool Seek(AssetsSeekTypes origin, long long offset) {
    ...
    case AssetsSeek_End:
      ...
      if (offset < 0)
        return false; // INT64_MIN
      if ((unsigned long long)offset > pBaseReader->bufLen)
        return false;
...
IAssetsReader *
Create_AssetsReaderFromMemory(const void *buf, size_t bufLen, bool copyBuf,
                              cbFreeMemoryResource freeBufCallback) {
  if (buf == NULL && bufLen > 0)
    return NULL;

  AssetsReaderFromMemory *pReader =
      new AssetsReaderFromMemory(buf, bufLen, copyBuf, freeBufCallback);
...
public:
  AssetsReaderFromReaderRange(IAssetsReader *pChild, QWORD rangeStart,
                              QWORD rangeSize, bool alwaysSeekChild) {
    this->pChild = pChild;
  ...
  bool Seek(AssetsSeekTypes origin, long long offset, QWORD &newPos) {
    ...
    case AssetsSeek_Cur: {
      ...
      } else if ((unsigned long long)offset >
                 (this->rangeSize - this->readerPos))
        ret = false;
      else {
        this->readerPos += (unsigned long long)offset;
        this->doSeek = true;
  ...
  QWORD Read(QWORD pos, QWORD size, void *outBuffer, bool nullUnread) {
    ...
    if (pChild == NULL)
      ...
      ret = 0;
    // else if ((doSeek || alwaysSeek) && !pChild->SetPosition(this->rangeStart
    // + this->readerPos)) 	ret = 0;
    else {
      if (pos == (QWORD)-1)
        pos = this->readerPos;
      QWORD actualSize = size;
      ...
      QWORD readPos =
          (doSeek || alwaysSeek) ? (this->rangeStart + pos) : ((QWORD)-1);
  ...
  IAssetsReader *CreateView() {
    ...
    alwaysSeek = true;
    return Create_AssetsReaderFromReaderRange(pChild, rangeStart, rangeSize,
                                              true);
  }
};

IAssetsReaderFromReaderRange *
Create_AssetsReaderFromReaderRange(IAssetsReader *pChild, QWORD rangeStart,
                                   QWORD rangeSize, bool alwaysSeek) {
  if (pChild == NULL)
    return NULL;

  AssetsReaderFromReaderRange *pReader = new AssetsReaderFromReaderRange(
      pChild, rangeStart, rangeSize, alwaysSeek);
...
public:
  AssetsWriterToFile(const TCHAR *filePath, bool discard, bool binary,
                     AssetsRWOpenFlags flags) {
    this->flags = flags;
  ...
  bool Reopen() {
    ...
    if (IsOpen())
      ...
      if (discard) {
        ...
        // Without this, unpredictable behaviour would occur since a Close()
        // call can be issued by garbage collection at any time.
        discard = false;
      } else {
        mode[0] = _T('r');
        mode[1] = _T('+');
...
IAssetsWriter *Create_AssetsWriterToFile(const wchar_t *filePath, bool discard,
                                         bool binary,
                                         AssetsRWOpenFlags openFlags) {
  if (filePath == NULL)
    return NULL;

  AssetsWriterToFile *pWriter =
      new AssetsWriterToFile(filePath, discard, binary, openFlags);
...
IAssetsWriter *Create_AssetsWriterToFile(const char *filePath, bool discard,
                                         bool binary,
                                         AssetsRWOpenFlags openFlags) {
  if (filePath == NULL)
    return NULL;
  ...
  IAssetsWriter *pWriter =
      Create_AssetsWriterToFile(tcPath, discard, binary, openFlags);
...
public:
    ...
    case AssetsSeek_Cur: {
      if (offset < 0) {
        ...
        unsigned long long _offset =
            (unsigned long long)offset + this->writerPos;
  ...
  bool IsDynamicBuffer() { return isDynamic; }
...
IAssetsWriterToMemory *Create_AssetsWriterToMemory(void *buf, size_t bufLen,
                                                   size_t initialDataLen) {
  if (buf == NULL && bufLen > 0)
    return NULL;
  if (initialDataLen > bufLen)
    return NULL;

  AssetsWriterToMemory *pWriter =
      new AssetsWriterToMemory(buf, bufLen, initialDataLen);

  // AssetsWriterToMemory isn't garbage collectable
  // AddReopenable(pWriter);
  return pWriter;
}
IAssetsWriterToMemory *Create_AssetsWriterToMemory(size_t initialLen,
                                                   size_t maximumLen) {
  if (initialLen > maximumLen)
    return NULL;

  AssetsWriterToMemory *pWriter =
      new AssetsWriterToMemory(maximumLen, initialLen);
...
public:
  AssetsWriterToWriterOffset(IAssetsWriter *pChild, QWORD offset,
                             bool alwaysSeekChild) {
    this->pChild = pChild;
    ...
    if (pChild == NULL)
      ...
      case AssetsSeek_Begin: {
        if (offset < 0)
          ...
          if ((ret = pChild->Seek(AssetsSeek_Begin,
                                  this->childOffset + offset))) {
            QWORD tempPos = this->childOffset + (size_t)offset;
            if (pChild->Tell(tempPos) &&
                ((tempPos - this->childOffset) != (size_t)offset)) {
              this->doSeek = true;
              ret = false;
            } else {
              this->writerPos = (size_t)offset;
      ...
      case AssetsSeek_Cur: {
        if (offset < 0) {
          ...
          if ((ret = pChild->Seek(AssetsSeek_Cur, offset))) {
            ...
            if (pChild->Tell(tempPos) &&
                ((tempPos - this->childOffset) != this->writerPos)) {
              this->doSeek = true;
              ret = false;
            } else {
              this->writerPos += (unsigned long long)offset;
  ...
  QWORD Write(QWORD pos, QWORD size, const void *inBuffer) {
    ...
    if (pChild == NULL)
      ...
      QWORD writePos = (doSeek || alwaysSeek)
                           ? (this->childOffset + this->writerPos)
                           : ((QWORD)-1);
  ...
  bool Flush() { return (pChild != NULL) && pChild->Flush(); }
...
IAssetsWriterToWriterOffset *
Create_AssetsWriterToWriterOffset(IAssetsWriter *pChild, QWORD offset,
                                  bool alwaysSeek) {
  if (pChild == NULL)
    return NULL;

  AssetsWriterToWriterOffset *pWriter =
      new AssetsWriterToWriterOffset(pChild, offset, alwaysSeek);
