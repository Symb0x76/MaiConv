...
ASSETSTOOLS_API AssetTypeValue::AssetTypeValue(EnumValueTypes type,
                                               void *valueContainer) {
  // freeValue = false;
  this->type = type;
  Set(valueContainer);
...
ASSETSTOOLS_API void AssetTypeValue::Set(void *valueContainer,
                                         EnumValueTypes contType) {
  bool mismatch = false;
  ...
  case ValueType_UInt16:
    if (contType == ValueType_None ||
        (contType >= ValueType_Int16 && contType <= ValueType_UInt64))
  ...
  case ValueType_UInt32:
    if (contType == ValueType_None ||
        (contType >= ValueType_Int32 && contType <= ValueType_UInt64))
  ...
  case ValueType_UInt64:
    if (contType == ValueType_None ||
        (contType >= ValueType_Int64 && contType <= ValueType_UInt64))
  ...
  case ValueType_ByteArray:
    if (contType == ValueType_None || contType == ValueType_ByteArray)
      memcpy(&this->value.asByteArray, valueContainer,
             sizeof(AssetTypeByteArray));
    else
      mismatch = true;
    break;
  }
  if (mismatch)
    throw AssetTypeValue_ConfusionError(
        "AssetTypeValue::Set: Mismatching value type supplied.");
}
...
ASSETSTOOLS_API AssetTypeValueField *
AssetTypeValueField::operator[](const char *name) const {
  if (childrenCount == -1)
...
ASSETSTOOLS_API AssetTypeValueField *
AssetTypeValueField::operator[](uint32_t index) const {
  if (childrenCount == -1)
    return GetDummyAssetTypeField();
...
ASSETSTOOLS_API void AssetTypeValueField::Read(
    AssetTypeValue *pValue, AssetTypeTemplateField *pTemplate,
    uint32_t childrenCount, AssetTypeValueField **pChildren) {
  this->value = pValue;
...
ASSETSTOOLS_API QWORD AssetTypeValueField::Write(IAssetsWriter *pWriter,
                                                 QWORD filePos,
                                                 bool bigEndian) {
  QWORD qwValueTmp;
  float fValueTmp;
  ...
  if (this->templateField->children.empty() && this->value != NULL &&
      this->value->GetType() != ValueType_ByteArray) {
    switch (this->templateField->valueType) {
    case ValueType_Bool:
    case ValueType_Int8:
    case ValueType_UInt8:
      byValueTmp = this->value->AsInt() & 0xFF;
      pWriter->Write(filePos, 1, &byValueTmp);
      filePos++;
      break;
    case ValueType_Int16:
    case ValueType_UInt16:
      ...
      pWriter->Write(filePos, 2, &dwValueTmp);
      filePos += 2;
      break;
    case ValueType_Int32:
    case ValueType_UInt32:
      ...
      pWriter->Write(filePos, 4, &dwValueTmp);
      filePos += 4;
      break;
    case ValueType_Int64:
    case ValueType_UInt64:
      ...
      pWriter->Write(filePos, 8, &qwValueTmp);
      filePos += 8;
      break;
    case ValueType_Float:
      ...
      pWriter->Write(filePos, 4, &fValueTmp);
      filePos += 4;
      break;
    case ValueType_Double:
      ...
      pWriter->Write(filePos, 8, &dValueTmp);
      filePos += 8;
      break;
    default:
      break;
             ...
             this->value->GetType() == ValueType_String) {
    ...
    pWriter->Write(filePos, 4, &dwValueTmp);
    filePos += 4;
    pWriter->Write(filePos, curStrLen, strVal);
    filePos += curStrLen;
    if ((this->templateField->children.size() == 1) &&
        this->templateField->children[0].align)
      doPadding = true;
  } else if (this->value != NULL &&
             (this->value->GetType() == ValueType_Array ||
              this->value->GetType() == ValueType_ByteArray)) {
    if (this->value->GetType() == ValueType_ByteArray) {
      ...
      pWriter->Write(filePos, 4, &dwValueTmp);
      filePos += 4;
      pWriter->Write(filePos, curByteLen, this->value->AsByteArray()->data);
      filePos += curByteLen;
    } else {
    ...
    if ((this->templateField->children.size() == 1) &&
        this->templateField->children[0].align)
      doPadding =
          true; // For special case: String overwritten with ByteArray value.
  ...
  if (doPadding) {
    ...
    if (paddingLen > 0) {
      dwValueTmp = 0;
      pWriter->Write(filePos, paddingLen, &dwValueTmp);
      filePos += paddingLen;
    }
  }
  return filePos;
...
ASSETSTOOLS_API QWORD AssetTypeValueField::GetByteSize(QWORD filePos) {
  ...
  } else if ((this->templateField->valueType == ValueType_String) &&
             (this->value != NULL)) {
    filePos += 4 + strlen(this->value->AsString());
    if ((this->templateField->children.size() > 0) &&
        this->templateField->children[0].align)
      doPadding = true;
...
AssetTypeValueField *GetDummyAssetTypeField() { return &dummyAssetTypeField; }
...
EnumValueTypes GetValueTypeByTypeName(const char *type) {
  ...
  if (!_stricmp(type, "string")) {
  ...
  } else if (!_stricmp(type, "SInt32") || !_stricmp(type, "int") ||
             !_stricmp(type, "Type*")) {
    ret = ValueType_Int32;
    ...
    ret = ValueType_Int64;
  } else if (!_stricmp(type, "UInt64") || !_stricmp(type, "FileSize") ||
             !_stricmp(type, "unsigned long")) {
...
int _RecursiveGetValueFieldCount(AssetTypeTemplateField *pChild,
                                 IAssetsReader *pReader, QWORD maxFilePos,
                                 QWORD *pFilePos, size_t *pValueByteLen,
                                 size_t *pChildListLen, size_t *pRawDataLen,
                                 bool *pReadFailed, bool endianness) {
  ...
  if (!(*pReadFailed)) {
    ...
    if (pChild->isArray && (pChild->children.size() == 2)) {
      ...
      if ((pChild->children[0].valueType == ValueType_Int32) ||
          (pChild->children[0].valueType == ValueType_UInt32)) {
        pReader->Read(filePos, 4, &arrayLen);
        filePos += 4;
        if (endianness)
        ...
        if (!_stricmp(pChild->type.c_str(), "TypelessData")) {
          ...
          for (uint32_t i = 0; i < arrayLen; i++) {
            ret += _RecursiveGetValueFieldCount(
                &pChild->children[1], pReader, maxFilePos, &filePos,
                &valueByteLen, &childListLen, &rawDataLen, pReadFailed,
                endianness);
        ...
        assert(false);
      // else
      //	MessageBox(0, TEXT("Invalid array value type!"), TEXT("ERROR"),
      //16);
    } else if (pChild->valueType == ValueType_String) {
      unsigned int stringLen;
      pReader->Read(filePos, 4, &stringLen);
      filePos += 4;
      if (endianness)
      ...
      if ((filePos + stringLen) > maxFilePos) {
        *pReadFailed = true;
        ...
        filePos += stringLen;
        if (pChild->align ||
            ((pChild->children.size() > 0) && pChild->children[0].align))
          filePos = (filePos + 3) & (~3);
      ...
      for (uint32_t i = 0; i < (uint32_t)pChild->children.size(); i++) {
        ret += _RecursiveGetValueFieldCount(
            &pChild->children[i], pReader, maxFilePos, &filePos, &valueByteLen,
            &childListLen, &rawDataLen, pReadFailed, endianness);
      }
      if (pChild->align)
        filePos = (filePos + 3) & (~3);
    }
...
QWORD _RecursiveMakeValues(AssetTypeTemplateField *pTemplate,
                           IAssetsReader *pReader, QWORD filePos,
                           QWORD maxFilePos, AssetTypeValueField *pValueFields,
                           uint32_t &valueFieldIndex,
                           AssetTypeValue *&pCurValue,
                           AssetTypeValueField **&pCurValueFieldList,
                           uint8_t *&pCurRawData, bool bigEndian) {
  if (pTemplate->isArray) {
    if (pTemplate->children.size() == 2) {
      if ((pTemplate->children[0].valueType == ValueType_Int32) ||
          (pTemplate->children[0].valueType == ValueType_UInt32)) {
        unsigned int arrayLen;
        pReader->Read(filePos, 4, &arrayLen);
        filePos += 4;
        if (bigEndian)
        ...
        if (!_stricmp(pTemplate->type.c_str(), "TypelessData")) {
          ...
          pReader->Read(filePos, arrayLen, pCurRawData);
          filePos += arrayLen;
          if (filePos <= maxFilePos) {
            pCurRawData = &pCurRawData[arrayLen];
            AssetTypeValue _tmpValue =
                AssetTypeValue(ValueType_ByteArray, &_tmpArray);
            memcpy((void *)pCurValue, &_tmpValue, sizeof(AssetTypeValue));
            pValueFields[valueFieldIndex].Read(pCurValue, pTemplate, 0, NULL);
            valueFieldIndex++;
            pCurValue = (AssetTypeValue *)((uintptr_t)pCurValue +
                                           sizeof(AssetTypeValue));
          }
        } else {
          ...
          AssetTypeValue _tmpValue =
              AssetTypeValue(ValueType_Array, &_tmpArray);
          memcpy((void *)pCurValue, &_tmpValue, sizeof(AssetTypeValue));

          AssetTypeValueField **arrayItemList = pCurValueFieldList;
          pCurValueFieldList =
              (AssetTypeValueField **)((uintptr_t)pCurValueFieldList +
                                       (sizeof(AssetTypeValueField *) *
                                        arrayLen));
          uint32_t curValueFieldIndex = 0;

          pValueFields[valueFieldIndex].Read(pCurValue, pTemplate, arrayLen,
                                             arrayItemList);
          pCurValue =
              (AssetTypeValue *)((uintptr_t)pCurValue + sizeof(AssetTypeValue));
          valueFieldIndex++;
          for (uint32_t i = 0; i < arrayLen; i++) {
            arrayItemList[curValueFieldIndex] = &pValueFields[valueFieldIndex];
            filePos = _RecursiveMakeValues(
                &pTemplate->children[1], pReader, filePos, maxFilePos,
                pValueFields, valueFieldIndex, pCurValue, pCurValueFieldList,
                pCurRawData, bigEndian);
            curValueFieldIndex++;
            if (filePos > maxFilePos)
              break;
          }
    ...
    char *stringLocation =
        (char *)((uintptr_t)pCurValue + sizeof(AssetTypeValue));
    stringLocation[stringLen] = 0;
    pReader->Read(filePos, stringLen, stringLocation);
    filePos += stringLen;
    ...
    if (pTemplate->align ||
        ((pTemplate->children.size() > 0) && pTemplate->children[0].align))
      filePos = (filePos + 3) & (~3);
    valueFieldIndex++;
    pCurValue = (AssetTypeValue *)((uintptr_t)pCurValue +
                                   sizeof(AssetTypeValue) + stringLen + 1);
    ...
    if (filePos <= maxFilePos) {
      AssetTypeValue _tmpValue =
          AssetTypeValue(pTemplate->valueType, valueContainer);
      memcpy((void *)pCurValue, &_tmpValue, sizeof(AssetTypeValue));
      pValueFields[valueFieldIndex].Read(pCurValue, pTemplate, 0, NULL);
      valueFieldIndex++;
      pCurValue =
    ...
    pCurValueFieldList =
        (AssetTypeValueField **)((uintptr_t)pCurValueFieldList +
                                 (sizeof(AssetTypeValueField *) *
                                  pTemplate->children.size()));
    uint32_t curValueFieldIndex = 0;

    pValueFields[valueFieldIndex].Read(NULL, pTemplate,
                                       (uint32_t)pTemplate->children.size(),
                                       templateChildList);
    valueFieldIndex++;

    for (uint32_t i = 0; i < (uint32_t)pTemplate->children.size(); i++) {
      templateChildList[curValueFieldIndex] = &pValueFields[valueFieldIndex];
      filePos = _RecursiveMakeValues(&pTemplate->children[i], pReader, filePos,
                                     maxFilePos, pValueFields, valueFieldIndex,
                                     pCurValue, pCurValueFieldList, pCurRawData,
                                     bigEndian);
      curValueFieldIndex++;
    }
    if (pTemplate->align)
...
ASSETSTOOLS_API AssetTypeTemplateField::AssetTypeTemplateField()
    : valueType(ValueType_None), isArray(false), align(false), hasValue(false) {
}
...
ASSETSTOOLS_API QWORD AssetTypeTemplateField::MakeValue(
    IAssetsReader *pReader, QWORD filePos, QWORD fileLen,
    AssetTypeValueField **ppValueField, bool endianness) {
  AssetTypeValue *newValue = NULL;
  QWORD tmpFilePos = filePos;
  size_t newValueByteLen = 0;
  size_t childListByteLen = 0;
  size_t rawDataByteLen = 0;
  // Set to true if it goes EOF while reading an array; This allows parsing
  // empty files and having them filled with zeros without risking crashes on
  // invalid files.
  bool readFailed = false;
  int newChildrenCount = _RecursiveGetValueFieldCount(
      this, pReader, filePos + fileLen, &tmpFilePos, &newValueByteLen,
      &childListByteLen, &rawDataByteLen, &readFailed, endianness);
  // ppValueField will be set to pValueFieldMemory so the caller knows which
  // pointer to free
  if (readFailed) {
    *ppValueField = NULL;
    return filePos;
  }
  void *pValueFieldMemory =
      malloc((newChildrenCount * sizeof(AssetTypeValueField)) +
             newValueByteLen + childListByteLen + rawDataByteLen);
  if (pValueFieldMemory == NULL) {
    *ppValueField = NULL;
    return filePos;
  }
  AssetTypeValueField *pValueFields = (AssetTypeValueField *)pValueFieldMemory;
  AssetTypeValue *pCurValue = (AssetTypeValue *)(&(
      (uint8_t *)
          pValueFieldMemory)[newChildrenCount * sizeof(AssetTypeValueField)]);

  AssetTypeValueField **pCurValueList = (AssetTypeValueField **)(&(
      (uint8_t *)
          pValueFieldMemory)[newChildrenCount * sizeof(AssetTypeValueField) +
                             newValueByteLen]);
  uint8_t *pCurRawByte = (uint8_t *)(&(
      (uint8_t *)
          pValueFieldMemory)[newChildrenCount * sizeof(AssetTypeValueField) +
                             newValueByteLen + childListByteLen]);

  uint32_t valueFieldIndex = 0;
  filePos = _RecursiveMakeValues(this, pReader, filePos, filePos + fileLen,
                                 pValueFields, valueFieldIndex, pCurValue,
                                 pCurValueList, pCurRawByte, endianness);
  //_RecursiveDumpValues(pValueFields, 0);
  *ppValueField = &pValueFields[0];
  return filePos;
}

ASSETSTOOLS_API bool AssetTypeTemplateField::From0D(Type_0D *pU5Type,
                                                    uint32_t fieldIndex) {
  if (pU5Type->typeFieldsExCount <= fieldIndex) {
    memset((void *)this, 0, sizeof(AssetTypeTemplateField));
    return false;
  }
  TypeField_0D *pTypeField = &pU5Type->pTypeFieldsEx[fieldIndex];
  type =
      pTypeField->GetTypeString(pU5Type->pStringTable, pU5Type->stringTableLen);
  name =
      pTypeField->GetNameString(pU5Type->pStringTable, pU5Type->stringTableLen);
  if (!type.empty())
...
ASSETSTOOLS_API bool AssetTypeTemplateField::FromClassDatabase(
    ClassDatabaseFile *pFile, ClassDatabaseType *pType, uint32_t fieldIndex) {
  ...
  hasValue =
      (pType->fields.size() > (fieldIndex + 1)) ? (newChildCount == 0) : true;
  children.resize(newChildCount);
  size_t childIndex = 0;
  bool ret = true;
  for (uint32_t i = fieldIndex + 1; i < pType->fields.size(); i++) {
...
AssetTypeTemplateField::SearchChild(const char *name) {
...
ASSETSTOOLS_API AssetTypeInstance::AssetTypeInstance(
    uint32_t baseFieldCount, AssetTypeTemplateField **ppBaseFields,
    QWORD fileLen, IAssetsReader *pReader, bool bigEndian, QWORD filePos) {
  ...
  for (uint32_t i = 0; i < baseFieldCount; i++) {
    filePos = ppBaseFields[i]->MakeValue(pReader, filePos,
                                         fileLen - (filePos - nullPos),
                                         &this->baseFields[i], bigEndian);
...
AssetTypeInstance::SetChildList(AssetTypeValueField *pValueField,
                                AssetTypeValueField **pChildrenList,
                                uint32_t childrenCount, bool freeMemory) {
  ...
  for (size_t i = 0; i < this->memoryToClear.size(); i++) {
    if (this->memoryToClear[i] == pValueField->GetChildrenList()) {
      for (uint32_t _i = 0; _i < pValueField->GetChildrenCount(); _i++) {
        bool found = false;
        AssetTypeValueField *pTarget = pValueField->Get(_i);
...
ASSETSTOOLS_API AssetTypeInstance::~AssetTypeInstance() { Clear(); }