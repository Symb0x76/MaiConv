...
ASSETSTOOLS_API QWORD AssetFileInfo::Read(uint32_t version, QWORD pos,
                                          IAssetsReader *pReader,
                                          bool bigEndian) {
  uint8_t fileInfoData[AssetFileInfo_MaxSize];
  if (version >= 0x0E)
    pos = (pos + 3) & ~3;
  pReader->Read(pos, GetSize(version), fileInfoData);
  pos += GetSize(version);
  this->index = 0;
  int fileInfoDataOffs = 0;
  memcpy(&this->index, &fileInfoData[fileInfoDataOffs],
         (version >= 0x0E) ? 8 : 4);
  fileInfoDataOffs += ((version >= 0x0E) ? 8 : 4);
  if (bigEndian) {
    if (version >= 0x0E)
  ...
  this->offs_curFile = 0;
  memcpy(&this->offs_curFile, &fileInfoData[fileInfoDataOffs],
         (version >= 0x16) ? 8 : 4);
  fileInfoDataOffs += (version >= 0x16) ? 8 : 4; // always 4-byte aligned
  if (bigEndian) {
    if (version >= 0x16)
  ...
  memcpy(&this->curFileSize, &fileInfoData[fileInfoDataOffs], 4);
  fileInfoDataOffs += 4;
  if (bigEndian)
    SwapEndians_(this->curFileSize);
  memcpy(&this->curFileTypeOrIndex, &fileInfoData[fileInfoDataOffs], 4);
  fileInfoDataOffs += 4;
  if (bigEndian)
    SwapEndians_(this->curFileTypeOrIndex);
  if (version < 0x10) {
    memcpy(&this->inheritedUnityClass, &fileInfoData[fileInfoDataOffs], 2);
    fileInfoDataOffs += 2;
    if (bigEndian)
  ...
  if ((version >= 0x0B) && (version <= 0x10)) {
    memcpy(&this->scriptIndex, &fileInfoData[fileInfoDataOffs], 2);
    fileInfoDataOffs += 2;
    if (bigEndian)
  ...
  if ((version >= 0x0F) && (version <= 0x10)) {
    memcpy(&this->unknown1, &fileInfoData[fileInfoDataOffs], 1);
    fileInfoDataOffs += 1;
  } else
    this->unknown1 = 0;
  return pos;
}
ASSETSTOOLS_API QWORD AssetFileInfo::Write(uint32_t version, QWORD pos,
  ...
  memcpy(&fileInfoData[fileInfoDataOffs], &tempQW, (version >= 0x0E) ? 8 : 4);
  fileInfoDataOffs += ((version >= 0x0E) ? 8 : 4);
  ...
  memcpy(&fileInfoData[fileInfoDataOffs], &tempQW, (version >= 0x16) ? 8 : 4);
  fileInfoDataOffs += ((version >= 0x16) ? 8 : 4);

  tempDW = this->curFileSize;
  if (bigEndian)
    SwapEndians_(tempDW);
  memcpy(&fileInfoData[fileInfoDataOffs], &tempDW, 4);
  fileInfoDataOffs += 4;
  ...
  if (version < 0x11) {
    if (version >= 0x0B) {
      ...
      uint16_t wordTmp = 0;
      memcpy(&fileInfoData[fileInfoDataOffs], &wordTmp, 2);
    }
    fileInfoDataOffs += 2;
    if (version >= 0x0F) {
      memcpy(&fileInfoData[fileInfoDataOffs], &this->unknown1, 1);
      fileInfoDataOffs += 1;
    }
  }
  pWriter->Write(pos, fileInfoDataOffs, fileInfoData);
  pos += fileInfoDataOffs;
  return pos;
}
...
ASSETSTOOLS_API QWORD AssetFileList::Read(uint32_t version, QWORD pos,
                                          IAssetsReader *pReader,
                                          bool bigEndian) {
  for (unsigned int i = 0; i < this->sizeFiles; i++) {
...
ASSETSTOOLS_API QWORD AssetFileList::Write(uint32_t version, QWORD pos,
                                           IAssetsWriter *pWriter,
                                           bool bigEndian) {
  for (unsigned int i = 0; i < this->sizeFiles; i++) {
...
ASSETSTOOLS_API QWORD AssetsFileHeader::Read(QWORD absFilePos,
                                             IAssetsReader *pReader) {
  QWORD curFilePos = absFilePos;
  uint32_t dw00, dw04, dw0C;
  pReader->Read(curFilePos, 4, &dw00);
  SwapEndians_(dw00);
  curFilePos += 4;
  pReader->Read(curFilePos, 4, &dw04);
  SwapEndians_(dw04);
  curFilePos += 4;
  pReader->Read(curFilePos, 4, &this->format);
  SwapEndians_(this->format);
  curFilePos += 4;
  pReader->Read(curFilePos, 4, &dw0C);
  SwapEndians_(dw0C);
  curFilePos += 4;
  if (this->format >= 0x16) {
    this->unknown00 = (static_cast<QWORD>(dw00) << 32) | dw04;
    // dw0C is padding for format >= 0x16
    pReader->Read(curFilePos, 8, &this->metadataSize);
    SwapEndians_(this->metadataSize);
    curFilePos += 8;
    pReader->Read(curFilePos, 8, &this->fileSize);
    SwapEndians_(this->fileSize);
    curFilePos += 8;
    pReader->Read(curFilePos, 8, &this->offs_firstFile);
    SwapEndians_(this->offs_firstFile);
    curFilePos += 8;
    pReader->Read(curFilePos, 1, &this->endianness);
    curFilePos += 1;
    pReader->Read(curFilePos, 3, this->unknown);
    curFilePos += 3;
    ...
    if (this->format < 9 && (this->fileSize > this->metadataSize)) {
      ...
      pReader->Read(absFilePos + this->fileSize - this->metadataSize, 1,
                    &this->endianness);
    } else {
      pReader->Read(curFilePos, 1, &this->endianness);
      curFilePos += 1;
...
ASSETSTOOLS_API QWORD AssetsFileHeader::Write(QWORD pos,
                                              IAssetsWriter *pWriter) {
  QWORD qwTmp;
  uint32_t dwTmp;
  if (format >= 0x16) {
    qwTmp = SwapEndians(this->unknown00);
    pWriter->Write(pos, 8, &qwTmp);
    pos += 8;

    dwTmp = SwapEndians(this->format);
    pWriter->Write(pos, 4, &dwTmp);
    pos += 4;
    dwTmp = 0;
    pWriter->Write(pos, 4, &dwTmp);
    pos += 4; // Padding
    ...
    pWriter->Write(pos, 1, &this->endianness);
    pos += 1;
    pWriter->Write(pos, 3, this->unknown);
    pos += 3;
    dwTmp = 0;
    pWriter->Write(pos, 4, &dwTmp);
    pos += 4; // Padding
    ...
    /*if (this->format < 9 && (this->fileSize < this->metadataSize))
    {
            writer(pos - 16 + this->fileSize - this->metadataSize, 1,
    &this->endianness, writerPar); pos += 1;
    }
    else*/
    if (format >= 9) {
      pWriter->Write(pos, 1, &this->endianness);
...
ASSETSTOOLS_API unsigned int AssetsFileHeader::GetSizeBytes() {
  return (format >= 0x16)
             ? 0x30
             : 0x14; // + ((unsigned int)strlen(this->unityVersion) + 1);
}

const uint8_t GlobalTypeTreeStringTable[] = {
    };

ASSETSTOOLS_API QWORD TypeField_0D::Read(QWORD curFilePos,
                                         IAssetsReader *pReader,
                                         uint32_t format, bool bigEndian) {
  uint8_t _isArrayTemp;

  pReader->Read(curFilePos, 2, &this->version);
  curFilePos += 2;
  if (bigEndian)
    SwapEndians_(this->version);
  pReader->Read(curFilePos, 1, &this->depth);
  curFilePos++;
  pReader->Read(curFilePos, 1, &_isArrayTemp);
  curFilePos++;
  this->isArray = (format >= 0x13) ? _isArrayTemp : (_isArrayTemp != 0 ? 1 : 0);
  pReader->Read(curFilePos, 4, &this->typeStringOffset);
  curFilePos += 4;
  if (bigEndian)
    SwapEndians_(this->typeStringOffset);
  pReader->Read(curFilePos, 4, &this->nameStringOffset);
  curFilePos += 4;
  if (bigEndian)
    SwapEndians_(this->nameStringOffset);
  pReader->Read(curFilePos, 4, &this->size);
  curFilePos += 4;
  if (bigEndian)
    SwapEndians_(this->size);
  pReader->Read(curFilePos, 4, &this->index);
  curFilePos += 4;
  if (bigEndian)
    SwapEndians_(this->index);
  pReader->Read(curFilePos, 4, &this->flags);
  curFilePos += 4;
  if (bigEndian)
    SwapEndians_(this->flags);

  if (format >= 0x12) {
    pReader->Read(curFilePos, 8, &this->unknown1);
    curFilePos += 8;
  }

  return curFilePos;
}
ASSETSTOOLS_API QWORD TypeField_0D::Write(QWORD curFilePos,
                                          IAssetsWriter *pWriter,
                                          uint32_t format, bool bigEndian) {
  uint16_t wTmp;
  uint32_t dwTmp;
  ...
  if (bigEndian)
    SwapEndians_(wTmp);
  pWriter->Write(curFilePos, 2, &wTmp);
  curFilePos += 2;

  pWriter->Write(curFilePos, 1, &this->depth);
  curFilePos++;
  pWriter->Write(curFilePos, 1, &this->isArray);
  curFilePos++;

  dwTmp = this->typeStringOffset;
  if (bigEndian)
    SwapEndians_(dwTmp);
  pWriter->Write(curFilePos, 4, &dwTmp);
  curFilePos += 4;
  ...
  if (format >= 0x12) {
    pWriter->Write(curFilePos, 8, &this->unknown1);
    curFilePos += 8;
  }

  return curFilePos;
}
ASSETSTOOLS_API const char *TypeField_0D::GetTypeString(const char *stringTable,
                                                        size_t stringTableLen) {
  const char *type = NULL;
  if (typeStringOffset & 0x80000000) {
    if ((typeStringOffset & 0x7FFFFFFF) <
        (sizeof(GlobalTypeTreeStringTable) - 1))
      type = (const char *)&GlobalTypeTreeStringTable[typeStringOffset &
                                                      0x7FFFFFFF];
...
ASSETSTOOLS_API const char *TypeField_0D::GetNameString(const char *stringTable,
                                                        size_t stringTableLen) {
  const char *name = NULL;
  if (nameStringOffset & 0x80000000) {
    if ((nameStringOffset & 0x7FFFFFFF) <
        (sizeof(GlobalTypeTreeStringTable) - 1))
      name = (const char *)&GlobalTypeTreeStringTable[nameStringOffset &
                                                      0x7FFFFFFF];
...
ASSETSTOOLS_API QWORD Type_0D::Write(bool hasTypeTree, QWORD absFilePos,
                                     IAssetsWriter *pWriter, uint32_t version,
                                     bool bigEndian, bool secondaryTypeTree) {
  uint32_t dwTmp;
  uint16_t wTmp;
  pWriter->Write(absFilePos, 4, &classId);
  absFilePos += 4;
  if (version >= 16) {
    pWriter->Write(absFilePos, 1, &this->unknown16_1);
    absFilePos += 1;
    if (version >= 17) {
      wTmp = this->scriptIndex;
      if (bigEndian)
        SwapEndians_(wTmp);
      pWriter->Write(absFilePos, 2, &wTmp);
      absFilePos += 2;
    }
  }
  if ((classId < 0) || (this->classId == 0x72) ||
      (this->classId == 0x7C90B5B3) || ((short)this->scriptIndex) >= 0) {
    pWriter->Write(absFilePos, 16, this->scriptIDHash.qValue);
    absFilePos += 16;
  }
  pWriter->Write(absFilePos, 16, this->typeHash.qValue);
  absFilePos += 16;
  ...
  if (hasTypeTree) {
    ...
    if (bigEndian)
      SwapEndians_(dwTmp);
    pWriter->Write(absFilePos, 4, &dwTmp);
    absFilePos += 4;
    ...
    for (uint32_t i = 0; i < this->typeFieldsExCount; i++) {
      absFilePos =
          this->pTypeFieldsEx[i].Write(absFilePos, pWriter, version, bigEndian);
    }
    pWriter->Write(absFilePos, this->stringTableLen, this->pStringTable);
    absFilePos += this->stringTableLen;
    ...
    if (version >= 0x15) {
      if (!secondaryTypeTree) {
        ...
        if (bigEndian) {
          ...
          pWriter->Write(absFilePos, this->depListLen * 4, this->pDepList);
          absFilePos += this->depListLen * 4;
        }
      } else {
        ...
        pWriter->Write(absFilePos, header1Len, this->header1);
        absFilePos += header1Len;
        pWriter->Write(absFilePos, header2Len, this->header2);
        absFilePos += header2Len;
        pWriter->Write(absFilePos, header3Len, this->header3);
        absFilePos += header3Len;
      }
    }
  }
  return absFilePos;
}
ASSETSTOOLS_API QWORD Type_0D::Read(bool hasTypeTree, QWORD absFilePos,
                                    IAssetsReader *pReader, uint32_t version,
                                    bool bigEndian, bool secondaryTypeTree) {
  QWORD curFilePos = absFilePos;
  if (version >= 0x0D) // Unity 5
  {
    pReader->Read(curFilePos, 4, &this->classId);
    curFilePos += 4;
    if (bigEndian)
      SwapEndians_(this->classId);
    if (version >= 16) {
      pReader->Read(curFilePos, 1, &this->unknown16_1);
      curFilePos += 1;
    } else {
      this->unknown16_1 = 0;
    }
    if (version >= 17) {
      pReader->Read(curFilePos, 2, &this->scriptIndex);
      curFilePos += 2;
      if (bigEndian)
        SwapEndians_(this->scriptIndex);
    } else {
    ...
    if ((classId < 0) || (this->classId == 0x72) ||
        (this->classId == 0x7C90B5B3) ||
        ((short)this->scriptIndex) >= 0) // MonoBehaviour
    {
      pReader->Read(curFilePos, 16, this->scriptIDHash.qValue);
      curFilePos += 16;
    }
    pReader->Read(curFilePos, 16, this->typeHash.qValue);
    curFilePos += 16;
    ...
    if (hasTypeTree) {
      ...
      pReader->Read(curFilePos, 4, &dwVariableCount);
      curFilePos += 4;
      if (bigEndian)
        SwapEndians_(dwVariableCount);
      pReader->Read(curFilePos, 4, &dwStringTableLen);
      curFilePos += 4;
      if (bigEndian)
        SwapEndians_(dwStringTableLen);
      uint32_t variableFieldsLen =
          (dwVariableCount * (version >= 0x12 ? 32 : 24));
      ...
      if (pTreeBuffer == NULL) {
        ...
        if (pNewReader != NULL) {
          ...
          for (uint32_t i = 0; i < dwVariableCount; i++) {
            newFilePos =
                pTypeFields[i].Read(newFilePos, pNewReader, version, bigEndian);
          }
          ...
          bool appendNullTerminator =
              (typeTreeLen == 0) ||
              (((uint8_t *)pTreeBuffer)[typeTreeLen - 1]) != 0;
          void *pStringTable =
              malloc(dwStringTableLen + (appendNullTerminator ? 1 : 0));
          if (pStringTable != NULL) {
            memcpy(pStringTable, &((uint8_t *)pTreeBuffer)[variableFieldsLen],
                   dwStringTableLen);
      ...
      if (version >= 0x15) {
        if (!secondaryTypeTree) {
          pReader->Read(curFilePos, 4, &this->depListLen);
          curFilePos += 4;
          if (bigEndian)
            SwapEndians_(this->depListLen);
          if (static_cast<int>(this->depListLen) >= 0) {
            this->pDepList = new unsigned int[this->depListLen];
            pReader->Read(curFilePos, this->depListLen * 4, this->pDepList);
            curFilePos += this->depListLen * 4;
            if (bigEndian) {
              for (uint32_t i = 0; i < this->depListLen; i++)
...
ASSETSTOOLS_API QWORD TypeField_07::Read(bool hasTypeTree, QWORD absFilePos,
                                         IAssetsReader *pReader,
  ...
  pReader->Read(curFilePos, 4, &this->arrayFlag);
  curFilePos += 4;
  if (bigEndian)
    SwapEndians_(this->arrayFlag);
  pReader->Read(curFilePos, 4, &this->flags1);
  curFilePos += 4;
  if (bigEndian)
    SwapEndians_(this->flags1);
  if (version == 3) {
    this->flags2 = (uint32_t)-1;
  } else {
    pReader->Read(curFilePos, 4, &this->flags2);
    curFilePos += 4;
    if (bigEndian)
  ...
  if (hasTypeTree) {
    pReader->Read(curFilePos, 4, &this->childrenCount);
    curFilePos += 4;
    if (bigEndian)
      SwapEndians_(this->childrenCount);
    this->children = new TypeField_07[childrenCount]();
    for (uint32_t i = 0; i < childrenCount; i++)
      curFilePos = this->children[i].Read(hasTypeTree, curFilePos, pReader,
                                          version, bigEndian);
  }
  return curFilePos;
}
ASSETSTOOLS_API QWORD TypeField_07::Write(bool hasTypeTree, QWORD absFilePos,
                                          IAssetsWriter *pWriter,
                                          bool bigEndian) {
  uint32_t dwTmp;
  QWORD curFilePos = absFilePos;
  pWriter->Write(curFilePos, strlen(this->type) + 1, this->type);
  curFilePos += (strlen(this->type) + 1);

  pWriter->Write(curFilePos, strlen(this->name) + 1, this->name);
  curFilePos += (strlen(this->name) + 1);
  ...
  if (hasTypeTree) {
    ...
    for (uint32_t i = 0; i < childrenCount; i++)
      curFilePos =
          this->children[i].Write(hasTypeTree, curFilePos, pWriter, bigEndian);
  }
  return curFilePos;
}
ASSETSTOOLS_API QWORD Type_07::Write(bool hasTypeTree, QWORD absFilePos,
                                     IAssetsWriter *pWriter, bool bigEndian) {
  QWORD curFilePos = absFilePos;
...
ASSETSTOOLS_API QWORD Type_07::Read(bool hasTypeTree, QWORD absFilePos,
                                    IAssetsReader *pReader, uint32_t version,
                                    bool bigEndian) {
  QWORD curFilePos = absFilePos;
  ...
  pReader->Read(curFilePos, 4, &classId);
  curFilePos += 4;
  if (bigEndian)
...
TypeTree::Write(QWORD absFilePos, IAssetsWriter *pWriter, uint32_t version,
                bool bigEndian) // Minimum AssetsFile format : 7
{
  uint32_t dwTmp;
  QWORD curFilePos = absFilePos;
  pWriter->Write(curFilePos, strlen(this->unityVersion) + 1,
                 this->unityVersion);
  ...
  if (version >= 0x0D) // Unity 5
  {
    pWriter->Write(curFilePos, 1, &hasTypeTree);
    curFilePos++;
  }
    ...
    if (version < 0x0D) {
      if (this->pTypes_Unity4 != NULL) {
        for (uint32_t i = 0; i < fieldCount; i++)
          curFilePos = this->pTypes_Unity4[i].Write(hasTypeTree, curFilePos,
                                                    pWriter, bigEndian);
      }
    } else {
      if (this->pTypes_Unity5 != NULL) {
        for (uint32_t i = 0; i < fieldCount; i++)
          curFilePos = this->pTypes_Unity5[i].Write(
              hasTypeTree, curFilePos, pWriter, version, bigEndian);
      }
    }
  }
  if (version < 0x0E) {
...
ASSETSTOOLS_API QWORD
TypeTree::Read(QWORD absFilePos, IAssetsReader *pReader, uint32_t version,
               bool bigEndian) // Minimum AssetsFile format : 7
{
  _fmt = version;
  hasTypeTree = true;
  ...
  if (version > 6) {
    ...
    pReader->Read(curFilePos, 4, &this->platform);
    curFilePos += 4;
    if (bigEndian)
  ...
  if (version >= 0x0D) // Unity 5
  {
    pReader->Read(curFilePos, 1, &hasTypeTree);
    curFilePos++;
  }
  pReader->Read(curFilePos, 4, &this->fieldCount);
  curFilePos += 4;
  if (bigEndian)
  ...
  if (this->fieldCount > 0) {
    if (version < 0x0D) {
      ...
      for (uint32_t i = 0; i < fieldCount; i++) {
        curFilePos = this->pTypes_Unity4[i].Read(hasTypeTree, curFilePos,
                                                 pReader, version, bigEndian);
      }
    } else {
      this->pTypes_Unity5 = new Type_0D[fieldCount]();
      for (uint32_t i = 0; i < fieldCount; i++)
        curFilePos = this->pTypes_Unity5[i].Read(hasTypeTree, curFilePos,
                                                 pReader, version, bigEndian);
    }
  } else {
  ...
  if (version < 0x0E) {
    // actually belongs to the asset file info tree
    pReader->Read(curFilePos, 4, &this->dwUnknown);
    curFilePos += 4;
    if (bigEndian)
...
ASSETSTOOLS_API QWORD AssetsFileDependency::Read(QWORD absFilePos,
                                                 IAssetsReader *pReader,
                                                 uint32_t format,
                                                 bool bigEndian) {
  if (format >= 6) {
  ...
  if (format >= 5) {
    ...
    pReader->Read(absFilePos, 4, &this->type);
    absFilePos += 4;
    if (bigEndian)
...
ASSETSTOOLS_API QWORD AssetsFileDependency::Write(QWORD absFilePos,
                                                  IAssetsWriter *pWriter,
                                                  uint32_t format,
                                                  bool bigEndian) {
  if (format >= 6) {
    pWriter->Write(absFilePos, strlen(this->bufferedPath) + 1,
                   this->bufferedPath);
    absFilePos += strlen(this->bufferedPath) + 1;
  }
  ...
  pWriter->Write(absFilePos, strlen(this->assetPath) + 1, this->assetPath);
  absFilePos += strlen(this->assetPath) + 1;

  return absFilePos;
}

ASSETSTOOLS_API QWORD AssetsFileDependencyList::Write(QWORD absFilePos,
                                                      IAssetsWriter *pWriter,
                                                      uint32_t format,
                                                      bool bigEndian) {
  ...
  for (uint32_t i = 0; i < this->dependencyCount; i++)
    absFilePos =
        this->pDependencies[i].Write(absFilePos, pWriter, format, bigEndian);

  return absFilePos;
}
ASSETSTOOLS_API QWORD AssetsFileDependencyList::Read(QWORD absFilePos,
                                                     IAssetsReader *pReader,
                                                     uint32_t format,
                                                     bool bigEndian) {
  // if (format >= 0x0E) //actually another field
  //	absFilePos += 4;
  pReader->Read(absFilePos, 4, &this->dependencyCount);
  absFilePos += 4;
  if (bigEndian)
    SwapEndians_(this->dependencyCount);
  // reader(absFilePos, 1, &this->unknown, readerPar); absFilePos += 1;
  if (this->dependencyCount > 0) {
    this->pDependencies = (AssetsFileDependency *)malloc(
        this->dependencyCount * sizeof(AssetsFileDependency));
    if (this->pDependencies != NULL) {
      for (uint32_t i = 0; i < this->dependencyCount; i++)
        absFilePos =
            this->pDependencies[i].Read(absFilePos, pReader, format, bigEndian);
    } else {
      for (uint32_t i = 0; i < this->dependencyCount; i++)
        absFilePos =
            AssetsFileDependency().Read(absFilePos, pReader, format, bigEndian);
...
static QWORD SecondaryTypeList_Write(AssetsFile *pAssetsFile, QWORD absFilePos,
                                     IAssetsWriter *pWriter, uint32_t format,
                                     bool bigEndian) {
  if (format >= 0x14) {
    ...
    if (pAssetsFile->pSecondaryTypeList != NULL) {
      for (uint32_t i = 0; i < pAssetsFile->secondaryTypeCount; i++)
        absFilePos = pAssetsFile->pSecondaryTypeList[i].Write(
            pAssetsFile->typeTree.hasTypeTree, absFilePos, pWriter, format,
            bigEndian, true);
    }
  }
  return absFilePos;
}
static QWORD SecondaryTypeList_Read(AssetsFile *pAssetsFile, QWORD absFilePos,
                                    IAssetsReader *pReader, uint32_t format,
                                    bool bigEndian) {
  if (format >= 0x14) {
    pReader->Read(absFilePos, 4, &pAssetsFile->secondaryTypeCount);
    absFilePos += 4;
    if (bigEndian)
      SwapEndians_(pAssetsFile->secondaryTypeCount);
    if (pAssetsFile->secondaryTypeCount > 0) {
      pAssetsFile->pSecondaryTypeList =
          new Type_0D[pAssetsFile->secondaryTypeCount]();
      for (uint32_t i = 0; i < pAssetsFile->secondaryTypeCount; i++)
        absFilePos = pAssetsFile->pSecondaryTypeList[i].Read(
            pAssetsFile->typeTree.hasTypeTree, absFilePos, pReader, format,
            bigEndian, true);
    } else
...
static QWORD UnknownString_Write(AssetsFile *pAssetsFile, QWORD absFilePos,
                                 IAssetsWriter *pWriter, uint32_t format) {
  if (format >= 5) {
    pWriter->Write(absFilePos, strlen(pAssetsFile->unknownString) + 1,
                   pAssetsFile->unknownString);
    absFilePos += strlen(pAssetsFile->unknownString) + 1;
  }
  return absFilePos;
}
static QWORD UnknownString_Read(AssetsFile *pAssetsFile, QWORD absFilePos,
                                IAssetsReader *pReader, uint32_t format) {
  if (format >= 5) {
...
ASSETSTOOLS_API QWORD PreloadList::Write(QWORD absFilePos,
                                         IAssetsWriter *pWriter,
                                         uint32_t format, bool bigEndian) {
  uint32_t dwTmp;
...
ASSETSTOOLS_API QWORD PreloadList::Read(QWORD absFilePos,
                                        IAssetsReader *pReader, uint32_t format,
                                        bool bigEndian) {
  pReader->Read(absFilePos, 4, &len);
  absFilePos += 4;
  if (bigEndian)
  ...
  for (uint32_t i = 0; i < len; i++) {
    pReader->Read(absFilePos, 4, &items[i].fileID);
    absFilePos += 4;
    if (bigEndian)
      SwapEndians_(items[i].fileID);
    if (format >= 0x0E) {
      absFilePos = ((absFilePos + 3) & (~0x3));
      pReader->Read(absFilePos, 8, &items[i].pathID);
      absFilePos += 8;
      if (bigEndian)
        SwapEndians_(items[i].pathID);
    } else {
      uint32_t pathID;
      pReader->Read(absFilePos, 4, &pathID);
      absFilePos += 4;
      if (bigEndian)
        SwapEndians_(pathID);
...
ASSETSTOOLS_API QWORD AssetsFile::Write(IAssetsWriter *pWriter, QWORD filePos,
                                        AssetsReplacer **replacers,
                                        size_t replacerCount, uint32_t fileID,
  ...
  if (this->header.format >= 0x0D) {
    types0D.assign(&this->typeTree.pTypes_Unity5[0],
                   &this->typeTree.pTypes_Unity5[this->typeTree.fieldCount]);
    preloadDependencies.assign(
        &this->preloadTable.items[0],
        &this->preloadTable.items[this->preloadTable.len]);
  }
  ...
  for (size_t i = 0; i < replacerCount; ++i) {
    if (replacers[i] != nullptr &&
        (replacers[i]->GetFileID() == fileID ||
         replacers[i]->GetFileID() == 0 || fileID == (uint32_t)-1))
      pendingReplacers.push_back({i, replacers[i]});
  }
  for (auto pendingReplacersIt = pendingReplacers.begin();
       pendingReplacersIt != pendingReplacers.end();) {
    ...
    if (this->header.format >= 0x0D &&
      ...
      auto *pDepModifier =
          reinterpret_cast<AssetsDependenciesReplacer *>(pendingReplacer);
      const std::vector<AssetsFileDependency> &dependencies =
          pDepModifier->GetDependencies();
  ...
  //  - Process the replacers for the current path ID in the order of
  //  pReplacers.
  //  -> The replacer will be removed from pendingReplacers.
  //  -> If a remover is encountered, leave all further replacers for that path
  //  ID intact.
  //     (Further replacers may add a new asset with that ID, but are not
  //     processed in this stage).
  //  - If the asset is removed by one of the replacers, skip it.
  //  - Otherwise (if the asset is kept unchanged or is modified without having
  //  been removed),
  //    add an entry to assetFiles.
  {
    std::unordered_map<uint64_t,
                       std::vector<decltype(pendingReplacers)::iterator>>
        pendingReplacersLookup(pendingReplacers.size());
    for (auto pendingReplacersIt = pendingReplacers.begin();
         pendingReplacersIt != pendingReplacers.end(); ++pendingReplacersIt) {
      AssetsReplacer *pendingReplacer = pendingReplacersIt->second;
      if ((pendingReplacer->GetType() == AssetsReplacement_Remove ||
           pendingReplacer->GetType() == AssetsReplacement_AddOrModify)) {
        uint64_t pathID =
    ...
    for (unsigned int i = 0; i < fileListLen; i++) {
      ...
      readerFilePos =
          fileInfo.finf.Read(this->header.format, readerFilePos, this->pReader,
                             this->header.endianness == 1);

      if (this->header.format < 0x10) {
        fileInfo.actualFileType = fileInfo.finf.curFileTypeOrIndex;
        if (((int)fileInfo.actualFileType) < 0)
          fileInfo.finf.scriptIndex =
              (uint16_t)(-((int)fileInfo.actualFileType + 1));
      } else {
        if (fileInfo.finf.curFileTypeOrIndex >= types0D.size())
          fileInfo.actualFileType = fileInfo.finf.inheritedUnityClass = 0;
        else {
          fileInfo.actualFileType =
              types0D[fileInfo.finf.curFileTypeOrIndex].classId;
          fileInfo.finf.inheritedUnityClass =
              (uint16_t)types0D[fileInfo.finf.curFileTypeOrIndex].classId;
          if (this->header.format >= 0x11)
            fileInfo.finf.scriptIndex =
                types0D[fileInfo.finf.curFileTypeOrIndex].scriptIndex;
        }
      }
      ...
      if (replacersLookupIt != pendingReplacersLookup.end()) {
        ...
        for (iProcessedReplacer = 0;
             iProcessedReplacer < replacersItVec.size() && !doIgnore;
             ++iProcessedReplacer) {
          AssetsReplacer *pReplacerUnchecked =
              replacersItVec[iProcessedReplacer]->second;
          ...
          if (pReplacerUnchecked->GetType() == AssetsReplacement_Remove ||
            ...
            AssetsEntryReplacer *pCurReplacer =
                reinterpret_cast<AssetsEntryReplacer *>(pReplacerUnchecked);
        ...
        replacersItVec.erase(replacersItVec.begin(),
                             replacersItVec.begin() + iProcessedReplacer);
      }
      if (!doIgnore) {
        ...
        for (uint32_t i = 0; i < preloadDependencies.size(); i++) {
          if (preloadDependencies[i].fileID == 0 &&
              preloadDependencies[i].pathID == fileInfo.finf.index) {
    ...
    if (assetWriteOrder.size() != assetFiles.size())
      return 0; // TODO : Add an error message
  }

  // Process all entry replacers that are not based on assets in the original
  // AssetsFile.
  // TODO: Add a faster assetFiles entry lookup and maintain it inside the loop
  ...
  for (auto pendingReplacersIt = pendingReplacers.begin();
    ...
    if (pReplacer->GetType() != AssetsReplacement_Remove &&
      ...
      continue;
    AssetsEntryReplacer *pCurReplacer =
        reinterpret_cast<AssetsEntryReplacer *>(pReplacer);
    if ((pCurReplacer->GetFileID() == fileID ||
      ...
      if (pCurReplacer->GetType() == AssetsReplacement_AddOrModify) {
        ...
        if (this->header.format < 0x10) {
          ...
          for (size_t i = 0; i < types0D.size(); i++) {
            if ((pCurReplacer->GetClassID() >= 0)
                    ...
                    : ((types0D[i].classId == 114) &&
                       (types0D[i].scriptIndex ==
                        pCurReplacer->GetMonoScriptID()))) {
              pFileInfo->finf.curFileTypeOrIndex = (uint32_t)i;
              pFileInfo->actualFileType = (uint32_t)pCurReplacer->GetClassID();
              pFileInfo->finf.inheritedUnityClass =
                  (uint16_t)types0D[i].classId;
              hasIndex = true;
              break;
            }
          }
          if (!hasIndex) {
            Type_0D newType = {};
            newType.classId = (pCurReplacer->GetClassID() >= 0)
                                  ? pCurReplacer->GetClassID()
                                  : 114; // MonoBehaviour
            ...
            newType.scriptIndex = (pCurReplacer->GetClassID() >= 0)
                                      ? 0xFFFF
                                      : pCurReplacer->GetMonoScriptID();
            ...
            Hash128 hash;
            bool hasHash = false;
            if (pCurReplacer->GetPropertiesHash(hash))
              hasHash = true;
            ...
            if (typeTree.hasTypeTree) {
              ...
              if (pCurType) {
                // Generate TypeField_0Ds and the string table from a class
                // database type
                // TODO: Refactor (move to a new function, e.g. a static one).
                uint32_t newTypeFieldsCount = (uint32_t)pCurType->fields.size();
                TypeField_0D *pNewTypeFields =
                    new TypeField_0D[newTypeFieldsCount];
                ...
                for (uint32_t k = 0; k < newTypeFieldsCount; k++) {
                  ...
                  memset(&pNewTypeFields[k].unknown1, 0,
                         sizeof(pNewTypeFields[k].unknown1));
                  const char *nameString =
                      curField.fieldName.GetString(pCurFile.get());
                  const char *typeString =
                      curField.typeName.GetString(pCurFile.get());
                  size_t nameStringLen = strlen(nameString);
                  size_t typeStringLen = strlen(typeString);
                  // GlobalTypeTreeStringTable has 1042 bytes in Unity 5.5.0f3
                  // (format 0x11)
                  if (typeStringLen < (sizeof(GlobalTypeTreeStringTable) - 1)) {
                    for (uint32_t l = 0;
                         l < (sizeof(GlobalTypeTreeStringTable) -
                              typeStringLen - 1);
                         l++) {
                      if (!memcmp(&GlobalTypeTreeStringTable[l], typeString,
                                  typeStringLen + 1)) {
                  ...
                  if (nameStringLen < (sizeof(GlobalTypeTreeStringTable) - 1)) {
                    for (uint32_t l = 0;
                         l < (sizeof(GlobalTypeTreeStringTable) -
                              nameStringLen - 1);
                         l++) {
                      if (!memcmp(&GlobalTypeTreeStringTable[l], nameString,
                                  nameStringLen + 1)) {
                  ...
                  if (!foundTypeStringOffset && newStringTableLen &&
                      typeStringLen < (newStringTableLen - 1)) {
                    for (uint32_t l = 0;
                         l < (newStringTableLen - typeStringLen); l++) {
                      if (!memcmp(&pNewStringTable[l], typeString,
                                  typeStringLen + 1)) {
                  ...
                  if (!foundNameStringOffset && newStringTableLen &&
                      nameStringLen < (newStringTableLen - 1)) {
                    for (uint32_t l = 0;
                         l < (newStringTableLen - nameStringLen); l++) {
                      if (!memcmp(&pNewStringTable[l], nameString,
                                  nameStringLen + 1)) {
                  ...
                  if (!foundTypeStringOffset) {
                    char *_tmp = (char *)realloc(
                        pNewStringTable, newStringTableLen + typeStringLen + 1);
                    if (!_tmp) {
                    ...
                    memcpy(&pNewStringTable[newStringTableLen], typeString,
                           typeStringLen + 1);
                  ...
                  if (!foundNameStringOffset) {
                    char *_tmp = (char *)realloc(
                        pNewStringTable, newStringTableLen + nameStringLen + 1);
                    if (!_tmp) {
                      delete[] pNewTypeFields;
                      newTypeFieldsCount = 0;
                      if (pNewStringTable)
                    ...
                    pNewStringTable = _tmp;
                    memcpy(&pNewStringTable[newStringTableLen], nameString,
                           nameStringLen + 1);
            ...
            pFileInfo->actualFileType = (uint32_t)pCurReplacer->GetClassID();
            pFileInfo->finf.inheritedUnityClass =
                (uint16_t)pCurReplacer->GetMonoScriptID();
        ...
        if (this->header.format < 0x10) {
          if (((int)pFileInfo->finf.curFileTypeOrIndex) < 0)
            ...
            pFileInfo->finf.inheritedUnityClass =
                (uint16_t)pFileInfo->finf.curFileTypeOrIndex;
        }
      ...
      } else if (pCurReplacer->GetType() == AssetsReplacement_Remove &&
                 alreadyExists) {
  ...
  for (unsigned int i = 0; i < assetWriteOrder.size(); i++) {
    ...
    if (fileInfo.pReplacer == NULL) // is this file untouched?
    {
      ...
      if (initializedModifierIndex < untouchedAssetCount) {
        ...
        fileInfo.isUntouched = true;
        untouchedModifiers[initializedModifierIndex] = AssetModifierFromReader(
            fileID, fileInfo.finf.index, fileInfo.actualFileType,
            fileInfo.finf.scriptIndex, this->pReader, fileInfo.finf.curFileSize,
            this->header.offs_firstFile + fileInfo.finf.offs_curFile, 16384);
    ...
    nextAssetOffset =
        (nextAssetOffset + 7) &
        (~7); // use 8-byte alignment here because Unity wants me to
  }

  QWORD metadataSize = 0;
  QWORD firstFilePos = headerPos;
...
WriteAssetsFile_Step1: {
  ...
  if (this->header.format >= 0x0B) {
    ...
    filePos = tempPreloadTable.Write(filePos, pWriter, this->header.format,
                                     this->header.endianness ? true : false);
  }
  QWORD dependencyTablePos = filePos;
  AssetsFileDependencyList _dependencies;
  _dependencies.pDependencies =
      const_cast<AssetsFileDependency *>(pDependencies);
  _dependencies.dependencyCount = (uint32_t)dependencyCount;
  filePos = _dependencies.Write(filePos, pWriter, this->header.format,
                                this->header.endianness ? true : false);
  filePos = SecondaryTypeList_Write(this, filePos, pWriter, this->header.format,
                                    this->header.endianness ? true : false);
...
WriteAssetsFile_Step2: {
  ...
  if (this->header.format < 9) {
    pWriter->Write(filePos, 1, &this->header.endianness);
    filePos++;
...
ASSETSTOOLS_API AssetsFile::AssetsFile(IAssetsReader *pReader) {
  ...
  filePos = this->typeTree.Read(filePos, pReader, this->header.format,
                                this->header.endianness == 1);
  if (this->typeTree.unityVersion[0] < '0' ||
  ...
  {
    AssetFileList tmpFileList;
    tmpFileList.sizeFiles = 0;
  ...
  if (this->header.format >= 0x0B) {
    filePos = this->preloadTable.Read(filePos, pReader, this->header.format,
                                      this->header.endianness == 1);
  } else {
    this->preloadTable.len = 0;
    this->preloadTable.items = NULL;
  }
  filePos = this->dependencies.Read(filePos, pReader, this->header.format,
                                    this->header.endianness == 1);
  filePos = SecondaryTypeList_Read(this, filePos, pReader, this->header.format,
                                   this->header.endianness == 1);
...
uint32_t SwapEndians(uint32_t old) {
  uint32_t ret = (((old & 0xFF000000) >> 24) + ((old & 0x00FF0000) >> 8) +
                  ((old & 0x0000FF00) << 8) + ((old & 0x000000FF) << 24));
  return ret;
}
void SwapEndians_(uint32_t &old) {
  old = (((old & 0xFF000000) >> 24) + ((old & 0x00FF0000) >> 8) +
         ((old & 0x0000FF00) << 8) + ((old & 0x000000FF) << 24));
}
...
AssetsFile::VerifyAssetsFile(AssetsFileVerifyLogger logger) {
  ...
  const void *errorData = 0;
  void *errorData2 = 0;
  uint8_t tmpLastFile;
  AssetFileInfo *pLastFileInfo;
  if (logger == NULL)
    logger = &_BlankVerifyLogger;
  if (pReader == NULL) {
  ...
  {
          errorData = (void*)0; errorData2 = (void*)(sizeof(AssetsFileHeader) +
  10); goto _readerError;
  }*/
  if (!this->header.format || this->header.format > 0x40) {
  ...
  if (this->header.format > 0x16 || this->header.format < 0x08)
    logger("WARNING: AssetsTools (for .assets versions 8-22) wasn't tested "
           "with this .assets' version, likely parsing or writing the file "
           "won't work properly!");

  fileListOffs = this->AssetTablePos;
  fileListSize = 0;
  if (!pReader->Read(fileListOffs, 4, &fileListSize)) {
    errorData = (void *)fileListOffs;
    errorData2 = (void *)4;
    goto _readerError;
  }
  if (this->header.endianness)
  ...
  pFileList->Read(this->header.format, fileListOffs, pReader,
                  this->header.endianness);
  ...
  if (pFileList->sizeFiles > 0) {
    ...
    if ((this->header.offs_firstFile + pLastFileInfo->offs_curFile +
         pLastFileInfo->curFileSize - 1) < this->header.metadataSize) {
      errorData = "Last asset begins before the header ends";
      goto _fileFormatError;
    }
    // sprintf_s(sprntTmp, "%u;%u;%u.", pLastFileInfo->index,
    // pLastFileInfo->offs_curFile, pLastFileInfo->curFileSize);
    if (!pReader->Read(this->header.offs_firstFile +
                           pLastFileInfo->offs_curFile +
                           pLastFileInfo->curFileSize - 1,
                       1, &tmpLastFile)) {
