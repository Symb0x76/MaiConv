...
public:
  ...
  QWORD GetSize();
  bool Init(AssetBundleFile *pBundleFile, IAssetsReader *pEntryReader,
...
public:
  BundleEntryRenamer(const char *oldName, const char *newName,
                     unsigned int bundleListIndex, bool hasSerializedData);
  BundleReplacementType GetType();
  ~BundleEntryRenamer();
  unsigned int GetBundleListIndex();
  const char *GetOriginalEntryName();
...
public:
  BundleEntryModifier(const char *oldName, const char *newName,
                      unsigned int bundleListIndex, bool hasSerializedData,
                      std::shared_ptr<IAssetsReader> pReader, QWORD size,
...
public:
  ...
  BundleEntryModifierFromMem(const char *oldName, const char *newName,
                             unsigned int bundleListIndex,
                             bool hasSerializedData, void *pMem, size_t size,
                             cbFreeMemoryResource freeMemoryCallback);
  BundleReplacementType GetType();
...
public:
  BundleEntryModifierFromAssets(const char *oldName, const char *newName,
                                unsigned int bundleListIndex,
                                AssetsFile *pAssetsFile,
                                AssetsReplacer **pReplacers,
...
public:
  BundleEntryModifierFromBundle(const char *oldName, const char *newName,
                                unsigned int bundleListIndex,
...
public:
  ...
  inline QWORD getSize() {
    return resources.empty()
               ? 0
               : (resources.back().outRangeBegin + resources.back().rangeSize);
  }

public:
  BundleEntryModifierByResources(const char *oldName, const char *newName,
                                 unsigned int bundleListIndex,
                                 std::vector<ReplacedResourceDesc> resources,
                                 size_t copyBufferLen);
  BundleReplacementType GetType();
