...
public:
  ...
  // AssetsHeader format >= 0x10 : equals
  // TypeTree.pTypes_Unity5[curFileTypeOrIndex].classId or (uint32_t)INT_MIN if
  // the index is out of bounds
  uint32_t curFileType;
  QWORD absolutePos;

  // If the file type id is known to have a name field, it reads up to 100
  // characters (including the null-terminator) to "out".
  ASSETSTOOLS_API bool ReadName(AssetsFile *pFile, std::string &out,
                                IAssetsReader *pReaderView = nullptr);
};

class AssetFileInfoEx_KeyRef {
public:
  AssetFileInfoEx *pInfo;
  inline AssetFileInfoEx_KeyRef() : pInfo(nullptr) {}
  inline AssetFileInfoEx_KeyRef(AssetFileInfoEx *pInfo) : pInfo(pInfo) {}
...
// Provides additional info for assets and an O(log(assetFileInfoCount)) lookup
// by path ID. The names are always read from the asset itself if possible, 	see
//AssetBundleFileTable or ResourceManagerFile for more detailed names (usually
//not set for all assets with m_Name fields).
