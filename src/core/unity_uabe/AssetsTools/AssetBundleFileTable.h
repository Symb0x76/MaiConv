...
public:
  ...
  ASSETSTOOLS_API bool ReadBundleFile(void *data, size_t dataLen,
                                      size_t *filePos,
                                      AssetTypeTemplateField *pBaseField,
                                      bool bigEndian);
  ASSETSTOOLS_API void ReadBundleFile(void *data, size_t dataLen,
                                      size_t *filePos, int assetsVersion,
                                      bool bigEndian);
  ASSETSTOOLS_API void FlushChanges();
  ASSETSTOOLS_API int GetFileSize();
  ASSETSTOOLS_API bool WriteBundleFile(void *buffer, size_t bufferLen,
                                       size_t *size, bool bigEndian);

  inline void SetModified() { isModified = true; }
  inline bool IsModified() const { return isModified; }
