...
namespace maiconv {
  namespace {
    ...
    std::optional<std::string> detect_image_extension_by_magic(const std::filesystem::path& path) {
      ...
      if (read_bytes >= 3 && head[0] == 0xFFU && head[1] == 0xD8U && head[2] == 0xFFU) {
    ...
    std::string constant_to_display_level(int level_int, int level_dec) {
      return level_id_to_display_level(constant_to_level_id(level_int * 10 + level_dec));
    }
    ...
    TrackInfo parse_track_info(const std::filesystem::path& music_xml, const std::string& fallback_id) {
      ...
      if (auto* notes_data = find_first_element_by_name(root, "notesData")) {
        ...
        for (auto* notes = notes_data->FirstChildElement("Notes"); notes != nullptr;
          ...
          int level_int = to_int(element_text(notes->FirstChildElement("level")), 0);
          int level_dec = to_int(element_text(notes->FirstChildElement("levelDecimal")), 0);
          if (level_int == 0 && level_dec == 0) {
            continue;
          }

          std::string constant_level = std::to_string(level_int) + "." + std::to_string(level_dec);
          std::string display_level;
          const int level_id = to_int(element_text(notes->FirstChildElement("musicLevelID")), -1);
          if (level_id >= 0) {
          ...
          if (ma2_diff >= 0) {
            const int output_diff = notes_slot_to_output_difficulty(notes_slot, info.is_utage);
            raw_difficulties.push_back({ ma2_diff, {constant_level, display_level, designer} });
            info.chart_output_difficulties[normalize_chart_path_key(std::filesystem::path(file_name))] = output_diff;
            info.difficulties[output_diff] = { constant_level, display_level, designer };
          }
        }
        ...
        if (info.chart_output_difficulties.empty()) {
          for (const auto& entry : raw_difficulties) {
            const int output_diff = to_output_difficulty_index(entry.ma2_diff, info.zero_based_difficulty);
    ...
    int infer_inote_index(const std::filesystem::path& ma2_file, bool zero_based_difficulty) {
    ...
    int infer_output_difficulty(const TrackInfo& info,
      ...
      const auto it = info.chart_output_difficulties.find(normalize_chart_path_key(ma2_file));
    ...
    std::string normalize_maidata_genre(std::string_view genre) {
      static const std::array<std::pair<std::string_view, std::string_view>, 4> kGenreMappings = { {
          {"POPSアニメ", "POPS＆アニメ"},
    ...
    void push_warning(std::vector<std::string>& warnings,
    ...
    void emit_track_output(const TrackInfo& info,
      ...
      std::cout << (incomplete ? "Incomplete: " : "Completed: ")
        << info.id << " " << export_display_title(info);
      if (log_level == AssetsLogLevel::Verbose) {
    ...
    std::string compose_maidata_document(const TrackInfo& info,
      ...
      out << "&genre=" << normalize_maidata_metadata_value(normalize_maidata_genre(info.genre)) << "\n";
      ...
      for (const auto& [diff, body] : inotes) {
        ...
        if (difficulty_meta != info.difficulties.end()) {
          out << (level_mode == MaidataLevelMode::Display ? difficulty_meta->second.display_level
  ...
  int run_compile_assets(const AssetsOptions& options) {
    try {
      ...
      for (const auto& folder : folders) {
        ...
        for (const auto& ma2_file : all_ma2_files) {
          if (target_difficulty.has_value() &&
            infer_output_difficulty(info, ma2_file, zero_based_difficulty) != *target_difficulty) {
            continue;
          }
        ...
        for (const auto& ma2_file : ma2_files) {
          ...
          if (options.format == ChartFormat::Simai || options.format == ChartFormat::SimaiFes ||
            ...
            const int inote = infer_output_difficulty(info, ma2_file, zero_based_difficulty);
        ...
        if (!inotes.empty()) {
          const std::string payload =
            (options.format == ChartFormat::Maidata)
            ? compose_maidata_document(info, inotes, options.strict_decimal, options.maidata_level_mode)
          ...
          if (!compressed_audio.empty()) {
            ...
            if (ext == ".mp3") {
              ...
              if (!convert_audio_to_mp3(compressed_audio, track_output / "track.mp3")) {
                push_warning(warnings, options.log_level,
                  "Audio conversion failed: " + path_to_utf8(compressed_audio) + " -> " + path_to_utf8(track_output / "track.mp3"));
                incomplete = true;
              }
            }
          }
            ...
            if (!acb_source.empty() && !awb_source.empty()) {
              if (!convert_acb_awb_to_mp3(acb_source, awb_source, track_output / "track.mp3")) {
                push_warning(warnings, options.log_level,
                  "Audio conversion failed: " + path_to_utf8(acb_source) + " + " + path_to_utf8(awb_source) + " -> " + path_to_utf8(track_output / "track.mp3"));
                incomplete = true;
              }
            }
            else {
              push_warning(warnings, options.log_level, "Music missing: " + info.name + " (" + info.id + ")");
              incomplete = true;
            }
          }
        }
        if (!cover_bases.empty()) {
          ...
          if (!cover_image.empty()) {
            ...
            if (!cover_ab.empty()) {
              const auto pseudo_image_ext = detect_image_extension_by_magic(cover_ab);
              if (pseudo_image_ext.has_value()) {
                // Some packs ship ui_jacket_*.ab as raw PNG/JPEG bytes instead of Unity bundles.
                std::filesystem::copy_file(cover_ab, track_output / ("bg" + *pseudo_image_ext),
                  std::filesystem::copy_options::overwrite_existing);
              }
              else {
                if (!convert_ab_to_png(cover_ab, track_output / "bg.png")) {
                  push_warning(warnings, options.log_level,
                    "Cover conversion failed: " + path_to_utf8(cover_ab) + " -> " + path_to_utf8(track_output / "bg.png"));
                  incomplete = true;
                }
              }
            }
            else {
              push_warning(warnings, options.log_level, "Cover missing: " + info.name + " (" + info.id + ")");
              incomplete = true;
            }
          }
        }
        if (!video_bases.empty()) {
          ...
          if (!video_source.empty()) {
            ...
            if (video_source.empty()) {
              push_warning(warnings, options.log_level, "Video missing: " + info.name + " (" + info.id + ")");
              incomplete = true;
            }
            else {
              if (!convert_dat_or_usm_to_mp4(video_source, track_output / "pv.mp4")) {
                push_warning(warnings, options.log_level,
                  "Video conversion failed: " + path_to_utf8(video_source) + " -> " + path_to_utf8(track_output / "pv.mp4"));
                incomplete = true;
              }
            }
          }
        ...
        if (incomplete) {
          ...
          if (options.export_zip) {
            if (!zip_and_remove(track_output)) {
              push_warning(warnings, options.log_level, "Zip export failed: " + path_to_utf8(track_output));
            }
          }
        }

        emit_track_output(info, final_track_output, incomplete, options.log_level);
      }
      ...
      emit_log_output(compiled_tracks, warnings, options.output_path, options.log_tracks_json, options.log_level);
