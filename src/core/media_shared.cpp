...
bool media_video_shared_write_embedded_png(
    const std::filesystem::path &source, const std::filesystem::path &png_file);

std::vector<std::string> media_video_shared_resolve_ffmpeg_mp3_encoders();
void media_video_shared_remove_file_if_exists(
...
bool media_shared_read_acb_stub_sidecar_awb_name(
    const std::filesystem::path &acb, std::string &awb_name_out,
    uint64_t &awb_size_out) {
  return media_video_shared_read_acb_stub_sidecar_awb_name(acb, awb_name_out,
                                                           awb_size_out);
}
