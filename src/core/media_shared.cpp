// Shared media helpers (moved from media_video.cpp).
...
std::vector<std::string> media_shared_resolve_ffmpeg_h264_encoders() {
  return resolve_ffmpeg_h264_encoders();
}

std::vector<std::string> media_shared_resolve_ffmpeg_vp9_encoders() {
  return resolve_ffmpeg_vp9_encoders();
}

bool media_shared_write_binary_file(const std::filesystem::path &path,
                                    const std::vector<uint8_t> &data) {
  return write_binary_file(path, data);
}

bool media_shared_read_exact(std::istream &in, uint8_t *out, std::size_t size) {
  return read_exact(in, out, size);
}

uint16_t media_shared_read_u16_be(const uint8_t *p) { return read_u16_be(p); }

uint16_t media_shared_read_u16_le(const uint8_t *p) { return read_u16_le(p); }

uint32_t media_shared_read_u32_be(const uint8_t *p) { return read_u32_be(p); }
...
bool media_shared_run_ffmpeg_capture_stdout(
  ...
  return run_ffmpeg_capture_stdout(args, stdout_bytes);
}

bool media_shared_run_ffmpeg_feed_stdin(
    const std::vector<std::wstring> &args,
    const std::vector<uint8_t> &stdin_bytes) {
  return run_ffmpeg_feed_stdin(args, stdin_bytes);
}
#else
