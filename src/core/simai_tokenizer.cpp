...
std::string normalize_simai_for_tokenize(std::string_view text) {
  ...
  if (cleaned.size() >= 3 && static_cast<unsigned char>(cleaned[0]) == 0xEF &&
...
simai::Tokenizer::tokenize_text(const std::string &text) const {
...
std::vector<std::string>
simai::Tokenizer::tokenize_file(const std::filesystem::path &path) const {
...
SimaiDocument
simai::Tokenizer::parse_file(const std::filesystem::path &path) const {
