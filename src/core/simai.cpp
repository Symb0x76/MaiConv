...
std::string format_slide_duration(const Chart &chart, int start_tick,
  ...
  if (note.wait_ticks == default_wait) {
    ...
    if (start_bpm > 0.0 && action_bpm > 0.0) {
      const double base_ticks =
          static_cast<double>(note.last_ticks) * start_bpm / action_bpm;
  ...
  const double wait_sec = chart.ticks_to_seconds(start_tick + note.wait_ticks) -
                          chart.ticks_to_seconds(start_tick);
  const double last_sec =
      chart.ticks_to_seconds(start_tick + note.wait_ticks + note.last_ticks) -
      chart.ticks_to_seconds(start_tick + note.wait_ticks);
  return "[" + format_decimal_compact(wait_sec) + "##" +
         format_decimal_compact(last_sec) + "]";
}
...
std::string SimaiComposer::compose_chart(const Chart &source) const {
  ...
  const auto has_matching_slide = [&](const Note &candidate) {
    ...
    // Keep explicit Ex slide-start markers (e.g. `x_`) in output even when
    // a slide begins at the same slot; other states can be inherited.
    if (candidate.state == SpecialState::Ex) {
      return false;
    }
  ...
  const auto find_open_chain =
      [&](const Note &note) -> std::optional<ChainRef> {
    ...
    if (note.state == SpecialState::ConnectingSlide) {
      const auto by_origin =
          open_slide_chains_by_origin.find({stamp, note.key});
      if (by_origin != open_slide_chains_by_origin.end()) {
        return by_origin->second;
      }

      const auto range_begin = open_slide_chains_by_end.lower_bound(
          {stamp, std::numeric_limits<int>::min()});
      const auto range_end = open_slide_chains_by_end.upper_bound(
          {stamp, std::numeric_limits<int>::max()});
  ...
  for (const auto &note : chart.notes()) {
    ...
    max_tick =
        std::max(max_tick, ts + std::max(0, note.wait_ticks + note.last_ticks));
    ...
    const auto matching_slide_start_state =
        is_slide_type(note.type) ? find_matching_slide_start_state(note)
                                 : std::nullopt;
    const bool has_explicit_ex_slide_start =
        matching_slide_start_state.has_value() &&
        matching_slide_start_state.value() == SpecialState::Ex;
    const SpecialState effective_slide_state =
        matching_slide_start_state.has_value() &&
                matching_slide_start_state.value() != SpecialState::Normal &&
                !has_explicit_ex_slide_start
            ? matching_slide_start_state.value()
            : note.state;
    const std::string state = state_suffix(
        note.type == NoteType::SlideStart ? note.state : effective_slide_state);
    ...
    if (note.type == NoteType::Tap || note.type == NoteType::SlideStart) {
      ...
      const int display_start_key =
          chain_ref.has_value() ? chain_ref->current_end_key : note.key;
      ...
      const bool repeat_break_suffix =
          end_on_outer_boundary &&
          effective_slide_state == SpecialState::Break &&
          (note.type == NoteType::SlideStraight ||
           note.type == NoteType::SlideCurveLeft ||
           note.type == NoteType::SlideCurveRight);
      const std::string end_state =
          repeat_break_suffix ? state_suffix(effective_slide_state) : "";
      const std::string segment = notation + std::to_string(note.end_key + 1) +
                                  end_state +
                                  format_slide_duration(chart, ts, note);
      const auto bundle_key = std::make_tuple(
          ts, display_start_key, static_cast<int>(effective_slide_state),
          note.wait_ticks);

      if (!chain_ref.has_value() &&
        ...
        if (bundle_it != simultaneous_slide_bundles.end()) {
          ...
          prior_notes[bundle_it->second.note_index] +=
              "*" + notation + std::to_string(note.end_key + 1) + end_state +
    ...
    if (!token.empty()) {
      ...
      if (is_slide_type(note.type)) {
        simultaneous_slide_bundles[std::make_tuple(
            ts, note.key, static_cast<int>(effective_slide_state),
            note.wait_ticks)] = {ts, notes.size() - 1, note.key, note.end_key};
