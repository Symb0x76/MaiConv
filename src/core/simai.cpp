...
std::string format_double_trim(double value) {
...
std::string SimaiComposer::compose_chart(const Chart& source) const {
  ...
  std::map<int, std::vector<std::string>> tokens_at;
  const int def = chart.definition();

  if (!chart.bpm_changes().empty()) {
    const auto& bpm0 = chart.bpm_changes().front();
    tokens_at[bpm0.tick_stamp(def)].push_back("(" + format_double_trim(bpm0.bpm) + ")");
  }
  tokens_at[0].push_back("{4}");

  for (std::size_t i = 1; i < chart.bpm_changes().size(); ++i) {
    const auto& bpm = chart.bpm_changes()[i];
    tokens_at[bpm.tick_stamp(def)].push_back("(" + format_double_trim(bpm.bpm) + ")");
  }

  for (std::size_t i = 1; i < chart.measure_changes().size(); ++i) {
    const auto& mc = chart.measure_changes()[i];
    tokens_at[mc.tick_stamp(def)].push_back("{" + std::to_string(mc.quaver) + "}");
  }

  int max_tick = 0;
  for (const auto& note : chart.notes()) {
    const int ts = note.tick_stamp(def);
    max_tick = std::max(max_tick, ts + note.wait_ticks + note.last_ticks);
    ...
    if (note.type == NoteType::Tap || note.type == NoteType::SlideStart) {
      ...
      const double wait_sec = chart.ticks_to_seconds(ts + note.wait_ticks) - chart.ticks_to_seconds(ts);
      const double last_sec = chart.ticks_to_seconds(ts + note.wait_ticks + note.last_ticks) -
                              chart.ticks_to_seconds(ts + note.wait_ticks);

      token = std::to_string(note.key + 1) + state + notation + std::to_string(note.end_key + 1) + "[" +
              format_double_trim(wait_sec) + "##" + format_double_trim(last_sec) + "]";
    }

    if (!token.empty()) {
      tokens_at[ts].push_back(token);
    }
  }

  std::vector<int> stamps;
  stamps.reserve(tokens_at.size() + 1);
  stamps.push_back(0);
  for (const auto& [tick, _] : tokens_at) {
    stamps.push_back(tick);
  }
  stamps.push_back(max_tick);
  std::sort(stamps.begin(), stamps.end());
  stamps.erase(std::unique(stamps.begin(), stamps.end()), stamps.end());

  int step = 0;
  for (std::size_t i = 1; i < stamps.size(); ++i) {
    const int diff = stamps[i] - stamps[i - 1];
    if (diff > 0) {
      step = (step == 0) ? diff : gcd_int(step, diff);
    }
  }
  if (step <= 0) {
    step = 96;
  }
  step = std::max(1, std::min(step, 96));

  std::ostringstream out;
  for (int tick = 0; tick <= max_tick; tick += step) {
    const auto it = tokens_at.find(tick);
    if (it != tokens_at.end() && !it->second.empty()) {
