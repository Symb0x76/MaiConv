...
namespace maiconv
{
  namespace
  {
    ...
    uint32_t resolve_class_id(const AssetsFile &assets_file,
                              const AssetFileInfoEx &asset_info)
    {
      ...
      return static_cast<uint32_t>(
          assets_file.typeTree.pTypes_Unity5[asset_info.curFileTypeOrIndex]
              .classId);
    }

    Type_0D *resolve_type_tree(AssetsFile &assets_file,
                               const AssetFileInfoEx &asset_info)
    {
      if (assets_file.header.format < 0x10)
      {
        ...
        {
          if (assets_file.typeTree.pTypes_Unity5[i].classId ==
              static_cast<int>(asset_info.curFileTypeOrIndex))
          {
    ...
    bool copy_byte_array_field(AssetTypeValueField *field,
                               std::vector<uint8_t> &output)
    {
      ...
      if (byte_array == nullptr || byte_array->data == nullptr ||
          byte_array->size == 0)
      {
        output.clear();
        return true;
    ...
    bool read_texture_payload(TexturePayload &out,
                              AssetTypeValueField *base_field)
    {
      ...
      if (width == nullptr || height == nullptr || format == nullptr ||
          image_data == nullptr || stream_data == nullptr)
      {
        return false;
      }
      if (width->IsDummy() || height->IsDummy() || format->IsDummy() ||
          image_data->IsDummy() || stream_data->IsDummy())
      {
        return false;
      }
      ...
      if (stream_offset == nullptr || stream_size == nullptr ||
          stream_path == nullptr)
      {
        return false;
      }
      if (stream_offset->IsDummy() || stream_size->IsDummy() ||
          stream_path->IsDummy())
      {
        return false;
      }
    ...
    bool read_bundle_entry_bytes(AssetBundleFile &bundle,
                                 IAssetsReader *bundle_reader, size_t entry_index,
                                 std::vector<uint8_t> &output)
    {
      ...
      {
        if (bundle.bundleInf6 == nullptr ||
            entry_index >= bundle.bundleInf6->directoryCount)
        {
          return false;
        }
        entry_size = bundle.bundleInf6->dirInf[entry_index].decompressedSize;
        entry_reader = bundle.MakeAssetsFileReader(
            bundle_reader, &bundle.bundleInf6->dirInf[entry_index]);
      }
      else if (bundle.bundleHeader6.fileVersion == 3)
      {
        if (bundle.assetsLists3 == nullptr ||
            entry_index >= bundle.assetsLists3->count)
        {
          return false;
        }
        entry_size = bundle.assetsLists3->ppEntries[entry_index]->length;
        entry_reader = bundle.MakeAssetsFileReader(
            bundle_reader, bundle.assetsLists3->ppEntries[entry_index]);
      }
      if (entry_reader == nullptr || entry_size == 0 ||
          entry_size > static_cast<uint64_t>(SIZE_MAX))
      {
    ...
    bool load_stream_payload(AssetBundleFile &bundle, IAssetsReader *bundle_reader,
                             ...
                             TexturePayload &payload)
    {
          ...
          std::filesystem::is_regular_file(direct_path))
      {
        ...
        in.read(reinterpret_cast<char *>(payload.picture_data.data()),
                static_cast<std::streamsize>(payload.picture_data.size()));
      ...
      const std::string target_name = normalize_entry_name(
          std::filesystem::path(payload.stream_path).filename().generic_string());
      const size_t entry_count =
          bundle.bundleHeader6.fileVersion >= 6
              ? static_cast<size_t>(bundle.bundleInf6 != nullptr
                                        ? bundle.bundleInf6->directoryCount
                                        : 0)
              : static_cast<size_t>(bundle.assetsLists3 != nullptr
                                        ? bundle.assetsLists3->count
                                        : 0);
      for (size_t i = 0; i < entry_count; ++i)
      {
        ...
        const std::string entry_filename = normalize_entry_name(
            std::filesystem::path(entry_name_raw).filename().generic_string());
        ...
        const size_t available =
            entry_bytes.size() - static_cast<size_t>(payload.stream_offset);
        if (available < payload.stream_size)
        {
          continue;
        }

        const auto begin = entry_bytes.begin() +
                           static_cast<std::ptrdiff_t>(payload.stream_offset);
        payload.picture_data.assign(
            begin, begin + static_cast<std::ptrdiff_t>(payload.stream_size));
        return true;
      }
    ...
    {
      const uint8_t red =
          static_cast<uint8_t>(((color >> 11U) & 0x1FU) * 255U / 31U);
      const uint8_t green =
          static_cast<uint8_t>(((color >> 5U) & 0x3FU) * 255U / 63U);
                           ...
                           bool &use_transparent_color)
    {
      const uint16_t color0 =
          static_cast<uint16_t>(block[0] | (static_cast<uint16_t>(block[1]) << 8U));
      const uint16_t color1 =
          static_cast<uint16_t>(block[2] | (static_cast<uint16_t>(block[3]) << 8U));
    ...
    bool decode_dxt1(const TexturePayload &payload, std::vector<uint8_t> &rgba)
    {
      ...
      const size_t expected_size =
          static_cast<size_t>(blocks_x) * static_cast<size_t>(blocks_y) * 8U;
      if (payload.picture_data.size() < expected_size)
      {
        return false;
      }

      rgba.assign(static_cast<size_t>(payload.width) *
                      static_cast<size_t>(payload.height) * 4U,
                  0U);
      size_t offset = 0;
          ...
          for (uint32_t y = 0; y < 4U; ++y)
          {
            for (uint32_t x = 0; x < 4U; ++x)
            {
              ...
              const uint32_t palette_index =
                  (indices >> (2U * (4U * y + x))) & 0x03U;
              uint8_t *dst = rgba.data() +
                             ((static_cast<size_t>(py) * payload.width + px) * 4U);
              std::copy(palette[palette_index].begin(),
                        palette[palette_index].end(), dst);
            }
          }
        }
      }
    ...
    bool decode_dxt5(const TexturePayload &payload, std::vector<uint8_t> &rgba)
    {
      ...
      const uint32_t blocks_y = (payload.height + 3U) / 4U;
      const size_t expected_size =
          static_cast<size_t>(blocks_x) * static_cast<size_t>(blocks_y) * 16U;
      if (payload.picture_data.size() < expected_size)
      {
        return false;
      }
      ...
      for (uint32_t by = 0; by < blocks_y; ++by)
      {
        for (uint32_t bx = 0; bx < blocks_x; ++bx)
        {
          ...
          if (alpha_palette[0] > alpha_palette[1])
          {
            for (size_t i = 1; i < 7; ++i)
            {
              alpha_palette[i + 1] = static_cast<uint8_t>(
                  ((7U - i) * alpha_palette[0] + i * alpha_palette[1]) / 7U);
            }
          }
          else
          {
            ...
            {
              alpha_palette[i + 1] = static_cast<uint8_t>(
                  ((5U - i) * alpha_palette[0] + i * alpha_palette[1]) / 5U);
            }
            alpha_palette[6] = 0U;
          ...
          for (int i = 0; i < 6; ++i)
          {
            alpha_indices |= static_cast<uint64_t>(block[2 + i])
          ...
          for (uint32_t y = 0; y < 4U; ++y)
          {
            for (uint32_t x = 0; x < 4U; ++x)
            {
              ...
              const uint32_t palette_index =
                  (color_indices >> (2U * pixel_index)) & 0x03U;
              const uint32_t alpha_index = static_cast<uint32_t>(
                  (alpha_indices >> (3U * pixel_index)) & 0x07U);
              uint8_t *dst = rgba.data() +
                             ((static_cast<size_t>(py) * payload.width + px) * 4U);
              std::copy(palette[palette_index].begin(),
                        palette[palette_index].end(), dst);
    ...
    bool decode_bc7_with_d3d11(const TexturePayload &payload,
                               std::vector<uint8_t> &rgba)
    {
      const uint32_t block_width = (payload.width + 3U) / 4U;
      const uint32_t block_height = (payload.height + 3U) / 4U;
      const size_t expected_size = static_cast<size_t>(block_width) *
                                   static_cast<size_t>(block_height) * 16U;
      if (payload.picture_data.size() < expected_size || payload.width == 0 ||
          payload.height == 0)
      {
        return false;
      }
      ...
      HRESULT hr = D3D11CreateDevice(
          ...
          static_cast<UINT>(std::size(feature_levels)), D3D11_SDK_VERSION, &device,
      ...
      const char *vertex_shader_source =
          ...
          "  float2 pos[3] = { float2(-1.0, -1.0), float2(-1.0, 3.0), float2(3.0, "
          "-1.0) };"
          "  float2 uv[3] = { float2(0.0, 1.0), float2(0.0, -1.0), float2(2.0, "
          "1.0) };"
          "  VSOut outv; outv.pos = float4(pos[vertexId], 0.0, 1.0); outv.uv = "
          "uv[vertexId]; return outv; }";
      const char *pixel_shader_source = "Texture2D tex0 : register(t0);"
      ...
      hr = D3DCompile(vertex_shader_source, std::strlen(vertex_shader_source),
                      nullptr, nullptr, nullptr, "main", "vs_4_0", 0, 0, &vs_blob,
                      &error_blob);
      ...
      hr = D3DCompile(pixel_shader_source, std::strlen(pixel_shader_source),
                      nullptr, nullptr, nullptr, "main", "ps_4_0", 0, 0, &ps_blob,
                      &error_blob);
      safe_release(error_blob);
      ...
      hr = device->CreateShaderResourceView(source_texture, nullptr,
      ...
      if (SUCCEEDED(hr))
      {
        hr = device->CreateRenderTargetView(render_texture, nullptr,
                                            &render_target_view);
      }
      ...
      if (FAILED(hr) || shader_resource_view == nullptr ||
          render_texture == nullptr || render_target_view == nullptr ||
      ...
      rgba.resize(static_cast<size_t>(payload.width) *
                  static_cast<size_t>(payload.height) * 4U);
      for (uint32_t y = 0; y < payload.height; ++y)
      {
        const uint8_t *src = static_cast<const uint8_t *>(mapped.pData) +
                             static_cast<size_t>(y) * mapped.RowPitch;
        uint8_t *dst = rgba.data() + static_cast<size_t>(y) *
                                         static_cast<size_t>(payload.width) * 4U;
    ...
    bool decode_texture_to_rgba(const TexturePayload &payload,
                                std::vector<uint8_t> &rgba)
    {
      const size_t pixel_count =
          static_cast<size_t>(payload.width) * static_cast<size_t>(payload.height);
    ...
    void flip_rgba_rows(std::vector<uint8_t> &rgba, uint32_t width,
                        uint32_t height)
    {
      if (height <= 1 || width == 0)
      {
  ...
  {
    IAssetsReader *bundle_reader = Create_AssetsReaderFromFile(
        ab_file.c_str(), true, RWOpenFlags_Immediately);
    ...
    if (bundle.IsCompressed())
    {
      ...
      IAssetsWriter *unpacked_writer = Create_AssetsWriterToFile(
          unpacked_bundle_path.c_str(), true, true, RWOpenFlags_Immediately);
      ...
      bundle_reader = Create_AssetsReaderFromFile(unpacked_bundle_path.c_str(),
                                                  true, RWOpenFlags_Immediately);
      if (bundle_reader == nullptr)
      {
        std::error_code ec;
        std::filesystem::remove(unpacked_bundle_path, ec);
    ...
    for (size_t entry_index = 0; entry_index < entry_count && !ok;
         ++entry_index)
    {
      ...
      if (bundle.bundleHeader6.fileVersion >= 6)
      {
        assets_reader = bundle.MakeAssetsFileReader(
            bundle_reader, &bundle.bundleInf6->dirInf[entry_index]);
      }
      else
      {
      ...
      for (unsigned int i = 0; i < asset_table.assetFileInfoCount && !ok; ++i)
      {
        ...
        AssetTypeInstance instance(
            1, &base_template, asset_info.curFileSize, assets_file.pReader,
            is_big_endian(assets_file), asset_info.absolutePos);
        ...
        if (payload.picture_data.empty() &&
            !load_stream_payload(bundle, bundle_reader, ab_file, payload))
        {
          continue;
        }
...
#if defined(_WIN32)
        ok = lodepng_encode32_fileW(png_file.c_str(), rgba.data(), payload.width,
                                    payload.height) == 0U;
#else
        const std::string png_path = png_file.string();
        ok = lodepng_encode32_file(png_path.c_str(), rgba.data(), payload.width,
                                   payload.height) == 0U;
#endif
      }
