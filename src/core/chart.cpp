...
void Chart::normalize() {
  ...
  std::stable_sort(
      ...
      [this, from_simai_source](const Note &a, const Note &b) {
        ...
        // Simai parsing path in MaichartConverter puts start anchors before
        // slide bodies on identical stamps.
  ...
  if (bpm_changes_.front().tick_stamp(definition_) != 0) {
    bpm_changes_.insert(bpm_changes_.begin(),
                        BpmChange{0, 0, bpm_changes_.front().bpm});
  }
  if (measure_changes_.front().tick_stamp(definition_) != 0) {
    measure_changes_.insert(measure_changes_.begin(),
...
double Chart::ticks_to_seconds(int tick_stamp) const {
  ...
  for (std::size_t i = 1; i < bpm_changes_.size(); ++i) {
    ...
    if (tick_stamp <= segment_end) {
      total += static_cast<double>(tick_stamp - consumed_tick) *
               bpm_time_unit(current_bpm, definition_);
      return total;
    }
    total += static_cast<double>(segment_end - consumed_tick) *
             bpm_time_unit(current_bpm, definition_);
...
int Chart::seconds_to_ticks(double seconds) const {
  ...
  if (bpm_changes_.empty()) {
    return static_cast<int>(
        std::llround(seconds / bpm_time_unit(120.0, definition_)));
  }
  ...
  for (std::size_t i = 1; i < bpm_changes_.size(); ++i) {
    ...
    const double segment_seconds = static_cast<double>(segment_ticks) *
                                   bpm_time_unit(current_bpm, definition_);
    if (remaining <= segment_seconds + kEpsilon) {
      return accumulated_tick +
             static_cast<int>(std::llround(
                 remaining / bpm_time_unit(current_bpm, definition_)));
    }
  ...
  return accumulated_tick +
         static_cast<int>(
             std::llround(remaining / bpm_time_unit(current_bpm, definition_)));
}
...
void Chart::shift_by_offset(int overall_ticks) {
  ...
  notes_.erase(
      std::remove_if(notes_.begin(), notes_.end(),
                     [](const Note &n) { return n.type == NoteType::Rest; }),
      notes_.end());
