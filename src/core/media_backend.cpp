...
bool media_backend_convert_audio_to_mp3(
    const std::filesystem::path &source,
    const std::filesystem::path &target_mp3) {
  if (!file_non_empty(source)) {
    return false;
...
bool media_backend_convert_acb_awb_to_mp3(
...
bool media_backend_convert_mp3_to_acb_awb(
    const std::filesystem::path &source_mp3,
...
bool media_backend_convert_dat_or_usm_to_mp4(
    const std::filesystem::path &source,
    const std::filesystem::path &target_mp4) {
  if (!file_non_empty(source)) {
    return false;
...
bool media_backend_generate_single_frame_black_mp4(
    const std::filesystem::path &target_mp4) {
  if (!target_mp4.parent_path().empty()) {
    std::filesystem::create_directories(target_mp4.parent_path());
  }
