...
int fixed_last_length_like_mailib(const Chart &chart, int start_tick,
                                  int wait_ticks, int last_ticks) {
...
std::string format_slide_duration(const Chart &chart, int start_tick,
  ...
  double wait_sec = from_simai_source
  ...
  const double duration_sec =
      chart.ticks_to_seconds(end_tick) -
      chart.ticks_to_seconds(start_tick + resolved_wait);
...
std::string format_hold_duration(const Chart &chart, int start_tick,
  ...
  const int resolved_last = std::max(0, note.last_ticks);
  const bool tick_bpm_disagree =
      has_bpm_change_between(chart, start_tick, start_tick + resolved_last);
...
DelayBounds estimate_delay_bounds_like_mailib(const Chart &chart) {
  ...
  for (std::size_t i = 0; i < slide_notes.size(); ++i) {
    ...
    while (found_next) {
      ...
      for (std::size_t j = 0; j < slide_notes.size(); ++j) {
        ...
        current_end_stamp = candidate.tick_stamp(def) + candidate.wait_ticks +
                            candidate.last_ticks;
  ...
  compose_group_notes.reserve(non_slide_notes.size() +
                              grouped_slide_notes.size());
  compose_group_notes.insert(compose_group_notes.end(), non_slide_notes.begin(),
                             non_slide_notes.end());
  compose_group_notes.insert(compose_group_notes.end(),
                             grouped_slide_notes.begin(),
  ...
  for (const auto &note : folded_notes) {
    ...
    bounds.max_end_tick = std::max(bounds.max_end_tick,
                                   stamp + note.wait_ticks + note.last_ticks);
  }
  return bounds;
}
  ...
  for (std::size_t note_index = 0; note_index < chart.notes().size();
    ...
    const auto matching_slide_start_state =
        is_slide_type(note.type) ? find_matching_slide_start_state(note)
                                 : std::nullopt;
    const bool has_matching_slide_start =
        matching_slide_start_state.has_value();
    const SpecialState slide_start_state =
        has_matching_slide_start ? matching_slide_start_state.value()
                                 : SpecialState::Normal;
    ...
    if (note.type == NoteType::Tap || note.type == NoteType::SlideStart) {
      ...
      bundle_prefix += start_state;
      if (!chain_handle.has_value() && !has_matching_slide_start) {
        bundle_prefix += "?";
      }
