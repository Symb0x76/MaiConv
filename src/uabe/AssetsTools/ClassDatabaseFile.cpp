...
#include "uabe/inc/LZMA/LzmaLib.h"
...
#define TEXT(x) x
#endif
inline int MessageBox(void *, const char *text, const char *caption,
                      unsigned int)
{
...
namespace
{
  ...
  inline uint32_t md4_rotl(uint32_t x, uint32_t n)
  {
    return (x << n) | (x >> (32 - n));
  }
  ...
  inline uint32_t md4_h(uint32_t x, uint32_t y, uint32_t z) { return x ^ y ^ z; }
...
#define MD4_R1(a_, b_, c_, d_, k_, s_) \
  a_ = md4_rotl(a_ + md4_f(b_, c_, d_) + x[k_], s_)
#define MD4_R2(a_, b_, c_, d_, k_, s_) \
  a_ = md4_rotl(a_ + md4_g(b_, c_, d_) + x[k_] + 0x5A827999u, s_)
#define MD4_R3(a_, b_, c_, d_, k_, s_) \
  a_ = md4_rotl(a_ + md4_h(b_, c_, d_) + x[k_] + 0x6ED9EBA1u, s_)
...
#undef MD4_R3
  ...
  void md4_update(Md4Context &ctx, const void *data, size_t len)
  {
    ...
    {
      const size_t take =
          (len < (64 - ctx.bufferSize)) ? len : (64 - ctx.bufferSize);
      std::memcpy(ctx.buffer + ctx.bufferSize, ptr, take);
      ctx.bufferSize += take;
      ptr += take;
      len -= take;
    ...
    if (len > 0)
    {
      std::memcpy(ctx.buffer, ptr, len);
      ctx.bufferSize = len;
    }
  }

  void md4_final(Md4Context &ctx, uint8_t out[16])
  {
    const uint64_t totalBits = ctx.totalBytes * 8ull;
    const uint8_t oneBit = 0x80;
    md4_update(ctx, &oneBit, 1);
    ...
    {
      bitLen[i] = static_cast<uint8_t>((totalBits >> (i * 8)) & 0xFFu);
    }
    md4_update(ctx, bitLen, sizeof(bitLen));
  ...
  Hash128
  md4_from_chunks(const std::vector<std::pair<const void *, size_t>> &chunks)
  {
    Hash128 ret = {};
    Md4Context ctx;
    md4_init(ctx);
    ...
    {
      md4_update(ctx, chunk.first, chunk.second);
    }
    uint8_t digest[16];
    md4_final(ctx, digest);
    std::memcpy(ret.bValue, digest, sizeof(digest));
    return ret;
  }

} // namespace

ASSETSTOOLS_API const char *
ClassDatabaseFileString::GetString(ClassDatabaseFile *pFile)
{
  if (!fromStringTable)
    return str.string;
...
ASSETSTOOLS_API QWORD ClassDatabaseFileString::Read(IAssetsReader *pReader,
                                                    QWORD filePos)
{
...
ASSETSTOOLS_API QWORD ClassDatabaseFileString::Write(IAssetsWriter *pWriter,
                                                     QWORD filePos)
{
...
ASSETSTOOLS_API QWORD ClassDatabaseTypeField::Read(IAssetsReader *pReader,
                                                   QWORD filePos, int version)
{
  filePos = typeName.Read(pReader, filePos);
  filePos = fieldName.Read(pReader, filePos);
  pReader->Read(filePos, 1, &depth);
  filePos++;
  pReader->Read(filePos, 1, &isArray);
  filePos++;
  pReader->Read(filePos, 4, &size);
  filePos += 4;
  this->version = 1;
  if (version < 1)
  {
    uint32_t index;
    pReader->Read(filePos, 4, &index);
    filePos += 4;
    if (index & 0x80000000)
    {
      pReader->Read(filePos, 2, &this->version);
      filePos += 2;
    }
  }
  else if (version >= 3)
  ...
  pReader->Read(filePos, 4, &flags2);
  filePos += 4;
  return filePos;
}
ASSETSTOOLS_API QWORD ClassDatabaseTypeField::Write(IAssetsWriter *pWriter,
                                                    QWORD filePos,
                                                    int version)
{
  filePos = typeName.Write(pWriter, filePos);
  filePos = fieldName.Write(pWriter, filePos);
  pWriter->Write(filePos, 1, &depth);
  filePos++;
  pWriter->Write(filePos, 1, &isArray);
  filePos++;
  pWriter->Write(filePos, 4, &size);
  filePos += 4;
  // pWriter->Write(filePos, 4, &index); filePos+=4;
  if (version >= 3)
  {
    pWriter->Write(filePos, 2, &this->version);
    filePos += 2;
  }
  pWriter->Write(filePos, 4, &flags2);
  filePos += 4;
  return filePos;
}
...
ClassDatabaseType::ClassDatabaseType(const ClassDatabaseType &other)
{
  ...
  memcpy(&assemblyFileName, &other.assemblyFileName,
         sizeof(ClassDatabaseFileString));
...
int _RecursiveAddTemplateFieldToClassDatabase(
    std::vector<ClassDatabaseTypeField> &list, uint8_t depth,
    AssetTypeTemplateField *pTemplate)
{
  ...
  if (depth < 255)
  {
    ...
    {
      int curSize = _RecursiveAddTemplateFieldToClassDatabase(
          list, depth + 1, &pTemplate->children[i]);
...
ClassDatabaseType::FromTemplateField(int classId, int baseClass,
                                     AssetTypeTemplateField *pTemplateBase)
{
  if (!pTemplateBase)
    return false;
...
ASSETSTOOLS_API ClassDatabaseType::~ClassDatabaseType()
{
  /*if (freeStrings)
  {
          ...
          if (!assemblyFileName.fromStringTable && assemblyFileName.str.string
  != NULL)
          {
  ...
  field.fieldName.str.string != NULL)
                  ...
                  if (!field.typeName.fromStringTable &&
...
ASSETSTOOLS_API QWORD ClassDatabaseType::Read(IAssetsReader *pReader,
                                              QWORD filePos, int version,
                                              uint8_t flags)
{
  uint32_t fieldCount;
  pReader->Read(filePos, 4, &classId);
  filePos += 4;
  pReader->Read(filePos, 4, &baseClass);
  filePos += 4;
  ...
  pReader->Read(filePos, 4, &fieldCount);
  filePos += 4;

  /*fields = (ClassDatabaseTypeField*)malloc(sizeof(ClassDatabaseTypeField) *
  fieldCount); if (fields == NULL)
  {
          for (uint32_t i = 0; i < fieldCount; i++)
                  filePos = ClassDatabaseTypeField().Read(reader, readerPar,
  filePos); fieldCount = 0;
  }
  for (uint32_t i = 0; i < fieldCount; i++)
  {
...
ASSETSTOOLS_API QWORD ClassDatabaseType::Write(IAssetsWriter *pWriter,
                                               QWORD filePos, int version,
                                               uint8_t flags)
{
  uint32_t fieldCount = (uint32_t)fields.size();

  pWriter->Write(filePos, 4, &classId);
  filePos += 4;
  pWriter->Write(filePos, 4, &baseClass);
  filePos += 4;
  filePos = name.Write(pWriter, filePos);
  if (flags & 1)
    filePos = assemblyFileName.Write(pWriter, filePos);
  pWriter->Write(filePos, 4, &fieldCount);
  filePos += 4;
...
ASSETSTOOLS_API Hash128
ClassDatabaseType::MakeTypeHash(ClassDatabaseFile *pDatabaseFile)
{
  Hash128 ret = {};
  std::vector<std::pair<const void *, size_t>> chunks;
  chunks.reserve(fields.size() * 6);
  for (size_t k = 0; k < fields.size(); k++)
  {
    ...
    chunks.emplace_back(typeName, std::strlen(typeName));
    chunks.emplace_back(fieldName, std::strlen(fieldName));
    chunks.emplace_back(&size, sizeof(size));
    chunks.emplace_back(&isArray, sizeof(isArray));
    chunks.emplace_back(&version, sizeof(version));
    chunks.emplace_back(&flag, sizeof(flag));
  }
  ret = md4_from_chunks(chunks);
  return ret;
}

ASSETSTOOLS_API Hash128 MakeScriptID(const char *scriptName,
                                     const char *scriptNamespace,
                                     const char *scriptAssembly)
{
  Hash128 ret = {};
  const std::vector<std::pair<const void *, size_t>> chunks = {
      {scriptName, std::strlen(scriptName)},
      {scriptNamespace, std::strlen(scriptNamespace)},
      {scriptAssembly, std::strlen(scriptAssembly)}};
  ret = md4_from_chunks(chunks);
  return ret;
}

ASSETSTOOLS_API QWORD ClassDatabaseFileHeader::Read(IAssetsReader *pReader,
                                                    QWORD filePos)
{
  ...
  if (memcmp(header, "cldb", 4))
    return 0;
  filePos += 4;
  pReader->Read(filePos, 1, &fileVersion);
  filePos++;
  if (fileVersion >= 4)
  {
    pReader->Read(filePos, 1, &flags);
    filePos++;
  }
  else
    flags = 0;
  if (fileVersion >= 2)
  {
    pReader->Read(filePos, 1, &compressionType);
    filePos++;
    pReader->Read(filePos, 4, &compressedSize);
    filePos += 4;
    pReader->Read(filePos, 4, &uncompressedSize);
    filePos += 4;
  }
  else
  {
  ...
  if (fileVersion == 0)
  {
    ...
    pReader->Read(filePos, 1, &assetsVersionCount);
    filePos++;
    filePos += assetsVersionCount;
  }
  else
  {
    pReader->Read(filePos, 1, &unityVersionCount);
    filePos++;
  ...
  pReader->Read(filePos, 4, &stringTableLen);
  filePos += 4;
  pReader->Read(filePos, 4, &stringTablePos);
  filePos += 4;
  return filePos;
}
ASSETSTOOLS_API QWORD ClassDatabaseFileHeader::Write(IAssetsWriter *pWriter,
                                                     QWORD filePos)
{
  pWriter->Write(filePos, 4, "cldb");
  filePos += 4;
  pWriter->Write(filePos, 1, &fileVersion);
  filePos++;
  if (fileVersion >= 4)
  {
    pWriter->Write(filePos, 1, &flags);
    filePos++;
  }
  if (fileVersion >= 2)
  {
    pWriter->Write(filePos, 1, &compressionType);
    filePos++;
    pWriter->Write(filePos, 4, &compressedSize);
    filePos += 4;
    pWriter->Write(filePos, 4, &uncompressedSize);
    filePos += 4;
  }
  pWriter->Write(filePos, 1, &unityVersionCount);
  filePos++;
  for (uint32_t i = 0; i < unityVersionCount; i++)
  {
    uint8_t byTmp = (uint8_t)strlen(pUnityVersions[i]);
    pWriter->Write(filePos, 1, &byTmp);
    filePos++;
    pWriter->Write(filePos, byTmp, pUnityVersions[i]);
    filePos += byTmp;
  }
  pWriter->Write(filePos, 4, &stringTableLen);
  filePos += 4;
  pWriter->Write(filePos, 4, &stringTablePos);
  filePos += 4;
  return filePos;
}

ASSETSTOOLS_API QWORD ClassDatabaseFile::Read(IAssetsReader *pReader,
                                              QWORD filePos)
{
  valid = false;
  ...
  if (header.compressionType && header.compressionType < 3)
  {
    ...
    if (pReader->Read(filePos, header.compressedSize, compressedBuf) !=
    ...
    if (header.compressionType == 1)
    {
      ...
      uncompressedSize = (uint32_t)LZ4_decompress_safe(
          compressedBuf, uncompressedBuf, header.compressedSize,
          header.uncompressedSize + 1);
#endif
    }
    else if (header.compressionType == 2 &&
             header.compressedSize > LZMA_PROPS_SIZE)
    {
    ...
    pReader = Create_AssetsReaderFromMemory(uncompressedBuf,
                                            header.uncompressedSize, false);
    if (pReader == NULL)
    {
      free(uncompressedBuf);
      return postHeaderPos + header.compressedSize;
  ...
  uint32_t classCount;
  pReader->Read(filePos, 4, &classCount);
  filePos += 4;
  classes.clear();
  ...
  {
    filePos =
        classes[i].Read(pReader, filePos, header.fileVersion, header.flags);
  }
  /*classes = (ClassDatabaseType*)malloc(sizeof(ClassDatabaseType) *
  classCount); if (classes == NULL)
  {
          for (uint32_t i = 0; i < classCount; i++)
                  filePos = ClassDatabaseType().Read(reader, readerPar,
  filePos); classCount = 0;
  }
  for (uint32_t i = 0; i < classCount; i++)
  {
  ...
  if (pReader->Read(header.stringTablePos, header.stringTableLen,
                    stringTable) != header.stringTableLen)
  {
    if (uncompressedBuf)
    {
      Free_AssetsReader(pReader);
...
{
  // better result without AddStringsOptimized and less memory overhead but
  // slower than hash table
  StringTableWriter_Opt_LinearSearch,
  // memory overhead of 512KiB(32)/1MiB(64) + sizeof(size_t) * stringCount; only
  // better than linear with AddStringsOptimized called before
...
public:
                        ...
                        StringTableWriter_Opt_LinearSearch)
  {
    ...
    {
      stringHashTable = (StringTableHashEntry *)malloc(
          sizeof(StringTableHashEntry) * (MAXWORD + 1)); // 1 MiB
  ...
  // a small bit smaller stringtables than only AddStrings (1260485 vs. 1261082
  // Bytes) but A LOT slower (~0:02 vs. ~1:35)
  bool AddStringsOptimized(const char **strings, size_t count)
  {
    ...
    // StringTableHashEntryTmp *_TmpStringHashTable =
    // (StringTableHashEntryTmp*)malloc(sizeof(StringTableHashEntryTmp) *
    // (MAXWORD+1)); //1 MiB memset(_TmpStringHashTable, 0,
    // sizeof(StringTableHashEntryTmp) * (MAXWORD+1));
    StringTableOptTmp *_TmpStringTable =
    ...
    if (_TmpStringTable)
    {
      if (!_TmpStringHashTable)
      {
        ...
        for (uint32_t i = 0; i < stringCount; i++)
        {
          ...
          _TmpStringTable[curStringIndex].strLen =
              (_TmpStringTable[curStringIndex].str
                   ? strlen(_TmpStringTable[curStringIndex].str)
                   : 0);
        ...
        for (uint32_t i = 0; i < stringCount; i++)
        {
          ...
          if (_TmpStringTable[curStringIndex - 1].str)
          {
            StringTableHashEntryTmp *hashEntry =
                &_TmpStringHashTable[_TmpStringTable[curStringIndex - 1].strLen
                                         ? (*(const uint16_t *)strings[i])
                                         : 0];
            {
              bool isDuplicate = false;
              for (uint32_t l = 0; l < hashEntry->matchCount; l++)
              {
                if ((hashEntry->matches[l]->strLen ==
                     _TmpStringTable[curStringIndex - 1].strLen) &&
                    !memcmp(_TmpStringTable[curStringIndex - 1].str,
                            hashEntry->matches[l]->str,
                            hashEntry->matches[l]->strLen * sizeof(char)))
                {
                  _TmpStringTable[curStringIndex - 1].parent =
                      hashEntry->matches[l];
                  isDuplicate = true;
                  break;
                }
            ...
            {
              void *tmp = realloc(hashEntry->matches,
                                  ((hashEntry->matchCount + 5) & (~3)) *
                                      sizeof(void *));
              if (!tmp)
              {
            ...
            hashEntry->matches[hashEntry->matchCount] =
                &_TmpStringTable[curStringIndex - 1];
      ...
      for (uint32_t i = 0; i < stringCount; i++)
      {
        ...
        {
          if ((k == i) ||
              (_TmpStringTable[k].strLen > _TmpStringTable[i].strLen))
            continue;
          if ((_TmpStringTable[k].str == NULL) ||
              (_TmpStringTable[k].parent != NULL))
            continue;
          // for (size_t l = 0; l <= (_TmpStringTable[i].strLen -
          // _TmpStringTable[k].strLen); l++)
          {
            const char *cmpTarget =
                &_TmpStringTable[i].str[_TmpStringTable[i].strLen -
                                        _TmpStringTable[k].strLen];
            if (!memcmp(_TmpStringTable[k].str, cmpTarget,
                        (_TmpStringTable[k].strLen + 1) * sizeof(char)))
            {
      ...
      return false;
    // MessageBox(NULL, TEXT("Out of memory while optimizing the string
    // table!"), TEXT("ERROR"), 0);
  }
  size_t AddString(const char *string, bool optimize)
  {
    ...
    if (optimize)
    {
      if (stringHashTable)
      {
        // only detects 100% equal strings -> best if AddStringsOptimized was
        // called before
        StringTableHashEntry *pHashEntry = &stringHashTable[hash];
        for (size_t i = 0; i < pHashEntry->matchCount; i++)
          if (!strncmp(&stringTable[pHashEntry->matches[i]], string,
                       strLen + 1))
    ...
    {
      char *pNewStringTable = (char *)realloc(
          stringTable,
          stringTableBufferLen = ((stringTableLen + strLen + 1) + 15) & (~15));
      if (!pNewStringTable)
        return -1;
    ...
    if (stringHashTable)
    {
      // if out of memory, it works less efficiently (depending on how many of
      // the matches arrays are affected)
      StringTableHashEntry *pHashEntry = &stringHashTable[hash];
      bool resized = true;
      if (!pHashEntry->matchCount || !((pHashEntry->matchCount + 1) & 3))
      {
        size_t *tmp = (size_t *)realloc(pHashEntry->matches,
                                        ((pHashEntry->matchCount + 5) & (~3)) *
                                            sizeof(size_t));
        if (!tmp)
        {
  ...
  size_t GetStringTableLen() { return stringTableLen; }
};
ASSETSTOOLS_API QWORD ClassDatabaseFile::Write(IAssetsWriter *pWriter,
                                               QWORD filePos,
                                               int optimizeStringTable,
                                               uint32_t compress,
                                               bool writeStringTable)
{
  ...
  if (writeStringTable)
  {
    ...
    // puts only the strings that can't be interpreted as a part of another
    // string into the string table
    if (optimizeStringTable == 2)
    {
      ...
      const char **stringList =
          (const char **)malloc(sizeof(char *) * stringCount);
      ...
      if (stringList)
      {
        ...
        for (uint32_t i = 0; i < classCount; i++)
        {
          ...
          {
            stringList[curStringIndex] =
                pType->fields[k].fieldName.GetString(this);
            curStringIndex++;
            stringList[curStringIndex] =
                pType->fields[k].typeName.GetString(this);
            curStringIndex++;
          }
        }
        optResult =
            strTableWriter.AddStringsOptimized(stringList, curStringIndex);
        free(stringList);
      }
      if (!optResult)
        MessageBox(NULL,
                   TEXT("Out of memory while optimizing the string table!"),
                   TEXT("ERROR"), 0);
    }
    for (uint32_t i = 0; i < classCount; i++)
    {
      ClassDatabaseType *pType = &classes[i];
      pType->name.str.stringTableOffset = (uint32_t)strTableWriter.AddString(
          pType->name.GetString(this), optimizeStringTable ? true : false);
      pType->name.fromStringTable = true;
      pType->assemblyFileName.str.stringTableOffset =
          (uint32_t)strTableWriter.AddString(
              pType->assemblyFileName.GetString(this),
              optimizeStringTable ? true : false);
      ...
      for (uint32_t k = 0; k < pType->fields.size(); k++)
      {
        ...
        pTypeField->fieldName.str.stringTableOffset =
            (uint32_t)strTableWriter.AddString(
                pTypeField->fieldName.GetString(this),
                optimizeStringTable ? true : false);
        pTypeField->fieldName.fromStringTable = true;
        pTypeField->typeName.str.stringTableOffset =
            (uint32_t)strTableWriter.AddString(
                pTypeField->typeName.GetString(this),
                optimizeStringTable ? true : false);
...
goto_post_checkNeeds3:
  ...
  if (compress)
  {
    ...
    if (!pMemWriter)
    {
      // we don't want to throw an error here, so silently don't compress the
      // file
  ...
  pDataWriter->Write(filePos, 4, &classCount);
  filePos += 4;
  for (uint32_t i = 0; i < classCount; i++)
  {
    filePos = classes[i].Write(pDataWriter, filePos, header.fileVersion,
                               header.flags);
  }
  ...
  header.uncompressedSize =
      filePos - (compress ? 0 : postHeaderPos); // doesn't include the header

  if (compress)
  {
    size_t memWriterPos;
    void *memWriterBuf = NULL;
    ((IAssetsWriterToMemory *)pDataWriter)
        ->GetBuffer(memWriterBuf, memWriterPos);
    ...
    if (compress == 2)
      maxSize =
          (uint32_t)(memWriterPos + memWriterPos / 3 + 128 + LZMA_PROPS_SIZE);
#ifndef _WITHOUT_LZ4
    else
...
#ifndef _WITHOUT_LZ4
      if (compress == 1)
        header.compressedSize = (uint32_t)LZ4_compress_default(
            (char *)memWriterBuf, (char *)compressBuf, (int)memWriterPos,
            maxSize);
      else
#endif
          if (compress == 2)
      {
        size_t destLen = maxSize - LZMA_PROPS_SIZE;
        size_t propsLen = LZMA_PROPS_SIZE;
        int result = LzmaCompress(&((uint8_t *)compressBuf)[LZMA_PROPS_SIZE],
                                  &destLen, (uint8_t *)memWriterBuf,
                                  memWriterPos, (uint8_t *)compressBuf,
                                  &propsLen, -1, 0, -1, -1, -1, -1, -1);
    ...
    {
      // out of memory or compression failure, but we can still write the
      // uncompressed data
      header.compressionType = 0;
      compress = false;
...
ASSETSTOOLS_API bool ClassDatabaseFile::IsValid() { return valid; }

ASSETSTOOLS_API bool ClassDatabaseFile::InsertFrom(ClassDatabaseFile *pOther,
                                                   ClassDatabaseType *pType)
{
  if (this->dontFreeStringTable)
    return false;
  StringTableWriter strTableWriter =
      StringTableWriter(this->stringTable, this->header.stringTableLen);
  ...
  size_t typeNameOffset =
      strTableWriter.AddString(pType->name.GetString(pOther), true);
  ...
  for (size_t i = 0; i < pType->fields.size(); i++)
  {
    ...
    size_t nameOffset =
        strTableWriter.AddString(otherField.fieldName.GetString(pOther), true);
    size_t typeOffset =
        strTableWriter.AddString(otherField.typeName.GetString(pOther), true);
...
ClassDatabaseFile::operator=(const ClassDatabaseFile &other)
{
  valid = other.valid;
  ...
  if (header.unityVersionCount > 0)
  {
    ...
    else
    {
      ...
      for (uint32_t i = 0; i < other.header.unityVersionCount; i++)
      {
        ...
        memcpy(header.pUnityVersions[i], other.header.pUnityVersions[i],
               strLen + 1);
        stringPos += (strLen + 1);
      }
    }
...
{
  /*if ((header.assetsVersions != NULL) && (header.assetsVersions !=
  (uint8_t*)&header._tmp))
  {
...
ASSETSTOOLS_API ClassDatabaseFile::~ClassDatabaseFile() { Clear(); }

ASSETSTOOLS_API void FreeClassDatabase_Dummy(ClassDatabaseFile *pFile) {}

ASSETSTOOLS_API QWORD ClassDatabasePackageHeader::Read(IAssetsReader *pReader,
                                                       QWORD filePos)
{
...
ASSETSTOOLS_API QWORD ClassDatabasePackageHeader::Write(IAssetsWriter *pWriter,
                                                        QWORD filePos)
{
...
ASSETSTOOLS_API bool ClassDatabasePackage::Read(IAssetsReader *pReader)
{
  ...
  if (header.compressionType & 0x80) // Compress files in one block
  {
    ...
    if (pReader->Read(filePos, compressedSize, compressedBuf) !=
        compressedSize)
    {
      free(compressedBuf);
      Clear();
    ...
    {
      pDatabasesReader = Create_AssetsReaderFromMemory(
          compressedBuf, (size_t)compressedSize, false);
    ...
    else
    {
...
#else
        uncompressedSize = (uint32_t)LZ4_decompress_safe(
            compressedBuf, databasesBuf, compressedSize, header.fileBlockSize);
#endif
      }
      ...
      pDatabasesReader = Create_AssetsReaderFromMemory(
          databasesBuf, (size_t)header.fileBlockSize, false);
    }
  }
  ...
  for (uint32_t i = 0; i < header.fileCount; i++)
  {
    ...
    IAssetsReader *pTempReader = Create_AssetsReaderFromReaderRange(
        pDatabasesReader, header.files[i].offset, header.files[i].length);
  ...
  if (pReader->Read(header.stringTableOffset, header.stringTableLenCompressed,
                    compressedBuf) != header.stringTableLenCompressed)
  {
    if (header.compressionType != 0)
      free(compressedBuf);
    Clear();
...
#else
      uncompressedSize = (uint32_t)LZ4_decompress_safe(
          compressedBuf, this->stringTable, header.stringTableLenCompressed,
          header.stringTableLenUncompressed);
#endif
    }
    else if (compressAlgo == 2 &&
             header.stringTableLenCompressed > LZMA_PROPS_SIZE)
    {
      size_t lz_uncompressedSize = header.stringTableLenUncompressed;
      size_t compressedWithoutProps =
          header.stringTableLenCompressed - LZMA_PROPS_SIZE;
      int result = LzmaUncompress(
          (Byte *)this->stringTable, &lz_uncompressedSize,
...
static bool CompressToWriter(QWORD filePos, IAssetsWriter *pWriter,
                             void *uncompressedBuf, uint32_t uncompressedLen,
                             uint8_t compressAlgo, uint32_t &compressedLen)
{
  bool success = true;
  uint32_t maxSize;
  if (compressAlgo == 2)
    maxSize = (uint32_t)(uncompressedLen + uncompressedLen / 3 + 128 +
                         LZMA_PROPS_SIZE);
  else
#ifdef _WITHOUT_LZ4
    return false;
  ...
  if (compressBuf)
  {
#ifndef _WITHOUT_LZ4
    if (compressAlgo == 1)
      compressedLen = (uint32_t)LZ4_compress_default(
          (char *)uncompressedBuf, (char *)compressBuf, (int)uncompressedLen,
          maxSize);
    else
#endif
        if (compressAlgo == 2)
    {
      ...
      size_t propsLen = LZMA_PROPS_SIZE;
      int result = LzmaCompress(&((uint8_t *)compressBuf)[LZMA_PROPS_SIZE],
                                &destLen, (uint8_t *)uncompressedBuf,
                                uncompressedLen, (uint8_t *)compressBuf,
                                &propsLen, -1, 0, -1, -1, -1, -1, -1);
      if ((propsLen != LZMA_PROPS_SIZE) || (result != SZ_OK))
        compressedLen = 0;
      else
...
ASSETSTOOLS_API QWORD ClassDatabasePackage::Write(IAssetsWriter *pWriter,
                                                  QWORD filePos,
                                                  int optimizeStringTable,
                                                  uint32_t compress)
{
  compress =
      (((compress & 0x1F) > 2) ? 0 : (compress & 0x1F)) | (compress & 0xE0);
  ...
  if (optimizeStringTable == 2)
  {
    ...
    if (stringList)
    {
      ...
      for (uint32_t i = 0; i < header.fileCount; i++)
      {
        ...
        for (uint32_t k = 0; k < (uint32_t)pCurFile->classes.size(); k++)
        {
          ...
          {
            stringList[curStringIndex] =
                pType->fields[l].fieldName.GetString(pCurFile);
            curStringIndex++;
            stringList[curStringIndex] =
                pType->fields[l].typeName.GetString(pCurFile);
            curStringIndex++;
          }
        }
      }
    ...
    if (!optResult)
      MessageBox(NULL, TEXT("Out of memory while optimizing the string table!"),
                 TEXT("ERROR"), 0);
  }
  for (uint32_t i = 0; i < header.fileCount; i++)
  {
    ...
    {
      ClassDatabaseType *pType =
          (ClassDatabaseType *)&this->files[i]->classes[k];
      pType->name.str.stringTableOffset = (uint32_t)strTableWriter.AddString(
          pType->name.GetString(this->files[i]),
          optimizeStringTable ? true : false);
      pType->name.fromStringTable = true;
      for (uint32_t l = 0; l < (uint32_t)pType->fields.size(); l++)
      {
        pType->fields[l].fieldName.str.stringTableOffset =
            (uint32_t)strTableWriter.AddString(
                pType->fields[l].fieldName.GetString(this->files[i]),
                optimizeStringTable ? true : false);
        pType->fields[l].fieldName.fromStringTable = true;
        pType->fields[l].typeName.str.stringTableOffset =
            (uint32_t)strTableWriter.AddString(
                pType->fields[l].typeName.GetString(this->files[i]),
                optimizeStringTable ? true : false);
  ...
  this->stringTable = strTableWriter.GetStringTable();
  header.stringTableLenUncompressed =
      (uint32_t)strTableWriter.GetStringTableLen();
  QWORD headerPos = filePos;
  filePos = header.Write(pWriter, filePos);

  {
    ...
    for (uint32_t i = 0; i < header.fileCount; i++)
    {
      ...
      blockFilePos =
          this->files[i]->Write(pFileWriter, blockFilePos, false,
                                (compress & 0x80) ? 0 : compressAlgo, false);
    ...
    if (compress & 0x80)
    {
      ...
      void *pBlockBuffer = nullptr;
      size_t blockBufferSize = 0;
      if (!pBlockWriter->GetBuffer(pBlockBuffer, blockBufferSize))
      {
        MessageBox(NULL, TEXT("Unable to retrieve the raw class databases!"),
                   TEXT("ERROR"), 0);
        header.fileBlockSize = 0;
      }

      uint32_t compressedLen = 0;
      if (!CompressToWriter(filePos, pWriter, pBlockBuffer,
                            header.fileBlockSize, compressAlgo, compressedLen))
  ...
  {
    if (!CompressToWriter(filePos, pWriter, this->stringTable,
                          header.stringTableLenUncompressed, compressAlgo,
                          header.stringTableLenCompressed))
      header.compressionType |= 0x40;
  }
  else
    pWriter->Write(stringTablePos, header.stringTableLenUncompressed,
                   this->stringTable);
  header.Write(pWriter, headerPos);
...
ASSETSTOOLS_API bool ClassDatabasePackage::RemoveFile(uint32_t index)
{
  ...
  memcpy(&newFiles[index], &files[index + 1],
         sizeof(ClassDatabaseFile *) * (header.fileCount - index - 1));
  delete files[index];
  delete[] files;
  files = newFiles;
  header.fileCount--;
...
ASSETSTOOLS_API bool ClassDatabasePackage::IsValid() { return valid; }

ASSETSTOOLS_API ClassDatabasePackage::~ClassDatabasePackage() { Clear(); }