...
#include "uabe/inc/LZMA/LzmaDec.h"
#include "uabe/inc/LZMA/LzmaEnc.h"
...
AssetBundleBlockAndDirectoryList06::Read(QWORD filePos, IAssetsReader *pReader,
                                         AssetsFileVerifyLogger errorLogger)
{
  ...
  case 3:
          ...
          if (!outBuf)
                  goto __goto_allocerror;
          if (LZ4_decompress_safe((char*)dataBuf, (char*)outBuf, (int)sizeComp &
  0x7FFFFFFF, (int)sizeUncomp & 0x7FFFFFFF) != sizeUncomp)
          {
                  free(dataBuf);
                  free(outBuf);
  ...
  case 1:
          //LZMA
          //maybe support this, maybe don't (reading this structure isn't really
  required in our case if the bundle is compressed) free(dataBuf); return false;
          break;
  default:
          break;
  }*/
  ...
  blockInf = (AssetBundleBlockInfo06 *)malloc(this->blockCount *
                                              sizeof(AssetBundleBlockInfo06));
  if (!blockInf)
    goto __goto_allocerror;
  ...
  dirInf = (AssetBundleDirectoryInfo06 *)malloc(
      this->directoryCount * sizeof(AssetBundleDirectoryInfo06));
  if (!dirInf)
    goto __goto_allocerror;
  for (uint32_t i = 0; i < this->directoryCount; i++)
  {
    ...
    if (dirInf[i].name == NULL)
      goto __goto_allocerror;
    if (pReader->Read(filePos, strLen, const_cast<char *>(dirInf[i].name)) !=
        strLen)
      goto __goto_readerror;
...
__goto_allocerror:
  if (errorLogger)
    errorLogger("AssetBundleBlockAndDirectoryList06 : Out of memory!");
  return false;
__goto_readerror:
  if (errorLogger)
    errorLogger(
        "AssetBundleBlockAndDirectoryList06 : A file read error occured!");
  return false;
  /*__goto_decomperror:
  if (errorLogger) errorLogger("AssetBundleBlockAndDirectoryList06 : A
  decompress error occured!"); return false;*/
}
...
ASSETSTOOLS_API bool
AssetBundleBlockAndDirectoryList06::Write(IAssetsWriter *pWriter,
                                          QWORD &curFilePos,
                                          AssetsFileVerifyLogger errorLogger)
{
  uint32_t dwTmp;
...
__goto_writeerror:
  if (errorLogger)
    errorLogger("AssetBundleHeader06 : A file write error occured!");
  return false;
}
ASSETSTOOLS_API bool
AssetBundleHeader06::ReadInitial(IAssetsReader *pReader,
                                 AssetsFileVerifyLogger errorLogger)
{
  QWORD curFilePos = 0;
  char curChar;
...
__goto_readerror:
  if (errorLogger)
    errorLogger("AssetBundleHeader06 : A file read error occured!");
  return false;
}
ASSETSTOOLS_API bool
AssetBundleHeader06::Read(IAssetsReader *pReader,
                          AssetsFileVerifyLogger errorLogger)
{
  QWORD curPos = 0;
  char curChar;
  ...
  else
  {
    ...
    {
      if (errorLogger)
        errorLogger("That file version is unknown!");
      return false;
    }
  }
  ...
  for (unsigned int i = 0;; i++)
  {
    if (!pReader->Read(-1, 1, &curChar))
      goto __goto_readerror;
    if (i < sizeof(minPlayerVersion))
      minPlayerVersion[i] = curChar;
    if (curChar == 0)
      break;
  }
  ...
  for (unsigned int i = 0;; i++)
  {
    if (!pReader->Read(-1, 1, &curChar))
      goto __goto_readerror;
    if (i < sizeof(fileEngineVersion))
      fileEngineVersion[i] = curChar;
    if (curChar == 0)
      break;
  }
...
ASSETSTOOLS_API bool
AssetBundleHeader06::Write(IAssetsWriter *pWriter, QWORD &curFilePos,
                           AssetsFileVerifyLogger errorLogger)
{
  uint32_t dwTmp;
  QWORD qwTmp;
  ...
  {
    QWORD alignmentLen =
        (((curFilePos - startPos) + 15) & ~15) - (curFilePos - startPos);
...
ASSETSTOOLS_API bool
AssetBundleHeader03::Read(IAssetsReader *pReader,
                          AssetsFileVerifyLogger errorLogger)
{
  if (!pReader->Read(0, 9, &this->signature))
    goto __goto_readerror;
  if (_strnicmp(this->signature, "UnityRaw", 9) &&
      _strnicmp(this->signature, "UnityWeb", 9))
  {
    /*if (!_strnicmp(this->signature, "UnityWeb", 9))
    {
            if (errorLogger) errorLogger("AssetBundleHeader : I can't decompress
    compressed files!"); return false;
    }*/
    if (errorLogger)
      errorLogger("AssetBundleHeader : Unknown file type!");
    return false;
  }
  ...
  {
    if (errorLogger)
      errorLogger("AssetBundleHeader : Either that file is invalid or it uses "
                  "an unknown file version!");
    return false;
  }
  ...
  if (!pReader->Read(-1, 4, &this->blockCount))
    goto __goto_readerror;
  SwapEndians_(this->blockCount);
  if (this->pBlockList != NULL)
    free(this->pBlockList);
  this->pBlockList = (AssetBundleOffsetPair *)malloc(
      sizeof(AssetBundleOffsetPair) * blockCount);
  ...
  for (uint32_t i = 0; i < blockCount; i++)
  {
    if (!pReader->Read(-1, 4, &this->pBlockList[i].compressed))
      goto __goto_readerror;
    this->pBlockList[i].compressed =
        SwapEndians(this->pBlockList[i].compressed);
    if (!pReader->Read(-1, 4, &this->pBlockList[i].uncompressed))
      goto __goto_readerror;
    this->pBlockList[i].uncompressed =
        SwapEndians(this->pBlockList[i].uncompressed);
  }
...
__goto_readerror:
  if (errorLogger)
    errorLogger("AssetBundleHeader : A file read error occured!");
  return false;

__goto_outofmemory:
  if (errorLogger)
    errorLogger("AssetBundleHeader : Out of memory!");
  return false;
}
ASSETSTOOLS_API bool
AssetBundleHeader03::Write(IAssetsWriter *pWriter, QWORD &curFilePos,
                           AssetsFileVerifyLogger errorLogger)
{
  uint32_t dwTmp;
  size_t curStrLen;
...
__goto_writeerror:
  if (errorLogger)
    errorLogger("AssetBundleHeader : A file write error occured!");
  return false;
}

ASSETSTOOLS_API QWORD
AssetBundleDirectoryInfo06::GetAbsolutePos(AssetBundleHeader06 *pHeader)
{
  return (this->offset + pHeader->GetFileDataOffset());
}
ASSETSTOOLS_API QWORD
AssetBundleDirectoryInfo06::GetAbsolutePos(class AssetBundleFile *pFile)
{
  return GetAbsolutePos(&pFile->bundleHeader6);
}

ASSETSTOOLS_API unsigned int AssetBundleEntry::GetAbsolutePos(
    AssetBundleHeader03 *pHeader) //, uint32_t listIndex)
{
...
ASSETSTOOLS_API unsigned int AssetBundleEntry::GetAbsolutePos(
    class AssetBundleFile *pFile) //, uint32_t listIndex)
{
  return GetAbsolutePos(&pFile->bundleHeader3); //, listIndex);
}
ASSETSTOOLS_API bool AssetsList::Read(IAssetsReader *pReader, QWORD &curFilePos,
                                      AssetsFileVerifyLogger errorLogger)
{
  ...
  ppEntries = (AssetBundleEntry **)realloc(
      ppEntries, this->count * sizeof(AssetBundleEntry *));
  if (ppEntries == NULL)
    goto __goto_outofmemory;
  ...
  for (unsigned int i = 0; i < this->count; i++)
  {
    ...
    ppEntries[i] = (AssetBundleEntry *)realloc(
        ppEntries[i], sizeof(AssetBundleEntry) - 1 + strLen);
    if (ppEntries[i] == NULL)
      goto __goto_outofmemory;
...
__goto_readerror:
  if (errorLogger)
    errorLogger("AssetsList : A file read error occured!");
  return false;
__goto_outofmemory:
  if (errorLogger)
    errorLogger("AssetsList : Out of memory!");
  return false;
}
...
ASSETSTOOLS_API bool AssetsList::Write(IAssetsWriter *pWriter,
                                       QWORD &curFilePos,
                                       AssetsFileVerifyLogger errorLogger)
{
  uint32_t dwTmp;
...
__goto_writeerror:
  if (errorLogger)
    errorLogger("AssetsList : A file write error occured!");
  return false;
}
ASSETSTOOLS_API bool AssetsList::Write(IAssetsReader *pReader,
                                       IAssetsWriter *pWriter,
                                       bool doWriteAssets, QWORD &curFilePos,
                                       QWORD *curWritePos,
                                       AssetsFileVerifyLogger errorLogger)
{
  ...
  if (doWriteAssets)
  {
    ...
    for (unsigned int i = 0; i < this->count; i++)
    {
      ...
      {
        pReader->Read(setReadPos ? -1 : (this->pos + ppEntries[i]->offset),
                      transferBufferLen, pTransferBuffer);
...
ASSETSTOOLS_API AssetBundleFile::~AssetBundleFile() { this->Close(); }
...
static SRes LZMADecompressBlock(QWORD *fileInPos, QWORD *fileOutPos,
                                QWORD compressedSize, IAssetsReader *pReader,
                                IAssetsWriter *pWriter)
{
#define SizePerBuffer (1024 * 1024)
  ...
  QWORD decompCount = 0;
  void *pCompBuf =
      malloc((compressedSize > SizePerBuffer) ? SizePerBuffer : compressedSize);
  if (!pCompBuf)
  {
  ...
  while (compProcessCount < compressedSize)
  {
    ...
    bytesToProcess =
        bytesToProcess > SizePerBuffer ? SizePerBuffer : bytesToProcess;
    QWORD bytesAvailable =
        pReader->Read(*fileInPos + compProcessCount, bytesToProcess, pCompBuf);
    ...
    res =
        LzmaDec_DecodeToBuf(&dec, (Byte *)pDecompBuf, &decompLen,
                            (Byte *)pCompBuf, &compLen, LZMA_FINISH_ANY, &stat);
    ...
    if (pWriter->Write(*fileOutPos + decompCount, decompLen, pDecompBuf) <
        decompLen)
    {
      LzmaDec_Free(&dec, &g_Alloc);
      free(pCompBuf);
...
static int LZ4DecompressBlock_Read(void *buffer, int size,
                                   LZ4e_instream_t *stream)
{
  if (size <= 0)
    return 0;
  LZ4DecompressBlock_Read_User_t *user =
      (LZ4DecompressBlock_Read_User_t *)stream->user;

  QWORD readSize = (QWORD)size;
  if (stream->pos >= user->dataSize)
    return 0;
  if ((stream->pos + size) > user->dataSize)
    readSize = user->dataSize - stream->pos;

  return (int)user->pReader->Read(stream->pos + user->dataPos, readSize, buffer,
                                  false);
}
static int LZ4DecompressBlock_Write(const void *buffer, int size,
                                    LZ4e_outstream_t *stream)
{
  if (size <= 0)
    return 0;
  const uint8_t *pCur = (const uint8_t *)buffer;
  int remaining = size;
  int lastRead;
  while (remaining &&
         (lastRead = (int)((IAssetsWriter *)stream->user)
                         ->Write((QWORD)-1, (QWORD)remaining, pCur)) > 0)
  {
    pCur += lastRead;
    remaining -= lastRead;
  }
  return size -
         remaining; //(int) ((IAssetsWriter*)stream->user)->Write((QWORD)size,
                    // buffer);
}
static bool LZ4DecompressBlock(QWORD *fileInPos, QWORD *fileOutPos,
                               QWORD compressedSize, IAssetsReader *pReader,
                               IAssetsWriter *pWriter)
{
  ...
  bool ret =
      LZ4e_decompress_safe((char *)pCompBuf, (char *)pDecompBuf, compBufferSize,
                           decompBufferSize, &instream, &outstream) > 0;
...
static bool LZ4CompressBlock(QWORD *fileInPos, QWORD *fileOutPos,
                             QWORD decompressedSize, IAssetsReader *pReader,
                             IAssetsWriter *pWriter)
{
  ...
  bool ret = LZ4e_compress_fast(&instream, &outstream, 1,
...
ASSETSTOOLS_API bool AssetBundleFile::Unpack(IAssetsReader *pReader,
                                             IAssetsWriter *pWriter)
{
  ...
  if (bundleHeader6.fileVersion >= 6)
  {
    ...
    if (compressionType < 4)
    {
      ...
      if (!fileTableBuf)
        return false;
      IAssetsWriter *pTempWriter = Create_AssetsWriterToMemory(
      ...
      case 0:
        if (bundleHeader6.compressedSize == bundleHeader6.decompressedSize &&
            pReader->Read(curFilePos, bundleHeader6.compressedSize,
                          fileTableBuf) == bundleHeader6.decompressedSize)
          decompressSuccess = true;
        break;
      case 1: // LZMA
        if (LZMADecompressBlock(&curFilePos, &curUpFilePos,
                                bundleHeader6.compressedSize, pReader,
                                pTempWriter) == SZ_OK)
          decompressSuccess = true;
        break;
      case 2:
      case 3: // LZ4
        if (LZ4DecompressBlock(&curFilePos, &curUpFilePos,
                               bundleHeader6.compressedSize, pReader,
                               pTempWriter))
          decompressSuccess = true;
        break;
      ...
      if (!decompressSuccess ||
          curUpFilePos != bundleHeader6.decompressedSize)
      {
        free(fileTableBuf);
        return false;
      }

      IAssetsReader *pTempReader = Create_AssetsReaderFromMemory(
          fileTableBuf, bundleHeader6.decompressedSize, false);
      if (!pTempReader)
      {
        free(fileTableBuf);
        return false;
      ...
      else
      {
        ...
        bundleHeader6.decompressedSize =
            (uint32_t)(curUpFilePos - oldUpFilePos);
      }
      ...
      for (uint32_t i = 0; i < list.blockCount; i++)
      {
        ...
        {
        case 0: // none
          if (list.blockInf[i].compressedSize ==
              list.blockInf[i].decompressedSize)
          {
            uint32_t copiedCount = 0;
            uint32_t copyBufLen = 1024 * 1024;
            void *copyBuf = malloc(1024 * 1024);
            uint8_t tmp[256];
            if (!copyBuf)
            {
              copyBuf = tmp;
            ...
            while (copiedCount < list.blockInf[i].compressedSize)
            {
              ...
              uint32_t bytesRead =
                  (uint32_t)pReader->Read(curFilePos, bytesToCopy, copyBuf);
              uint32_t bytesWritten =
                  (uint32_t)pWriter->Write(curUpFilePos, bytesRead, copyBuf);
        ...
        case 1: // LZMA
          if (LZMADecompressBlock(&curFilePos, &curUpFilePos,
                                  list.blockInf[i].compressedSize, pReader,
                                  pWriter) != SZ_OK)
          {
            list.Free();
            return false;
  ...
  else if (bundleHeader3.fileVersion == 3)
  {
    ...
    {
      if ((bundleHeader3.blockCount == 0) ||
          (bundleHeader3.pBlockList[0].compressed < (LZMA_PROPS_SIZE + 8)))
        return false;
#define SizePerBuffer (1024 * 1024)
      ...
      for (uint32_t i = 0; i < bundleHeader.levelCount; i++)
      {
              if (bundleHeader.pLevelList[i].uncompressed > decompressedSize)
              {
                      ...
                      decompressedSize =
      bundleHeader.pLevelList[i].uncompressed;
              }
      }*/
      QWORD decompCount = 0;
      // for (uint32_t i = 0; i < bundleHeader.bundleCount; i++)
      {
        ...
        AssetsList decompEntryList;
        ZeroMemory(&decompEntryList, sizeof(AssetsList));
        {
          // read the entry list
          QWORD bytesToProcess =
              bundleHeader3.pBlockList[0].compressed - compProcessCount;
          bytesToProcess =
              bytesToProcess > SizePerBuffer ? SizePerBuffer : bytesToProcess;
          if (pReader->Read(curFilePos + sizeof(header) + compProcessCount,
                            bytesToProcess, compBuffer) != bytesToProcess)
          {
            LzmaDec_Free(&dec, &g_Alloc);
            free(buffers);
            return false;
          ...
          SizeT decompLen = SizePerBuffer;
          ELzmaStatus stat = LZMA_STATUS_NOT_SPECIFIED;
          res = LzmaDec_DecodeToBuf(&dec, decompBuffer, &decompLen, compBuffer,
                                    &compLen, LZMA_FINISH_ANY, &stat);
          ...
          IAssetsReader *pMemoryReader =
              Create_AssetsReaderFromMemory(decompBuffer, decompLen, false);
          ...
          // sort the entries (this one is slow but the entry lists aren't that
          // large
          bool repeat = false;
          for (uint32_t i = 0; i < decompEntryList.count; i++)
          {
            ...
            {
              if (decompEntryList.ppEntries[k]->offset <
                  decompEntryList.ppEntries[i]->offset)
              {
                AssetBundleEntry *pEntry = decompEntryList.ppEntries[k];
                memcpy(&decompEntryList.ppEntries[i + 1],
                       &decompEntryList.ppEntries[i], (k - i) * sizeof(void *));
          ...
          if (pWriter->Write(curUpFilePos, decompEntryList.ppEntries[0]->offset,
                             decompBuffer) !=
              decompEntryList.ppEntries[0]->offset)
          {
            LzmaDec_Free(&dec, &g_Alloc);
          ...
          LzmaDec_Free(&dec, &g_Alloc);
          LzmaDec_Construct(&dec);
          if (LzmaDec_Allocate(&dec, header, LZMA_PROPS_SIZE, &g_Alloc) !=
              SZ_OK)
          {
            decompEntryList.Free();
            free(buffers);
        ...
        for (uint32_t i = 0; i < decompEntryList.count; i++)
        {
          ...
          if ((i + 1) < decompEntryList.count)
          {
            ...
            decompressBytes += decompEntryList.ppEntries[i + 1]->offset -
                               (decompEntryList.ppEntries[i]->offset +
                                decompEntryList.ppEntries[i]->length);
          }
          do
          {
            ZeroMemory(buffers, 2 * SizePerBuffer);
            // QWORD bytesToProcess = compressedSize - compProcessCount;
            // bytesToProcess = bytesToProcess > SizePerBuffer ? SizePerBuffer :
            // bytesToProcess;
            QWORD bytesToDecompress =
                (decompEntryList.ppEntries[i]->offset + decompressBytes) -
                decompCount;
            bytesToDecompress = bytesToDecompress > SizePerBuffer
                                    ? SizePerBuffer
                                    : bytesToDecompress;
            QWORD bytesToRead =
                bundleHeader3.pBlockList[0].compressed - compProcessCount;
            bytesToRead =
                bytesToRead > SizePerBuffer ? SizePerBuffer : bytesToRead;
            if (pReader->Read(curFilePos + sizeof(header) + compProcessCount,
                              bytesToRead, compBuffer) != bytesToRead)
            {
              LzmaDec_Free(&dec, &g_Alloc);
              decompEntryList.Free();
              free(buffers);
            ...
            SizeT compLen = bytesToRead; // bytesToProcess;
            SizeT decompLen = bytesToDecompress;
            ELzmaStatus stat = LZMA_STATUS_NOT_SPECIFIED;
            res =
                LzmaDec_DecodeToBuf(&dec, decompBuffer, &decompLen, compBuffer,
                                    &compLen, LZMA_FINISH_ANY, &stat);
            if (compLen == 0 || res != SZ_OK)
            {
              LzmaDec_Free(&dec, &g_Alloc);
            ...
            if (pWriter->Write(curUpFilePos + decompCount, decompLen,
                               decompBuffer) != decompLen)
            {
              LzmaDec_Free(&dec, &g_Alloc);
              decompEntryList.Free();
              free(buffers);
      ...
      headerCopy.minimumStreamedBytes = headerCopy.fileSize2 =
          headerCopy.bundleDataOffs + (uint32_t)decompCount;
      /*for (uint32_t i = 0; i < bundleHeader.bundleCount; i++)
      {
              headerCopy.pBundleList[i].compressed =
      headerCopy.pBundleList[i].uncompressed;
      }*/
      /*if (bundleHeader.bundleCount)
      {
              bundleHeader.bundleCount = 1;
              headerCopy.pBundleList[0].compressed =
      headerCopy.pBundleList[0].uncompressed = decompressedSize;
      }*/
      curFilePos = 0;
...
static void LZMAEvent_Init(LZMAAutoResetEvent *pEvent)
{
  pEvent->signaled = false;
}

static void LZMAEvent_Set(LZMAAutoResetEvent *pEvent)
{
  std::lock_guard<std::mutex> lock(pEvent->mutex);
  pEvent->signaled = true;
  pEvent->cv.notify_one();
}

static void LZMAEvent_Reset(LZMAAutoResetEvent *pEvent)
{
  std::lock_guard<std::mutex> lock(pEvent->mutex);
  pEvent->signaled = false;
}

static void LZMAEvent_Wait(LZMAAutoResetEvent *pEvent)
{
  std::unique_lock<std::mutex> lock(pEvent->mutex);
  pEvent->cv.wait(lock, [pEvent]()
                  { return pEvent->signaled; });
  pEvent->signaled = false;
}
...
typedef struct
{
  ...
  LZMAAutoResetEvent dataEvent;
  LZMAAutoResetEvent requestDataEvent;
  bool isDone;
  size_t bufferSize;
  uint8_t *dataBuffer;
} LZMAInStream;
static SRes LZMAInStream_Read(void *p, void *buf, size_t *size)
{
  ...
  std::scoped_lock critSectReadingLock(ctx->critSectReading);
  {
    ...
    if (readSize == 0) //< (*size))
    {
      LZMAEvent_Set(&ctx->requestDataEvent);
      LZMAEvent_Wait(&ctx->dataEvent);
      if (ctx->isDone)
      {
        *size = readSize;
        return SZ_OK;
...
static void LZMAInStream_Init(LZMAInStream *pStream)
{
  ...
  LZMAEvent_Init(&pStream->dataEvent);
  LZMAEvent_Init(&pStream->requestDataEvent);
...
{
  pStream->isDone = true;
  LZMAEvent_Set(&pStream->dataEvent);
  LZMAEvent_Set(&pStream->requestDataEvent);
  std::this_thread::yield(); // Prevent entering the critical section between
                             // LZMAInStream_Read's isDone check and its
...
static void LZMAInStream_Put(LZMAInStream *pStream, void *data, size_t size,
                             bool waitForData)
{
  if (pStream->isDone)
    return;
  ...
  critSectLock.unlock();
  if (waitForData)
    LZMAEvent_Wait(&pStream->requestDataEvent);
}
...
static void LZMACompressThread(void *pData)
{
  ...
  pThreadData->result =
      LzmaEnc_Encode(pThreadData->enc, &pThreadData->pOut->SeqOutStream,
                     &pThreadData->pIn->SeqInStream, NULL, &g_Alloc, &g_Alloc);
  // LZMAInStream_Free(pThreadData->pIn);
  pThreadData->pIn->isDone = true;
  LZMAEvent_Set(&pThreadData->pIn->requestDataEvent);
}

static SRes LZMACompressBlock(QWORD *fileInPos, QWORD *fileOutPos,
                              QWORD decompressedSize, uint32_t alignmentBytes,
                              IAssetsReader *pReader, IAssetsWriter *pWriter)
{
#define SizePerBuffer (1024 * 1024)
  ...
  void *pDecompBuf = malloc(
      (decompressedSize > SizePerBuffer) ? SizePerBuffer : decompressedSize);
  if (!pDecompBuf)
  {
    LzmaEnc_Destroy(enc, &g_Alloc, &g_Alloc);
    return SZ_ERROR_MEM;
  ...
  LZMAInStream seqInStream;
  LZMAInStream_Init(&seqInStream);
  LZMAOutStream seqOutStream = {&LZMAOutStream_Write, pWriter, *fileOutPos,
                                headerSize};
  ...
  std::future<void> compressFuture =
      std::async(LZMACompressThread, &threadData);
  ...
  while (decompProcessCount < decompressedSize)
  {
    ...
    bytesToProcess =
        bytesToProcess > SizePerBuffer ? SizePerBuffer : bytesToProcess;
    QWORD bytesAvailable = pReader->Read(*fileInPos + decompProcessCount,
                                         bytesToProcess, pDecompBuf);
    if (bytesAvailable != bytesToProcess)
    {
...
/*static bool LZ4CompressBlocks(QWORD *fileInPos, QWORD *fileOutPos, QWORD
decompressedSize, IAssetsReader *pReader, IAssetsWriter *pWriter,
        std::vector<AssetBundleOffsetPair> &blockSizes)
{
        ...
        QWORD decompProcessCount = 0;
        QWORD compCount = 0;
        //pDecompBuf is a ring buffer with <decompBufCount> elements. LZ4 uses
the previous buffers as a dictionary, except the one to be compressed at a time.
        size_t decompSizePerBuffer = (decompressedSize > SizePerBuffer) ?
SizePerBuffer : decompressedSize; #define decompBufCount 2 uint8_t *pDecompBuf =
(uint8_t*)malloc(decompBufCount * decompSizePerBuffer); if (!pDecompBuf)
        {
        ...
        while (decompProcessCount < decompressedSize)
        {
                QWORD bytesToProcess = decompressedSize - decompProcessCount;
                bytesToProcess = bytesToProcess > SizePerBuffer ? SizePerBuffer
: bytesToProcess; QWORD bytesAvailable = pReader->Read(*fileInPos +
decompProcessCount, bytesToProcess,
&pDecompBuf[decompBufIndex*decompSizePerBuffer]); if (bytesAvailable !=
bytesToProcess)
                {
                        LZ4_freeStream(pStream);
                        free(pCompBuf);
                ...
                compLen = LZ4_compress_fast_continue(pStream,
(char*)&pDecompBuf[decompBufIndex*decompSizePerBuffer], (char*)pCompBuf,
decompLen, compLen, 1); if (compLen == 0)
                {
                        LZ4_freeStream(pStream);
                        free(pCompBuf);
                        free(pDecompBuf);
                ...
                AssetBundleOffsetPair sizePair; sizePair.compressed =
(uint32_t)compLen; sizePair.uncompressed = (uint32_t)decompLen;
                blockSizes.push_back(sizePair);
                decompProcessCount += decompLen;
                decompBufIndex++;
                if (decompBufIndex > decompBufCount)
                        decompBufIndex = 0;
                if (pWriter->Write(*fileOutPos + compCount, compLen, pCompBuf) <
compLen)
                {
                        LZ4_freeStream(pStream);
                        free(pCompBuf);
...
static bool CopyBlock(QWORD *fileInPos, QWORD *fileOutPos, QWORD totalSize,
                      IAssetsReader *pReader, IAssetsWriter *pWriter)
{
#define SizePerBuffer (1024 * 1024)
  QWORD processedSize = 0;
  void *pDecompBuf =
      malloc((totalSize > SizePerBuffer) ? SizePerBuffer : totalSize);
  if (!pDecompBuf)
  {
    return false;
  }
  ...
  while (processedSize < totalSize)
  {
    ...
    bytesToProcess =
        bytesToProcess > SizePerBuffer ? SizePerBuffer : bytesToProcess;
    QWORD bytesAvailable =
        pReader->Read(*fileInPos + processedSize, bytesToProcess, pDecompBuf);
    if (bytesAvailable != bytesToProcess)
    {
      ret = false;
      goto _GOTO_DO_RETURN;
    ...
    QWORD bytesProcessed =
        pWriter->Write(*fileOutPos + processedSize, bytesToProcess, pDecompBuf);
...
ASSETSTOOLS_API bool
AssetBundleFile::Pack(IAssetsReader *pReader, IAssetsWriter *pWriter,
                      ECompressionTypes *settings,
                      ECompressionTypes fileTableSettings)
{
  ...
  if (bundleHeader3.fileVersion == 3)
  {
    if (!strcmp(bundleHeader3.signature, "UnityRaw"))
    {
      ...
      for (uint32_t i = 0; i < this->assetsLists3[0].count; i++)
      {
        ...
        {
          if (this->assetsLists3[0].ppEntries[k]->offset <
              this->assetsLists3[0].ppEntries[i]->offset)
          {
            AssetBundleEntry *pEntry = this->assetsLists3[0].ppEntries[k];
            memcpy(&this->assetsLists3[0].ppEntries[i + 1],
                   &this->assetsLists3[0].ppEntries[i],
                   (k - i) * sizeof(void *));
...
#define SizePerBuffer (1024 * 1024)
      ...
      // for (uint32_t i = 0; i < bundleHeader.bundleCount; i++)
      {
        ...
        *(QWORD *)&header[LZMA_PROPS_SIZE] = 0;
        ...
        LZMAInStream seqInStream;
        LZMAInStream_Init(&seqInStream);
        LZMAOutStream seqOutStream = {&LZMAOutStream_Write, pWriter,
                                      lzmaHeaderFilePos, headerSize};
        LZMACompressThreadData threadData;
        threadData.pIn = &seqInStream;
        threadData.pOut = &seqOutStream;
        threadData.enc = enc;
        ...
        uint32_t levelIndex = 0;
        {
          ...
          /*SRes result = LzmaEnc_MemEncode(enc, compBuffer, &destLen,
          decompBuffer, srcLen, (this->assetsLists3->count > 0) ? 0 : 1, NULL,
          &g_Alloc, &g_Alloc); if ((destLen == 0) || (result != SZ_OK))
          {
                  free(buffers);
                  LzmaEnc_Destroy(enc, &g_Alloc, &g_Alloc);
                  return false;
          ...
          if (this->assetsLists3->count == 0 &&
              this->bundleHeader3.blockCount > 0)
          {
            this->bundleHeader3.pBlockList[levelIndex].compressed =
                (uint32_t)curCompressOffs;
            this->bundleHeader3.pBlockList[levelIndex].uncompressed =
                (uint32_t)curUncompressOffs;
            levelIndex++;
          }
        }
        ...
        for (uint32_t i = 0; i < this->assetsLists3->count; i++)
        {
          ...
          if ((i + 1) < this->assetsLists3->count)
          {
            ...
            compressBytes += this->assetsLists3->ppEntries[i + 1]->offset -
                             (this->assetsLists3->ppEntries[i]->offset +
                              this->assetsLists3->ppEntries[i]->length);
          }
          ...
          while (curBytesProcessed < compressBytes)
          {
            ...
            /*SRes result = LzmaEnc_MemEncode(enc, compBuffer, &destLen,
            decompBuffer, srcLen,
                    (((i+1) >= this->assetsLists3->count) && (srcLen >=
            writeLen)) ? 1 : 0, NULL, &g_Alloc, &g_Alloc); if ((destLen == 0) ||
            (result != SZ_OK))
            {
                    free(buffers);
                    LzmaEnc_Destroy(enc, &g_Alloc, &g_Alloc);
      ...
      headerCopy.minimumStreamedBytes = headerCopy.fileSize2 =
          headerCopy.bundleDataOffs + (uint32_t)curCompressOffs;
      curFilePos = 0;
      headerCopy.Write(pWriter, curFilePos);
      return true;
    }
  ...
  else if (bundleHeader3.fileVersion >= 6)
  {
    ...
    for (uint32_t i = 0; i < this->bundleInf6->directoryCount; i++)
    {
      ...
      {
        if (this->bundleInf6->dirInf[k].offset <
            this->bundleInf6->dirInf[i].offset)
        {
          AssetBundleDirectoryInfo06 entry = this->bundleInf6->dirInf[k];
          memcpy(&this->bundleInf6->dirInf[i + 1], &this->bundleInf6->dirInf[i],
                 (k - i) * sizeof(AssetBundleDirectoryInfo06));
    ...
    headerCopy.flags =
        0x80 | 0x40; // Don't know the block sizes before having written
                     // everything, so let's simply append the block info.
    headerCopy.Write(pWriter, curFilePos);
    curFilePos = bundleHeader6.GetFileDataOffset();
    curCompFilePos = headerCopy.GetFileDataOffset();

    QWORD curCompressOffs = 0;
    QWORD curAbsOffs = curCompFilePos; //"cleaned" file pointer to the new file,
                                       // as if it was uncompressed.
    uint8_t nullBuf[16] = {};
    for (uint32_t i = 0; i < bundleInf6->directoryCount; i++)
    {
      ...
      {
        if ((nameLen > 9 &&
             !strnicmp(&pDir->name[nameLen - 9], ".resource", 9)) ||
            (nameLen > 10 &&
             !strnicmp(&pDir->name[nameLen - 10], ".resources", 10)) ||
      ...
      {
        curType = settings[i];
        if (curType > (uint32_t)COMPRESS_LZ4)
          curType = COMPRESS_NONE;
      }
      QWORD entryBytesProcessed = 0;
      QWORD blockLimit =
          ((uint32_t)std::numeric_limits<int32_t>::max()) & ~15; // 0x7FFFFFF0
      while (entryBytesProcessed < pDir->decompressedSize)
      {
        ...
        {
        case COMPRESS_NONE:
          if (!CopyBlock(&curFilePos, &curCompFilePos, bytesToProcess, pReader,
                         pWriter))
            return false;
          newBlock.flags = 0; // uncompressed
          break;
        case COMPRESS_LZMA:
          if (LZMACompressBlock(&curFilePos, &curCompFilePos, bytesToProcess, 0,
                                pReader, pWriter) != SZ_OK)
            return false;
          newBlock.flags = 1; // LZMA, not streamed
          break;
        case COMPRESS_LZ4:
          if (!LZ4CompressBlock(&curFilePos, &curCompFilePos, bytesToProcess,
                                pReader, pWriter))
            return false;
    ...
    IAssetsWriterToMemory *pListWriter = Create_AssetsWriterToMemory();
    QWORD listWriterPos = 0;
    listCopy.Write(pListWriter, listWriterPos);
    pListWriter->SetFreeBuffer(true);
    void *pListRaw = nullptr;
    size_t listRawSize = 0;
    ...
    case COMPRESS_LZMA:
    {
      ...
      IAssetsReader *pListReader =
          Create_AssetsReaderFromMemory(pListRaw, listRawSize, false);
      if (LZMACompressBlock(&listReaderPos, &curCompFilePos, listRawSize, 0,
                            pListReader, pWriter) != SZ_OK)
      {
    ...
    case COMPRESS_LZ4:
    {
      ...
      IAssetsReader *pListReader =
          Create_AssetsReaderFromMemory(pListRaw, listRawSize, false);
      if (!LZ4CompressBlock(&listReaderPos, &curCompFilePos, listRawSize,
                            pListReader, pWriter))
      {
        Free_AssetsReader(pListReader);
        Free_AssetsWriter(pListWriter);
...
ASSETSTOOLS_API bool AssetBundleFile::Write(IAssetsReader *pReader,
                                            IAssetsWriter *pWriter,
                                            BundleReplacer **pReplacers,
                                            size_t replacerCount,
                                            AssetsFileVerifyLogger errorLogger,
                                            ClassDatabaseFile *typeMeta)
{
  ...
  {
    if (errorLogger)
      errorLogger("ERROR : Invalid bundle file!");
    return false;
  }
  ...
  std::vector<std::unique_ptr<BundleReplacer, void (*)(BundleReplacer *)>>
      replacersToDelete;
  std::vector<BundleReplacerInfo> replacers;
  // Internal directory: Removed entries are still counted (no shifting of
  // higher indices).
  //                     -> The first <this->bundleInf6[0].directoryCount>
  //                     indices represent original directory entries.
  ...
  // Maps internal directory indices to the latest replacer index for that
  // entry.
  ...
  for (size_t i = 0; i < replacerCount; i++)
  {
    ...
    if (bundleListIndex == (unsigned int)-1 &&
        (entryName != NULL ||
         (entryName = pReplacers[i]->GetEntryName()) != NULL) &&
      ...
      case BundleReplacement_Remove:
        if (entryName != NULL && bundleListIndex != (unsigned int)-1)
        {
          ...
          if (internalToOutputDirectoryMap[bundleListIndex] != -1)
            outputDirectoryNames.erase(
                outputDirectoryNames.begin() +
                internalToOutputDirectoryMap[bundleListIndex]);
          internalToOutputDirectoryMap[bundleListIndex] = (size_t)-1;
          for (size_t k = bundleListIndex + 1;
               k < internalToOutputDirectoryMap.size(); ++k)
          {
            ...
            newEntryName != nullptr)
        {
          ...
          if (internalToOutputDirectoryMap[bundleListIndex] != -1)
            outputDirectoryNames
                [internalToOutputDirectoryMap[bundleListIndex]] = newEntryName;
        }
      }
      break;
      }
    ...
    if (bundleListIndex != (unsigned int)-1)
    {
      ...
      {
        replacers[i].predecessorIndex =
            internalDirectoryLatestReplacerMap[bundleListIndex];
      }
      internalDirectoryLatestReplacerMap[bundleListIndex] = i;
    }
    ...
    else if (bundleListIndex >= numOriginalDirectories &&
             replacers[i].predecessorIndex == (size_t)-1)
    {
      // The replacer cannot be applied, as it is based on previous entry data
      // that doesn't exist. Shouldn't happen, unless an invalid replacers file
      // is read in.
      assert(false);
  ...
  for (size_t i = 0; i < internalToOutputDirectoryMap.size(); i++)
  {
    ...
    {
      if (replacers[k].pReplacer->GetType() !=
          BundleReplacement_Rename) // Skip renamers here.
    ...
    if (replacerReverseOrder.empty())
    {
      ...
      else
      {
        ...
        newReplacer.pReplacer = MakeBundleEntryModifier(
            this->GetEntryName(i), outputDirectoryNames[outputIndex].c_str(),
            hasSerializedData, pReader, NULL, decompressedSize, absolutePos,
            16384, (unsigned int)outputIndex);
        replacersToDelete.push_back(
            std::unique_ptr<BundleReplacer, void (*)(BundleReplacer *)>(
                newReplacer.pReplacer, FreeBundleReplacer));
    ...
    QWORD initSize = 0;
    QWORD initPos = 0;
    if (i < numOriginalDirectories && replacers[replacerReverseOrder.back()]
    ...
    for (auto it = replacerReverseOrder.rbegin();
         !error && it != replacerReverseOrder.rend(); ++it)
    {
      size_t curReplacerIdx = *it;
      BundleReplacerInfo &curReplacer = replacers[curReplacerIdx];
      if (!curReplacer.pReplacer->Init(this, pInitReader, initPos, initSize,
                                       typeMeta))
      {
        error = true;
        break;
      ...
      struct
      {
        void operator()(BundleReplacer *) {}
      } dummyDeleter;
      auto pNextReader = MakeReaderFromBundleEntryModifier(
          std::shared_ptr<BundleReplacer>(curReplacer.pReplacer, dummyDeleter));
      if (pNextReader != nullptr)
      {
        // If this is a simple file/memory-based replacer, we can use its
        // internal resource to generate a reader.
        curReplacer.pReplacer->Uninit();
        pInitReader_refholder = pNextReader;
        pInitReader = pNextReader.get();
        if (!pNextReader->Seek(AssetsSeek_End, 0) ||
            !pNextReader->Tell(initSize) ||
            !pNextReader->Seek(AssetsSeek_Begin, 0))
        {
          error = true;
          break;
        }
      ...
      // The replacer appears to be a more complex one, i.e. FromBundle or
      // FromAssets. Write the replaced file to a memory buffer, and proceed
      // using that buffer as input for the next replacer.
      ...
      size_t memBufferSize = 0;
      if (!pTempWriter->GetBuffer(memBuffer, memBufferSize) ||
          memBufferSize < initSize || !pTempWriter->SetFreeBuffer(false))
      {
      ...
      initPos = 0;
      pNextReader.reset(
          Create_AssetsReaderFromMemory(memBuffer, memBufferSize, false,
    ...
    if (error)
    {
      ...
      std::string warningMessage =
          std::string(
              "Error : Unable to prepare the entry modification for entry ") +
          entryName + ".";
      if (errorLogger)
        errorLogger(warningMessage.c_str());
      continue;
    }
  ...
  {
    std::vector<BundleReplacerInfo> &replacers;
    _Lambda_FreeReplacers(std::vector<BundleReplacerInfo> &replacers)
        : replacers(replacers) {}
    void operator()()
    {
      for (size_t i = 0; i < replacers.size(); ++i)
        if (replacers[i].pReplacer != nullptr)
          replacers[i].pReplacer->Uninit();
    }
  ...
  if (bundleHeader3.fileVersion >= 6)
  {
    ...
    if ((header.flags & 0x100) && header.fileVersion == 6)
      strcpy(header.signature, "UnityWeb"); // Is this accurate for all
                                            // versions?
    ...
    // Create dummy block info (we don't have exact sizes but have to assume
    // that <= or > 4 GiB can be differentiated). Also assign directory names.
    for (size_t i = 0; i < internalToOutputDirectoryMap.size(); i++)
    {
      ...
      directories[outputDirectoryIdx].name =
          outputDirectoryNames[outputDirectoryIdx].c_str();
    ...
    header.compressedSize = header.decompressedSize =
        (uint32_t)(curFilePos - blockAndDirListPos);
    header.flags &= ~(0x80);
    size_t blockIndex = 0;
    for (size_t i = 0; i < internalToOutputDirectoryMap.size(); i++)
    {
      ...
      if (replacer.pReplacer->GetType() == BundleReplacement_AddOrModify)
      {
        ...
        directories[outputDirectoryIdx].decompressedSize =
            curFilePos - beginFilePos;
        directories[outputDirectoryIdx].offset =
            beginFilePos - header.GetFileDataOffset();
        ...
        while (remainingSize >= 0xFFFFFFFFULL)
        {
          if (blocks.size() <= blockIndex)
            break;
          blocks[blockIndex].compressedSize =
              blocks[blockIndex].decompressedSize = 0xFFFFFFFFULL;
          blocks[blockIndex].flags =
              0x40; // TODO: Check if the 'streamed' flag was set before,
        ...
        {
          blocks[blockIndex].compressedSize =
              blocks[blockIndex].decompressedSize = (uint32_t)remainingSize;
  ...
  else if (bundleHeader3.fileVersion == 3)
  {
    ...
    for (size_t i = 0; i < internalToOutputDirectoryMap.size(); i++)
    {
      ...
      AssetBundleEntry *newDirectory = (AssetBundleEntry *)new uint8_t
          [8 + sizeof(char) *
                   (outputDirectoryNames[outputDirectoryIdx].size() + 1)];
      newDirectory->length = newDirectory->offset = 0;
      memcpy(
          newDirectory->name, outputDirectoryNames[outputDirectoryIdx].data(),
          sizeof(char) * (outputDirectoryNames[outputDirectoryIdx].size() + 1));
    ...
    header.unknown2 =
        (uint32_t)(curFilePos - assetsListPos); // not sure if this is right
    ...
    for (size_t i = 0; i < internalToOutputDirectoryMap.size(); i++)
    {
      ...
      if (replacer.pReplacer->GetType() == BundleReplacement_AddOrModify)
      {
        // align to 4 bytes
        uint32_t alignBytes =
            (uint32_t)(((curFilePos + 3) & (~3)) - curFilePos);
        uint32_t dwNull = 0;
        pWriter->Write(curFilePos, alignBytes, &dwNull);
        curFilePos += alignBytes;
        ...
        replacer.pReplacer->Uninit();
        // fix the sizes in the directory info
        directories[replacer.bundleEntryIndex]->length =
            (uint32_t)(curFilePos - beginFilePos);
        directories[replacer.bundleEntryIndex]->offset =
            (uint32_t)(beginFilePos - header.bundleDataOffs);
        // fix the sizes in the block info
        QWORD remainingSize = curFilePos - beginFilePos;
        while (remainingSize >= 0xFFFFFFFFULL)
        {
          if (blocks.size() <= blockIndex)
            break;
          blocks[blockIndex].compressed = blocks[blockIndex].uncompressed =
              0xFFFFFFFFULL;
          blockIndex++;
          remainingSize -= 0xFFFFFFFFULL;
        }
...
ASSETSTOOLS_API bool AssetBundleFile::Read(IAssetsReader *pReader,
                                           AssetsFileVerifyLogger errorLogger,
                                           bool allowCompressed,
                                           uint32_t maxFileTableLen)
{
  Close();
  // this->reader = reader;
  ...
  if (bundleHeader6.fileVersion >= 6)
  {
    ...
    if ((bundleHeader6.flags & 0x3F) != 0)
    {
      ...
      if (compressionType < 4)
      {
        ...
        pTempReader = Create_AssetsReaderFromMemory(
            fileTableBuf, bundleHeader6.decompressedSize, false);
        if (!pTempReader)
        {
          Free_AssetsWriter(pTempWriter);
        ...
        {
        case 1: // LZMA
          if (LZMADecompressBlock(&curInPos, &curOutPos,
                                  bundleHeader6.compressedSize, pReader,
                                  pTempWriter) == SZ_OK)
            decompressSuccess = true;
          break;
        case 2:
        case 3: // LZ4
          if (LZ4DecompressBlock(&curInPos, &curOutPos,
                                 bundleHeader6.compressedSize, pReader,
                                 pTempWriter))
            decompressSuccess = true;
          break;
        ...
        if (!decompressSuccess || curOutPos != bundleHeader6.decompressedSize)
        {
          ...
          if (errorLogger)
            errorLogger(
                "AssetBundleFile.Read : Failed to decompress the directory!");
          return false;
        }
    ...
    {
      bundleInf6 = (AssetBundleBlockAndDirectoryList06 *)malloc(
          sizeof(AssetBundleBlockAndDirectoryList06));
      if (!bundleInf6)
        goto __goto_outofmemory;
  ...
  else if (bundleHeader6.fileVersion == 3)
  {
    ...
    if (!bundleHeader3.Read(pReader, errorLogger))
      return false;
    if (!strcmp(
            bundleHeader3.signature,
            "UnityRaw")) // compressed bundles only have an uncompressed header
    {
  ...
  {
    if (errorLogger)
      errorLogger("AssetBundleFile.Read : Unknown file version!");
    return false;
  }
  return true;
...
__goto_readerror:
  if (errorLogger)
    errorLogger("AssetBundleFile.Read : A file read error occured!");
  goto __goto_errfreebuffers;
__goto_outofmemory:
  if (errorLogger)
    errorLogger("AssetBundleFile.Read : Out of memory!");
  goto __goto_errfreebuffers;
...
{
  if (assetsLists3 == NULL || bundleInf6 == NULL)
    return true;
  if (bundleHeader6.fileVersion >= 6)
  {
...
ASSETSTOOLS_API bool
AssetBundleFile::IsAssetsFile(IAssetsReader *pReader,
                              AssetBundleDirectoryInfo06 *pEntry)
{
  if (pEntry->name)
  {
    ...
    if (entryNameLen >= 5 && !stricmp(&pEntry->name[entryNameLen - 5], ".resS"))
      return false;
    if (entryNameLen >= 9 &&
        !stricmp(&pEntry->name[entryNameLen - 9], ".resource"))
      return false;
  }
  ...
  // mdb, sound bank, dll
  if (((*(QWORD *)&buf[0]) == 0x45E82623FD7FA614ULL) ||
      !strncmp(buf, "FSB5", 4) || !strncmp(buf, "MZ", 2))
    return false;
  IAssetsReaderFromReaderRange *pReaderRange =
      Create_AssetsReaderFromReaderRange(pReader, pos,
                                         pEntry->decompressedSize);
  if (pReaderRange)
  {
...
ASSETSTOOLS_API bool AssetBundleFile::IsAssetsFile(IAssetsReader *pReader,
                                                   AssetBundleEntry *pEntry)
{
...
{
  if (bundleHeader6.fileVersion >= 6)
    return this->bundleInf6 && this->bundleInf6->directoryCount > entryIdx &&
           IsAssetsFile(pReader, &this->bundleInf6->dirInf[entryIdx]);
  else if (bundleHeader6.fileVersion == 3)
    return this->assetsLists3 && this->assetsLists3->count > entryIdx &&
           IsAssetsFile(pReader, this->assetsLists3->ppEntries[entryIdx]);
  return false;
}
ASSETSTOOLS_API IAssetsReader *
AssetBundleFile::MakeAssetsFileReader(IAssetsReader *pReader,
                                      AssetBundleDirectoryInfo06 *pEntry)
{
  QWORD absoluteEntryPos = pEntry->GetAbsolutePos(this);
  QWORD entryLength = pEntry->decompressedSize;
  return Create_AssetsReaderFromReaderRange(pReader, absoluteEntryPos,
                                            entryLength);
}
ASSETSTOOLS_API IAssetsReader *
AssetBundleFile::MakeAssetsFileReader(IAssetsReader *pReader,
