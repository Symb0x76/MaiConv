...
//{
BundleEntryRemover::BundleEntryRemover(const char *name,
...
BundleReplacementType BundleEntryRemover::GetType() {
  return BundleReplacement_Remove;
}
...
unsigned int BundleEntryRemover::GetBundleListIndex() {
  return bundleListIndex;
}

const char *BundleEntryRemover::GetOriginalEntryName() {
...
bool BundleEntryRemover::Init(AssetBundleFile *pBundleFile,
                              IAssetsReader *pEntryReader, QWORD entryPos,
                              QWORD entrySize, ClassDatabaseFile *typeMeta) {
  return true;
}
void BundleEntryRemover::Uninit() {}

QWORD BundleEntryRemover::Write(QWORD pos, IAssetsWriter *pWriter) {
  return pos;
}
...
//{
BundleEntryRenamer::BundleEntryRenamer(const char *oldName, const char *newName,
                                       unsigned int bundleListIndex,
...
BundleReplacementType BundleEntryRenamer::GetType() {
  return BundleReplacement_Rename;
}
...
unsigned int BundleEntryRenamer::GetBundleListIndex() {
  return bundleListIndex;
}

const char *BundleEntryRenamer::GetOriginalEntryName() {
  return originalEntryName;
}
...
QWORD BundleEntryRenamer::Write(QWORD pos, IAssetsWriter *pWriter) {
  return pos;
}
...
BundleEntryModifier::BundleEntryModifier(
    const char *oldName, const char *newName, unsigned int bundleListIndex,
    bool hasSerializedData, std::shared_ptr<IAssetsReader> pReader, QWORD size,
    QWORD readerPos, size_t copyBufferLen)
...
BundleReplacementType BundleEntryModifier::GetType() {
  return BundleReplacement_AddOrModify;
}
BundleEntryModifier::~BundleEntryModifier() {}
...
QWORD BundleEntryModifier::Write(QWORD writerPos, IAssetsWriter *pWriter) {
  ...
  uint8_t stackCopyBuffer[1024];
  size_t actualBufferLen = copyBufferLen;
  ...
  while (remainingSize) {
    size_t curSize =
        (remainingSize > actualBufferLen) ? actualBufferLen : remainingSize;
...
BundleEntryModifierFromMem::BundleEntryModifierFromMem(
    const char *oldName, const char *newName, unsigned int bundleListIndex,
    bool hasSerializedData, void *pMem, size_t size,
...
BundleReplacementType BundleEntryModifierFromMem::GetType() {
  return BundleReplacement_AddOrModify;
}
BundleEntryModifierFromMem::~BundleEntryModifierFromMem() {
...
QWORD BundleEntryModifierFromMem::Write(QWORD writerPos,
                                        IAssetsWriter *pWriter) {
  return writerPos + pWriter->Write(writerPos, size, pMem);
}
QWORD BundleEntryModifierFromMem::WriteReplacer(QWORD pos,
                                                IAssetsWriter *pWriter) {
...
BundleEntryModifierFromAssets::BundleEntryModifierFromAssets(
    const char *oldName, const char *newName, unsigned int bundleListIndex,
    AssetsFile *pAssetsFile, AssetsReplacer **pReplacers, size_t replacerCount,
    uint32_t fileId)
...
BundleEntryModifierFromAssets::BundleEntryModifierFromAssets(
    const char *oldName, const char *newName, unsigned int bundleListIndex,
    std::shared_ptr<ClassDatabaseFile> typeMeta,
    std::vector<std::shared_ptr<AssetsReplacer>> pReplacers, uint32_t fileId)
    : BundleEntryRenamer(oldName, newName, bundleListIndex, true) {
...
BundleReplacementType BundleEntryModifierFromAssets::GetType() {
  return BundleReplacement_AddOrModify;
}
BundleEntryModifierFromAssets::~BundleEntryModifierFromAssets() { Uninit(); }

// Relatively precise; returns all raw asset sizes, the size plus alignment of
// the file table, the header size and the dependencies list size. Does not
// count the TypeTree size and ignores dependency list replacers.
QWORD BundleEntryModifierFromAssets::GetSize() {
  ...
  for (size_t i = 0; i < fileTable.assetFileInfoCount; i++) {
    ...
    for (size_t k = pReplacers.size(); k > 0; k--) {
      if (pReplacers[k - 1]->GetType() == AssetsReplacement_Dependencies) {
        ...
        const std::vector<AssetsFileDependency> &dependencies =
            pDepReplacer->GetDependencies();
        dependencyList.pDependencies =
            const_cast<AssetsFileDependency *>(dependencies.data());
      ...
      AssetsEntryReplacer *pReplacer =
          reinterpret_cast<AssetsEntryReplacer *>(pReplacers[k - 1]);
    ...
    if (!isDeleted)
      ret += ((fileSize + 3) & (~3)) +
             ((fileTable.pAssetFileInfo[i].GetSize(pAssetsFile->header.format) +
               3) &
              (~3));
  }
...
bool BundleEntryModifierFromAssets::Init(AssetBundleFile *pBundleFile,
  ...
  if (pEntryReader == nullptr)
    return false;
  pEntryReader =
      Create_AssetsReaderFromReaderRange(pEntryReader, entryPos, entrySize);
...
QWORD BundleEntryModifierFromAssets::Write(QWORD writerPos,
                                           IAssetsWriter *pWriter) {
  return pAssetsFile->Write(pWriter, writerPos, pReplacers.data(),
                            pReplacers.size(), fileId, typeMeta);
}
QWORD BundleEntryModifierFromAssets::WriteReplacer(QWORD pos,
                                                   IAssetsWriter *pWriter) {
...
uint32_t BundleEntryModifierFromAssets::GetFileID() { return fileId; }
...
BundleReplacementType BundleEntryModifierFromBundle::GetType() {
  return BundleReplacement_AddOrModify;
}
BundleEntryModifierFromBundle::~BundleEntryModifierFromBundle() { Uninit(); }
...
QWORD BundleEntryModifierFromBundle::GetSize() {
  ...
  if (pBundleFile->bundleHeader3.fileVersion == 3) {
    ...
    directorySize = (pBundleFile->assetsLists3 != nullptr)
                        ? pBundleFile->assetsLists3->count
                        : 0;
    ...
    directorySize = (pBundleFile->bundleInf6 != nullptr)
                        ? pBundleFile->bundleInf6->directoryCount
                        : 0;
  }
  ...
  for (uint32_t i = 0; i < directorySize; i++) {
    ...
    } else if (directoryToReplacerMapping[i]->GetType() ==
    ...
    if (directoryToReplacerMapping[i] != nullptr) {
      const char *replacerNewName =
          directoryToReplacerMapping[i]->GetEntryName();
...
bool BundleEntryModifierFromBundle::Init(AssetBundleFile *pBundleFile,
  ...
  if (!this->pBundleFile->Read(this->pBundleReader) ||
      (this->pBundleFile->bundleHeader3.fileVersion == 3 &&
       this->pBundleFile->assetsLists3 == nullptr) ||
      (this->pBundleFile->bundleHeader6.fileVersion >= 6 &&
       this->pBundleFile->bundleInf6 == nullptr) ||
      (this->pBundleFile->bundleHeader6.fileVersion < 6 &&
       this->pBundleFile->bundleHeader3.fileVersion != 3)) {
  ...
  for (size_t i = 0; i < this->pReplacers.size(); i++) {
    ...
    if (bundleListIdx == (unsigned int)-1) {
      ...
      if (this->pBundleFile->bundleHeader3.fileVersion == 3) {
        for (uint32_t iDirEntry = 0;
             iDirEntry < this->pBundleFile->assetsLists3->count; iDirEntry++) {
          if (!strcmp(entryName,
                      this->pBundleFile->assetsLists3->ppEntries[iDirEntry]
                          ->name)) {
        ...
        for (uint32_t iDirEntry = 0;
             iDirEntry < this->pBundleFile->bundleInf6->directoryCount;
             iDirEntry++) {
          if (!strcmp(entryName,
                      this->pBundleFile->bundleInf6->dirInf[iDirEntry].name)) {
            bundleListIdx = iDirEntry;
            break;
          }
        }
    ...
    if (this->pBundleFile->bundleHeader3.fileVersion == 3) {
      bundleEntryPos = this->pBundleFile->assetsLists3->ppEntries[bundleListIdx]
                           ->GetAbsolutePos(pBundleFile);
      bundleEntryLen =
          this->pBundleFile->assetsLists3->ppEntries[bundleListIdx]->length;
    } else if (this->pBundleFile->bundleHeader6.fileVersion >= 6) {
      bundleEntryPos =
          this->pBundleFile->bundleInf6->dirInf[bundleListIdx].GetAbsolutePos(
              pBundleFile);
      bundleEntryLen =
          this->pBundleFile->bundleInf6->dirInf[bundleListIdx].decompressedSize;
    } else {
      assert(false);
      continue;
    }
    this->pReplacers[i]->Init(this->pBundleFile, this->pBundleReader,
                              bundleEntryPos, bundleEntryLen, typeMeta);
  }
  return true;
}
...
QWORD BundleEntryModifierFromBundle::Write(QWORD writerPos,
                                           IAssetsWriter *pWriter) {
  IAssetsWriterToWriterOffset *pWriterWrapper =
      Create_AssetsWriterToWriterOffset(pWriter, writerPos);
  pBundleFile->Write(this->pBundleReader, pWriterWrapper,
                     this->pReplacers.data(), this->pReplacers.size(), nullptr,
                     this->typeMeta);
  QWORD ret = writerPos;
...
QWORD BundleEntryModifierFromBundle::WriteReplacer(QWORD pos,
                                                   IAssetsWriter *pWriter) {
...
BundleEntryModifierByResources::BundleEntryModifierByResources(
    const char *oldName, const char *newName, unsigned int bundleListIndex,
    std::vector<ReplacedResourceDesc> _resources, size_t copyBufferLen)
    : BundleEntryRenamer(oldName, newName, bundleListIndex, false) {
  this->resources = std::move(_resources);
  this->copyBufferLen = copyBufferLen;
}
BundleReplacementType BundleEntryModifierByResources::GetType() {
  return BundleReplacement_AddOrModify;
}
BundleEntryModifierByResources::~BundleEntryModifierByResources() {}
...
void BundleEntryModifierByResources::Uninit() { this->pEntryReader = nullptr; }
...
QWORD BundleEntryModifierByResources::Write(QWORD writerPos,
                                            IAssetsWriter *pWriter) {
  ...
  for (size_t i = 0; i < resources.size(); ++i) {
    ...
    if (readerRange > 0 && pReader != nullptr) {
      BundleEntryModifier tmpCopier(
          nullptr, nullptr, (unsigned int)-1, false,
    ...
    QWORD remaining = (resources[i].outRangeBegin + resources[i].rangeSize) -
                      (writerPos - writerPos_pre);
    if (resources.size() > (i + 1) &&
        resources[i + 1].outRangeBegin > (writerPos - writerPos_pre)) {
    ...
    while (remaining > 0) {
      ...
      QWORD written =
          pWriter->Write(writerPos, curCopyDepth, zeroBuffer.data());
      if (written == 0)
        return writerPos;
      writerPos += written;
...
QWORD BundleEntryModifierByResources::WriteReplacer(QWORD pos,
                                                    IAssetsWriter *pWriter) {
  ...
  uint32_t numResources = (uint32_t)std::min<size_t>(
      resources.size(), std::numeric_limits<uint32_t>::max());
  pos += pWriter->Write(pos, 4, &numResources);

  uint32_t i = 0;
  uint64_t curRangePos = 0;
  for (auto resIt = resources.begin();
       resIt != resources.end() && i < numResources; ++resIt, ++i) {
    assert(resIt->outRangeBegin == curRangePos);
    pos += pWriter->Write(pos, 8, &resIt->rangeSize);

    uint8_t resourceFlags = ((resIt->reader != nullptr) ? (1 << 0) : 0) |
    ...
    if (resIt->reader != nullptr) {
      BundleEntryModifier tmpCopier(nullptr, nullptr, (unsigned int)-1, false,
                                    resIt->reader, resIt->rangeSize,
                                    resIt->inRangeBegin, copyBufferLen);
...
void FreeMemoryResource_ReadBundleReplacer(void *pResource) {
  if (pResource)
    free(pResource);
}
// On allocation error for the names, it returns a valid replacer with NULL
// names (or doesn't return because the new operator fails)
static BundleReplacer *
ReadBundleReplacer(QWORD &pos, IAssetsReader *pReader,
                   std::shared_ptr<IAssetsReader> ref_pReader,
                   bool prefReplacerInMemory) {
  ...
  case BundleReplacer_BundleEntryRemover: {
    if (replacerFileVersion > 1)
      return NULL;
    BundleReplacer *ret =
        new BundleEntryRemover(originalName.get(), (unsigned int)-1);
    return ret;
  }
  case BundleReplacer_BundleEntryRenamer: {
    if (replacerFileVersion > 1)
      return NULL;
    BundleReplacer *ret = new BundleEntryRenamer(
        originalName.get(), newName.get(), (unsigned int)-1, hasSerializedData);
    return ret;
  }
  ...
  case BundleReplacer_BundleEntryModifierFromMem: {
    ...
    if (prefReplacerInMemory) {
      ...
      if (pMem) {
        ...
        ret = new BundleEntryModifierFromMem(
            originalName.get(), newName.get(), (unsigned int)-1,
            hasSerializedData, pMem, size,
    ...
    if (!ret) {
      ...
      ret = new BundleEntryModifier(originalName.get(), newName.get(),
                                    (unsigned int)-1, hasSerializedData,
                                    inner_pReader, size, pos, 0);
    }
    pos += size;
    return ret;
  }
  case BundleReplacer_BundleEntryModifierFromAssets: {
    ...
    BundleReplacer *ret = new BundleEntryModifierFromAssets(
        originalName.get(), newName.get(), (unsigned int)-1, NULL,
        std::move(pReplacers), (uint32_t)-1);
    return ret;
  }
  case BundleReplacer_BundleEntryModifierFromBundle: {
    ...
    BundleReplacer *ret = new BundleEntryModifierFromBundle(
        originalName.get(), newName.get(), (unsigned int)-1,
        std::move(pReplacers));
    return ret;
  }
  case BundleReplacer_BundleEntryModifierByResources: {
      ...
      if (resourceFlags & (1 << 0)) {
        ...
        if (prefReplacerInMemory) {
          ...
          if (pMem) {
            ...
            memset(&((uint8_t *)pMem)[actualRead], 0,
                   resources[i].rangeSize - actualRead);
            resources[i].reader =
                std::shared_ptr<IAssetsReader>(Create_AssetsReaderFromMemory(
                    pMem, resources[i].rangeSize, false, [](void *buf) {
                      if (buf)
                        free(buf);
                    }));
        ...
        if (!resources[i].reader) {
          ...
          if (inner_pReader == nullptr)
            resources[i].reader =
                std::shared_ptr<IAssetsReader>(pReader, nullDeleter);
          else
            resources[i].reader =
                std::shared_ptr<IAssetsReader>(inner_pReader, pReader);
    ...
    BundleReplacer *ret = new BundleEntryModifierByResources(
        originalName.get(), newName.get(), (unsigned int)-1,
...
ASSETSTOOLS_API BundleReplacer *ReadBundleReplacer(QWORD &pos,
                                                   IAssetsReader *pReader,
                                                   bool prefReplacerInMemory) {
  return ReadBundleReplacer(pos, pReader, nullptr, prefReplacerInMemory);
}
ASSETSTOOLS_API BundleReplacer *
ReadBundleReplacer(QWORD &pos, std::shared_ptr<IAssetsReader> pReader,
                   bool prefReplacerInMemory) {
  IAssetsReader *_pReader = pReader.get();
  return ReadBundleReplacer(pos, _pReader, std::move(pReader),
                            prefReplacerInMemory);
}
ASSETSTOOLS_API BundleReplacer *
MakeBundleEntryRemover(const char *name, unsigned int bundleListIndex) {
  return new BundleEntryRemover(name, bundleListIndex);
}
ASSETSTOOLS_API BundleReplacer *
MakeBundleEntryRenamer(const char *oldName, const char *newName,
                       bool hasSerializedData, unsigned int bundleListIndex) {
  return new BundleEntryRenamer(oldName, newName, bundleListIndex,
                                hasSerializedData);
}
ASSETSTOOLS_API BundleReplacer *MakeBundleEntryModifier(
    const char *oldName, const char *newName, bool hasSerializedData,
    IAssetsReader *pReader, cbFreeReaderResource freeReaderCallback, QWORD size,
    QWORD readerPos, size_t copyBufferLen, unsigned int bundleListIndex) {
  struct {
    void operator()(IAssetsReader *) {}
  } nullDeleter;
  ...
  if (freeReaderCallback == nullptr)
    ...
    pReader_shared =
        std::shared_ptr<IAssetsReader>(pReader, freeReaderCallback);
  return new BundleEntryModifier(oldName, newName, bundleListIndex,
                                 hasSerializedData, std::move(pReader_shared),
                                 size, readerPos, copyBufferLen);
}
ASSETSTOOLS_API BundleReplacer *MakeBundleEntryModifier(
    const char *oldName, const char *newName, bool hasSerializedData,
    std::shared_ptr<IAssetsReader> pReader, QWORD size, QWORD readerPos,
    size_t copyBufferLen, unsigned int bundleListIndex) {
  return new BundleEntryModifier(oldName, newName, bundleListIndex,
                                 hasSerializedData, std::move(pReader), size,
                                 readerPos, copyBufferLen);
}
ASSETSTOOLS_API BundleReplacer *
MakeBundleEntryModifierFromMem(const char *oldName, const char *newName,
                               bool hasSerializedData, void *pMem, size_t size,
                               unsigned int bundleListIndex,
                               cbFreeMemoryResource freeResourceCallback) {
  return new BundleEntryModifierFromMem(oldName, newName, bundleListIndex,
                                        hasSerializedData, pMem, size,
                                        freeResourceCallback);
}
ASSETSTOOLS_API BundleReplacer *MakeBundleEntryModifierFromAssets(
    const char *oldName, const char *newName, AssetsFile *pAssetsFile,
    AssetsReplacer **pReplacers, size_t replacerCount, uint32_t fileId,
    unsigned int bundleListIndex) {
  return new BundleEntryModifierFromAssets(oldName, newName, bundleListIndex,
                                           pAssetsFile, pReplacers,
                                           replacerCount, fileId);
}
ASSETSTOOLS_API std::unique_ptr<BundleReplacer>
MakeBundleEntryModifierFromAssets(
    const char *oldName, const char *newName,
    std::shared_ptr<ClassDatabaseFile> typeMeta,
    std::vector<std::shared_ptr<AssetsReplacer>> pReplacers, uint32_t fileId,
    unsigned int bundleListIndex) {
  return std::unique_ptr<BundleReplacer>(new BundleEntryModifierFromAssets(
      oldName, newName, bundleListIndex, std::move(typeMeta),
      std::move(pReplacers), fileId));
}
ASSETSTOOLS_API BundleReplacer *MakeBundleEntryModifierFromBundle(
    const char *oldName, const char *newName, BundleReplacer **pReplacers,
    size_t replacerCount, unsigned int bundleListIndex) {
  return new BundleEntryModifierFromBundle(oldName, newName, bundleListIndex,
                                           pReplacers, replacerCount);
}
ASSETSTOOLS_API std::unique_ptr<BundleReplacer>
MakeBundleEntryModifierFromBundle(
    const char *oldName, const char *newName,
    std::vector<std::unique_ptr<BundleReplacer>> pReplacers,
    unsigned int bundleListIndex) {
  return std::unique_ptr<BundleReplacer>(new BundleEntryModifierFromBundle(
      oldName, newName, bundleListIndex, std::move(pReplacers)));
}
ASSETSTOOLS_API std::unique_ptr<BundleReplacer>
MakeBundleEntryModifierByResources(const char *oldName, const char *newName,
                                   std::vector<ReplacedResourceDesc> resources,
                                   size_t copyBufferLen,
                                   unsigned int bundleListIndex) {
  return std::unique_ptr<BundleReplacer>(new BundleEntryModifierByResources(
      oldName, newName, bundleListIndex, std::move(resources), copyBufferLen));
}
...
// Works only for replacers created through MakeBundleEntryModifier or
// MakeBundleEntryModifierFromMem. For other kinds of replacers, nullptr will be
// returned.
//-> For MakeBundleEntryModifier, the internal reader will be reused.
//-> For MakeBundleEntryModifierFromMem, the internal buffer will be reused and
//the returned shared_ptr will also keep a BundleReplacer reference.
ASSETSTOOLS_API std::shared_ptr<IAssetsReader>
MakeReaderFromBundleEntryModifier(std::shared_ptr<BundleReplacer> pReplacer) {
  if (BundleEntryModifierFromMem *pEntryModifier =
          dynamic_cast<BundleEntryModifierFromMem *>(pReplacer.get())) {
    struct {
      std::shared_ptr<BundleReplacer> pReplacer;
      void operator()(IAssetsReader *pReader) {
    ...
    return std::shared_ptr<IAssetsReader>(
        Create_AssetsReaderFromMemory(pEntryModifier->pMem,
                                      pEntryModifier->size, false),
        deleter);
  } else if (BundleEntryModifier *pEntryModifier =
                 dynamic_cast<BundleEntryModifier *>(pReplacer.get())) {
    struct {
    ...
    return std::shared_ptr<IAssetsReader>(
        Create_AssetsReaderFromReaderRange(pEntryModifier->pReader.get(),
                                           pEntryModifier->readerPos,
                                           pEntryModifier->size, true),
        deleter);
  }
  return nullptr;
}
