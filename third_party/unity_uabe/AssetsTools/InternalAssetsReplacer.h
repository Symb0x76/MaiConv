...
public:
  ...
  AssetsEntryReplacerBase(uint32_t fileID, QWORD pathID, int classID,
                          uint16_t monoScriptIndex);
  ...
  bool GetTypeInfo(std::shared_ptr<ClassDatabaseFile> &pFile,
                   ClassDatabaseType *&pType);
  bool SetTypeInfo(std::shared_ptr<ClassDatabaseFile> pFile,
                   ClassDatabaseType *pType);

  bool GetPreloadDependencies(const AssetPPtr *&pPreloadList,
                              size_t &preloadListSize);
  bool SetPreloadDependencies(const AssetPPtr *pPreloadList,
                              size_t preloadListSize);
  bool AddPreloadDependency(const AssetPPtr &dependency);

  // Writes the elements of AssetsReplacerBase with the version headers (always
  // writes fileID 0).
...
public:
  AssetRemover(uint32_t fileID, QWORD pathID, int classID,
...
public:
  ...
  AssetEntryModifierBase(uint32_t fileID, QWORD pathID, int classID,
                         uint16_t monoScriptIndex);
  ~AssetEntryModifierBase();
  ...
  // Writes the elements of AssetModifierBase and AssetsReplacerBase with their
  // version headers.
  QWORD WriteReplacer(QWORD pos, IAssetsWriter *pWriter);
};
...
public:
  ...
  AssetModifierFromReader(uint32_t fileID, QWORD pathID, int classID,
...
public:
  ...
  AssetModifierFromMemory(uint32_t fileID, QWORD pathID, int classID,
                          uint16_t monoScriptIndex, void *buffer, size_t size,
                          cbFreeMemoryResource freeMemCallback = NULL);
...
public:
  ...
  AssetModifierFromFile(uint32_t fileID, QWORD pathID, int classID,
...
public:
  ...
  AssetsDependenciesReplacerImpl(
