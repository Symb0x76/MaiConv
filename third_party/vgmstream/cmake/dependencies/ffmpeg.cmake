if(USE_FFMPEG)
    ...
    if(USE_FFMPEG AND NOT WIN32 AND (NOT FFmpeg_FOUND OR FFMPEG_PATH OR BUILD_STATIC))
        ...
        if(FFMPEG_PATH)
            ...
            set(FFMPEG_BUILT_LIBS
                ${FFMPEG_LINK_PATH}/libavutil.a
                ${FFMPEG_LINK_PATH}/libavformat.a
                ${FFMPEG_LINK_PATH}/libswresample.a
                ${FFMPEG_LINK_PATH}/libavcodec.a
            )
            set(FFMPEG_CONFIGURE_STAMP ${FFMPEG_BIN}/.ffmpeg-configured.stamp)
            set(FFMPEG_BUILD_STAMP ${FFMPEG_BIN}/.ffmpeg-built.stamp)

            file(MAKE_DIRECTORY ${FFMPEG_BIN})
            add_custom_command(
                OUTPUT ${FFMPEG_CONFIGURE_STAMP}
                COMMAND "${FFMPEG_PATH}/configure" ${FFMPEG_CONF_ARGS}
                COMMAND ${CMAKE_COMMAND} -E touch ${FFMPEG_CONFIGURE_STAMP}
                DEPENDS "${FFMPEG_PATH}/configure"
                BYPRODUCTS ${FFMPEG_BIN}/Makefile
                WORKING_DIRECTORY ${FFMPEG_BIN}
                VERBATIM
            )
            add_custom_target(FFMPEG_CONFIGURE
                DEPENDS ${FFMPEG_CONFIGURE_STAMP}
            )
            add_custom_command(
                OUTPUT ${FFMPEG_BUILT_LIBS} ${FFMPEG_BUILD_STAMP}
                COMMAND make
                COMMAND make install DESTDIR="${FFMPEG_BIN}/bin"
                COMMAND ${CMAKE_COMMAND} -E touch ${FFMPEG_BUILD_STAMP}
                DEPENDS ${FFMPEG_CONFIGURE_STAMP}
                WORKING_DIRECTORY ${FFMPEG_BIN}
                VERBATIM
            )
            add_custom_target(FFMPEG_MAKE
                DEPENDS ${FFMPEG_BUILD_STAMP}
            )
