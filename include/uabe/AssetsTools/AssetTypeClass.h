...
public:
  inline AssetTypeValue_ConfusionError(std::string _desc)
      : desc(std::move(_desc)) {}
  inline const char *what() const noexcept override { return desc.c_str(); }
};
...
public:
  ...
  inline EnumValueTypes GetType() const { return type; }
  // valueContainer :
  //  - For primitive types (bool, int, uint, float, double) this is a pointer
  //  to the value.
  //  - For strings, this is the char* C-style string pointer.
  //  - For Array and ByteArray, this is the AssetTypeArray* /
  //  AssetTypeByteArray* value.
  // type : If set, is checked against GetType() to prevent type confusion. Does
  // NOT change the set value type!
  //  -> Bigger (u)ints can be assigned to smaller; no signed/unsigned check,
  //  bools treated as (u)int8.
  //     For instance, ValueType_UInt64 can be assigned to AssetTypeValue with
  //     GetType()==ValueType_Int32
  //                   but not the other way round.
  //  -> Other types require an exact match.
  //  Throws an AssetTypeValue_ConfusionError if a critical mismatch is
  //  detected.
  ASSETSTOOLS_API void Set(void *valueContainer,
                           EnumValueTypes type = ValueType_None);
...
public:
  ...
  ASSETSTOOLS_API bool FromClassDatabase(ClassDatabaseFile *pFile,
                                         ClassDatabaseType *pType,
                                         uint32_t fieldIndex);
  ASSETSTOOLS_API bool From07(TypeField_07 *pTypeField);
  ASSETSTOOLS_API QWORD MakeValue(IAssetsReader *pReader, QWORD filePos,
                                  QWORD fileLen,
                                  AssetTypeValueField **ppValueField,
                                  bool bigEndian);
...
public:
  ASSETSTOOLS_API void Read(AssetTypeValue *pValue,
  ...
  inline AssetTypeValueField *Get(const char *name) const {
    return (*this)[name];
  }
  inline AssetTypeValueField *Get(unsigned int index) const {
    return (*this)[index];
  }
  ...
  inline AssetTypeTemplateField *GetTemplateField() const {
    return templateField;
  }
  inline AssetTypeValueField **GetChildrenList() const { return pChildren; }
  inline void SetChildrenList(AssetTypeValueField **pChildren,
                              uint32_t childrenCount) {
...
private:
  ...
  operator=(const AssetTypeInstance &other) = delete;
...
public:
  ...
  ASSETSTOOLS_API AssetTypeInstance(uint32_t baseFieldCount,
                                    AssetTypeTemplateField **ppBaseFields,
                                    QWORD fileLen, IAssetsReader *pReader,
                                    bool bigEndian, QWORD filePos = 0);
  ASSETSTOOLS_API bool SetChildList(AssetTypeValueField *pValueField,
