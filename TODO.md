...
- [ ] 定义 Utage 1P/2P 判定优先级
  - [ ] 文件名显式标记优先
  - [ ] 元数据次优先
  - [ ] 无法判定时按 1P 兜底并 warning
...
- [ ] 新增本地 LZ4 适配层（接口与现有调用兼容）
- [ ] 替换以下调用点：
  - [ ] src/core 依赖链涉及的 AssetBundle 解包路径
  - [ ] ClassDatabase 直接解压路径
  - [ ] 相关编码路径（用于打包/写出）
- [ ] 保持流式 callback 机制可用（不可退化为仅一次性 buffer 模式）

### B2. 构建与链接

- [ ] 调整 CMake，切换 LZ4 源依赖到本地实现
...
- [ ] 更新 difficulty/筛选相关逻辑，确保 1P/2P 不互相覆盖
- [ ] 文档说明筛选行为变化
